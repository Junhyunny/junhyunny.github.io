---
title: "Google Tag Manager로 Google Analytics 연결하기"
search: false
category:
  - information
  - data-science
...
사용자로부터 인사이트(insight)를 얻으려면 데이터가 필요하다. 이를 위해 구글 애널리틱스(GA, google analytics)를 많이 사용한다. 애널리틱스를 더 효율적으로 사용하기 위해 구글 태그 매니저(GTM, google tag manager)을 함께 사용한다. 최근 진행 중인 프로젝트에서도 사용자의 행동을 파악하기 위해 GA, GTM을 도입하였다.

이번 포스트에선 직접 도입하면서 배운 내용들을 하나씩 정리해보겠다. 다음과 같은 환경에서 작업하였다.
...
블로그를 운영하기 때문에 애널리틱스는 친숙했지만, 태그 매니저는 잘 몰랐다. 어떤 서비스를 제공하는지 찾아봤다.

- 브라우저가 보여주는 웹 페이지에서 발생하는 이벤트를 트래킹(tracking)하는 도구이다.
- 태그를 기반으로 사용자의 활동을 추적하고 기록한다.
- 웹 분석, 광고 성과 측정, 제휴 페이지 추적 등을 위해 사용한다.

태그 매니저를 잘 활용하려면 태그(tag), 트리거(trigger), 변수(variable)에 대한 개념을 잡아야 한다. 이번 포스트는 애널리틱스와 태그 매니저를 연결하는 방법만 다루기 때문에 관련된 내용은 추후 이어질 포스트에서 다루겠다.

## 2. In Google Analytics

애널리틱스에서 필요한 작업에 대해 먼저 알아보겠다.

### 2.1. Add Property

- 왼쪽 아래 설정을 눌러 관리자 탭으로 이동한다.
- 자신의 계정 하위에 속성을 추가한다.
...
- 데이터를 수집할 서버의 공개 도메인(public domain)을 추가한다.
  - 테스트를 위해 AWS EC2 컨테이너를 사용했다.
...
- 데이터 스트림 설정이 완료되면 아래와 같은 창을 볼 수 있다.
- 구글 태그 매니저와 연결하기 위해선 `G-`로 시작하는 측정 ID가 필요하다.
...
태그 매니저에서 필요한 작업을 살펴보겠다. 이번 포스트에선 태그 매니저의 기능들에 대한 상세한 설명은 다루지 않는다. 간단한 클릭 이벤트만 만들어 구글 애널리틱스에서 감지되는지 살펴보겠다.
...
- 계정을 만들면 태그 관리자를 설치할 수 있는 스크립트를 받는다.
  - 상단 스크립트는 모든 페이지의 헤더(header) 최상단에 위치시킨다.
  - 하단 스크립트는 모든 페이지의 바디(body) 바로 아래 위치시킨다.
- 리액트(react)는 SPA(single page application)이므로 `index.html` 파일에 스크립트를 추가한다.
...
- 계정을 생성한 직후 스크립트를 복사하는 페이지를 놓쳤다면 다음 페이지에서 찾을 수 있다.
- 관리 탭에서 Google 태그 관리자 설치를 선택한다.
...
- 모든 요소에서 발생하는 모든 클릭을 감지한다.
- `Click`이라는 이름으로 트리거를 생성한다.
...
- 태그 구성을 눌러 태그 종류를 선택한다.
- 구글 애널리틱스 GA4 이벤트를 선택한다.
...
- 태그 이름을 `Click`으로 지정한다.
- 측정 ID는 구글 애널리틱스 생성 시 발급받은 `G-`로 시작하는 코드를 입력한다.
- 이벤트 이름을 `click`으로 지정한다.
  - 지정한 이벤트 이름으로 구글 애널리틱스에서 확인 가능하다.
...
- 버전 2로 배포되었다.
- 버전 변경사항엔 생성한 태그, 트리거 정보를 볼 수 있다.
...
- 애널리틱스에서 `click` 이벤트가 감지됨을 확인할 수 있다.
