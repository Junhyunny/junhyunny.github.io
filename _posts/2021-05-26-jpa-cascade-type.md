...
`(정보통신) 종속, 연속, 직렬` 이라는 의미가 오늘 주제인 `JPA Cascade Type`과 가장 잘 어울립니다. 
관련된 포스트들을 보면 JPA에서 Cascade라는 단어는 `영속성(Persistence) 전이`라는 의미로 사용됩니다. 
...
영속성 전이가 일어나는 시점을 엔티티들 사이의 관계를 맺을 때 지정할 수 있습니다. 
...
- PERSIST - 대상(target) 엔티티가 new 상태에서 managed 상태로 변경되는 시점에 종속된 엔티티들의 영속 상태 변경
- REMOVE - 대상 엔티티가 managed 상태에서 removed 상태로 변경되는 시점에 종속된 엔티티들의 영속 상태 변경
- DETACH - 대상 엔티티가 managed 상태에서 detached 상태로 변경되는 시점에 종속된 엔티티들의 영속 상태 변경
- MERGE - 대상 엔티티가 detached 상태에서 managed 상태로 변경되는 시점에 종속된 엔티티들의 영속 상태 변경
- REFRESH - 엔티티 매니저의 refresh() 메소드 호출 시점에 종속된 엔티티들의 영속 상태 변경
- ALL - 모든 상태 변화에 대해 종속된 엔티티들의 영속 상태 변경
...
개념은 얼추 이해가 되지만 역시 테스트를 통해 눈으로 확인하지 않으면 완벽하게 이해했다고 볼 수 없습니다. 
...
- SQL 제약 조건과 관련된 Exception을 볼 수 있습니다.
...
## OPINION
JPA 기능에 대해 예전에 작성한 포스트들을 다시 한번 훑어볼 수 있어서 유익한 시간이었습니다. 
