---
title: "FeignClient AOP 단위 테스트 개선"
search: false
category:
  - spring-boot
...
[애너테이션 기반 AOP(관점 지향 프로그래밍)][annotation-based-aop-link] 포스트를 작성할 당시에는 적절한 테스트 코드를 작성했다고 생각했다.
다음 날 출근해서 포스트에 정리했던 내용을 바탕으로 테스트와 클래스를 구현하다보니 허점이 많았는데, 이번 포스트에서는 부족했던 부분을 어떻게 개선하였는지 정리하였다.

## 1. 첫 테스트 코드와 구현 클래스

처음 작성한 테스트 코드를 살펴보면서 코드의 내용과 작성한 의도에 대해 먼저 살펴본다.

### 1.1. 첫 테스트 코드

- AOP 기능을 테스트해야 하기 때문에 `@SpringBootTest` 애너테이션을 사용하였다.
- `SimpleClient` 빈(bean)을 주입 받아서 테스트에서 호출한다.
- `SimpleClient` 빈의 메서드를 호출할 때마다 AOP 로직에서 이력 성격의 데이터를 추가한다.
- `InterfaceHistoryRepository` 빈을 주입 받아서 AOP 로직에서 데이터베이스에 추가된 데이터가 있는지 확인한다.
- `@Transactional` 애너테이션를 통해 테스트 종료 후 데이터베이스를 롤백한다.
...
- AOP 기능을 이용하여 특정 기능 호출 시점을 가로채어 실행한다.
- `@InterfaceMeta` 애너테이션이 붙은 메서드 호출 시 `aroundCallFeignClient` 메서드가 실행된다.
- 필요한 정보를 추출하여 이력성 테이블에 추가한다.
...
이제 첫 번째 테스트 코드를 작성하고, 구현체를 만들면서 몇 가지 문제점이 있다고 느꼈다.
1. 결합 테스트이기 때문에 실제 빈(bean) 객체가 필요하기 때문에 테스트가 무겁다.
1. `FeignClient` 객체가 실제로 API 요청을 수행하기 때문에 네트워크에 대한 의존성이 존재한다.

테스트는 빠르고, 독립적이어야 하고, 어떤 환경에서도 다시 동작하여 같은 결과를 얻을 수 있어야 한다.
극단적으로 테스트 코드들은 네트워크가 없는 폐쇄망에서도 동작할 수 있어야 한다.
그런 관점에서 봤을 때 처음 작성한 테스트 코드는 네트워크 상황에 따라 결과가 달리질 수 있었고, 실제 데이터베이스에 삽입되는 데이터 값에 대한 검증도 이뤄지지 않았다.
일단 몇 가지 문제점들을 개선하였다.

### 2.2. 1차 테스트 코드 개선

- 테스트 빈을 생성하여 실제 네트워크 동작을 수행하지 않는다.
- 실제 데이터베이스에 추가된 데이터 값에 대한 검증을 수행하였다.
...
일부 테스트 코드가 개선되었지만, 여전히 불합리한 부분이 보였다.
1. 여전히 결합 테스트가 동작하기 때문에 무겁다.
1. 테스트를 위한 `FeignClient`가 별도로 존재한다.

실제 네트워크 호출에 대한 의존성을 제거한 것에 타협하고 남은 테스트들을 작성하였지만, 참을 수 없는 문제점을 하나 발견했다.
메서드 위에 요청 방법을 결정하는 애너테이션들은 요청 경로 값을 넣는 방법에 따라 다른 속성에 값으로 저장한다.

예를 들면 다음과 같다.

- "/" 경로 정보가 `@GetMapping` 애너테이션의 `path` 속성에 저장된다.
...
- "/" 경로 정보가 `@GetMapping` 애너테이션의 `value` 속성에 저장된다.
...
구현한 사람마다 경로를 넣어주는 방법이 달랐는데, 이렇다 보니 어떤 케이스는 테스트가 실패하는 경우가 발생했다.
테스트 통과를 위해 경로 지정 방법을 바꾸기만 하면 되지만, 문제는 테스트 통과를 위해 만든 빈을 고칠 뿐 실제 사용하는 클라이언트를 수정한 것이 아니라는 점이었다.
작성한 테스트 코드는 실제 사용하는 클라이언트에 대한 검증을 해주지 못 했다.

## 4. 2차 테스트 코드 개선

빈을 사용하지 않고 AOP 기능을 테스트하는 방법을 찾다가 좋은 예시를 찾아 적용하였다.
`AspectJProxyFactory` 클래스를 사용하면 빈을 만들지 않아도 AOP 기능을 제공하는 프록시 객체를 만들 수 있다.
이를 통해 다음과 같은 내용들을 개선하였다.

- 빈 주입을 받지 않고 `SimpleClient` 객체를 mock 객체로 생성하였다.
  - 실제 API 요청 없이 필요한 응답 값을 확인할 수 있도록 스터빙(stubbing)이 가능해졌다.
- 빈 주입을 받지 않고 `InterfaceHistoryRepository` 객체를 mock 객체로 생성하였다.
  - 데이터베이스에 직접 조회하지 않고 `save(entity)` 메서드에 전달된 엔티티 객체의 속성 값들 확인이 가능해졌다.
- 빈 주입이 필요 없기 때문에 결합 테스트가 아닌 단위 테스트로 변경하였다.
...
좋지 않은 테스트 코드를 보완하기 위해 많은 시간이 소요되었지만, 새로운 테스트 방법을 찾은 것만으로 크게 만족했다.
