...
* 부모 클래스를 지정합니다.
* 서브젝트(subject) 객체의 메서드를 수행하기 전 기능을 확장할 수 있는 인터셉터를 지정합니다.
...
1. MethodInterceptor 객체의 intercept 메서드가 실행됩니다.
1. intercept 메서드 내부에서 필요한 기능들을 실행한 후 타겟(target) 객체에게 요청을 전달합니다. 
...
* 클래스를 직접 상속하는 방식이기 때문에 메서드가 `final`인 경우 확장할 수 없습니다.
...
MethodInterceptor 인터페이스를 상속받아 interceptor 메서드를 구현합니다. 

* intercept 메서드가 호출됨을 확인하기 위해 로그를 출력합니다.
...
#### Result - invoke_createPost 메서드

* 인터셉터가 실행되지 않고 createPost 메서드만 실행됩니다.
