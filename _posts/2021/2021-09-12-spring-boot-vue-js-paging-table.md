...
`Vue.js` 프레임워크를 사용한 프론트 엔드 서비스부터 살펴보겠다.
...
- 조회 버튼을 클릭 시 `search` 함수를 통해 데이터를 조회한다.
- `fetchList` 함수에 `page` 정보를 전달하여 API 요청을 수행한다.
- 정상적인 응답을 받으면 컴포넌트(component)의 스테이트(state) 정보를 변경한다.
- 정렬 기준과 정렬 방향을 선택할 수 있는 두 개의 select 박스를 추가한다.
- 정렬 기준과 정렬 방향이 바뀔 때마다 search() 함수를 호출하여 API 요청을 수행한다.
- Table 컴포넌트로 다음과 같은 props 정보를 전달한다.
...
- 부모 컴포넌트(component)에게 전달받은 `headerList` props 정보를 이용해 테이블 헤더를 구성한다.
- `itemList` props 정보를 이용해 테이블 행 데이터를 출력한다.
- `itemKeyList` props 정보를 이용해 테이블 행마다 열에 들어갈 데이터를 item 객체에서 추출한다.
- `Pagination` 컴포넌트로 다음과 같은 props 정보를 전달한다.
  - currentPage - 현재 페이지
  - totalPages - 총 페이지 수
  - pageChange - 페이지 변경 이벤트
...
- 페이징 처리를 위한 숫자와 화살표를 출력한다.
- 간단한 계산식을 이용해 5개 단위로 숫자를 출력한다.
- `pageChange` props 정보를 이용해 상위 컴포넌트로 선택한 페이지 번호를 전달한다.
...
- axios 라이브러리를 이용하여 생성한 API 호출 객체를 제공한다.
...
- 전달받은 질의를 request 객체를 이용하여 호출한다.
- `/posts` 경로로 `GET` 요청 방식으로 데이터를 요청한다.
- 전달받은 질의(query)는 params 값으로 전달한다.
...
`Spring Boot` 프레임워크를 사용한 백 엔드 서비스를 살펴보겠다.
...
- CommandLineRunner 인터페이스를 구현하여 서비스 기동 시 데이터를 초기화한다.
...
- `/posts` 경로에서 데이터 조회 기능을 제공한다.
- `@PageableDefault` 애너테이션을 이용해 페이징 관련 정보를 전달받지 못 했을 때 사용할 Pageable 정보를 지정한다.
...
Spring Boot 프레임워크는 API End-Point 메서드에 `Pageable` 인터페이스가 파라미터(parameter)로 들어있는 경우 특별한 기능을 제공한다. API 질의(query)를 통해 전달받은 정보를 이용하여 `Pageable` 구현체를 만들어 제공한다. 간단한 예시를 들어보겠다.
...
- 위와 같은 조건으로 조회할 수 있는 `Pageable` 구현체를 만들어 제공한다.
...
API 요청 질의에 page, size, sort 같은 정보가 없는 경우 `@PageableDefault` 애너테이션에서 지정한 디폴트 값으로 `Pageable` 구현체가 생성된다. 해당 애너테이션을 살펴보면 모두 디폴트(default) 값이 지정되어 있다. `sort` 값만 추가하면 정렬 기능을 사용 가능하다.
...
- 제목, 내용, 생성 일시 정보를 담은 엔티티(entity)이다.
...
- 별도 조회 조건은 없이 `Pageable` 구현체를 사용한 페이징 처리 조회만 제공한다.
- JpaRepository 인터페이스에서 기본적으로 제공하는 findAll 메서드를 사용한다.
...
<div align="center">
  <img src="{{ site.image_url_2021 }}/spring-boot-vue-js-paging-table-01.gif" width="95%">
</div>

## CLOSING

원래 이번 포스트는 간단하게 `@PageableDefault` 애너테이션에 대한 소개로 시작하였다. 작성하다 보니 내용이 단순하여 살을 붙이기 시작했는데 `Vue.js`와 `Spring Boot` 프레임워크를 활용한 페이징 테이블 구현으로 포스트가 마무리되었다. 페이징 처리는 단순할 것 같으면서도 상당히 번거로운 작업이다. 사용하는 기술 스택에 따라 구현 방법이 달라지고, 공통으로 사용하는 컴포넌트로 뽑아내면 입맛에 맞게 사용하기 어려울 때도 있다. 이 포스트가 `Vue.js`와 `Spring Boot` 프레임워크를 사용하는 분들께 많은 도움이 되길 바란다.
