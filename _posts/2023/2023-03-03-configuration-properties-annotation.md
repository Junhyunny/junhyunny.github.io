---
title: "@ConfigurationProperties 애너테이션"
search: false
category:
  - spring-boot
...
`application.yml` 설정에 등록한 값들을 리스트(list)로 사용하는 방법에 대해 정리하였다.

## 1. @Value Annotation

설정 값을 주입 받는 방법 중 가장 흔히 사용된다. `SpEL(Spring Expression Language)` 규칙을 사용하면 더 다양한 형식으로 설정 값을 받을 수 있다. 이번 포스트에선 단순하게 리스트로만 값을 주입 받았다.

### 1.1. application-value.yml

- 다음과 같이 호스트 정보들이 쉼표로 구분되어 있다.
...
- `@Value` 애너테이션을 사용해 설정 값들을 주입 받는다.
- 키(key) 하나에 매칭된 여러 설정 값들을 쉼표(,)로 구분한 경우 배열(array)나 리스트로 주입 받을 수 있다.
- `SpEL`을 사용하면 특정한 문자나 정규식 패턴을 통해 설정 값을 분리 후 주입 받을 수 있다.
...
`@Value` 애너테이션을 사용한 방법은 다음과 같은 불편함이 있다.

- 값이 긴 경우 항목이 많다면 옆으로 길어지기 때문에 `application.yml`의 가독성이 떨어진다.
- 문자열을 다시 파싱(parsing)해서 사용해야 한다.
  - 예상치 못한 버그나 에러가 발생할 수 있다.
- 포트 번호처럼 정수인 경우 파싱된 데이터를 다시 형변환해야 한다.

## 2. @ConfigurationProperties Annotation

`@ConfigurationProperties` 애너테이션을 사용하면 더 나은 방법으로 값을 주입 받을 수 있다.

### 2.1. application-config.yml

- 다음과 같은 방법으로 여러 개의 호스트 정보들을 구분한다.
...
`application.yml` 설정 파일에 명시된 키를 클래스 필드와 동일한 이름으로 지정하면 설정 값들을 객체 모습으로 사용할 수 있다.

- 객체 멤버 변수에 값을 주입 받거나 사용하려면 `Getter`, `Setter` 메서드가 필요하다.
- `@Profile` 애너테이션을 통해 프로파일이 `config`인 경우에만 빈을 생성한다.
  - 이번 포스트의 테스트를 위한 설정이다.
- `@Configuration` 애너테이션을 통해 설정 관련된 빈 객체로 지정한다.
- `@ConfigurationProperties` 애너테이션을 통해 주입 받기 원하는 데이터의 키 값을 접두사(prefix)로 지정한다.
- `redis` 키 하위에 존재하는 `sentinels` 키와 매칭된 클래스 필드를 작성한다.
  - `sentinels`이라는 이름을 가진 필드를 생성한다.
  - `SentinelInstance` 객체를 리스트로 주입 받는다.
- `sentinels` 키 하위에 존재하는 `host`, `port` 키와 매칭된 클래스 필드를 작성한다.
  - `SentinelInstance` 클래스에 `host`, `port`라는 이름을 가진 필드들을 생성한다.
  - `host` 값은 문자열, `port` 값은 정수로 주입 받는다.
...
- 주입 받은 객체의 센티널 정보 리스트 사이즈를 확인한다.
- 각 센티널 정보가 정상적으로 주입되었는지 확인한다.
