...
category:
  - information
  - database
last_modified_at: 2026-09-17T14:54:18+09:00
---
...
- [데이터베이스 파티셔닝(Database Partitioning)][database-partitioning-link]

## 0. 들어가면서

수평 파티셔닝(horizontal partitioning)을 샤딩(sharding)과 혼동하여 사용하는 경우가 많다. 둘은 비슷한 개념이지만, 다른 의미를 가진다. 이번 글에서는 샤딩 개념을 정리하였다.

## 1. Sharding

> 데이터베이스 수평 파티셔닝의 일종으로 여러 데이터베이스 인스턴스(instance)에 분산 저장하는 것을 의미

`수평 파티셔닝은 샤딩이다`라는 말은 정확한 표현이 아니다. 일반적으로 수평 파티셔닝은 하나의 데이터베이스 인스턴스에서 데이터를 분할하는 기법을 의미한다. 샤딩은 수평 파티셔닝을 넘어서 여러 개의 독립된 데이터베이스 인스턴스(혹은 노드, 서버)에 데이터를 저장하는 것이 특징이다. 이때 각 데이터베이스 인스턴스에 저장된 작은 데이터 단위를 샤드(shard)라고 한다.

샤딩 기법은 데이터를 분산 저장하기 위해 샤드 키(shard key)를 사용한다. 샤드 키로는 특정 필드나 컬럼을 사용한다. 이를 기준으로 어떤 샤드에 데이터가 저장될지 결정되므로 적절한 샤드 키를 만드는 것이 중요하다.

샤딩은 분산 처리와 확장성을 고려한 데이터베이스 아키텍처이다. 일반적으로 대규모 시스템에서 활용된다. 데이터베이스 차원의 수평 확장(scale-out)을 의미한다.

예전엔 데이터베이스 수준이 아닌 애플리케이션 수준에서 샤드를 구분하기 위한 구현이 필요했다. 최근엔 플랫폼 차원에서 이를 지원하는 기능을 제공한다.

- 애플리케이션 서버에서 제공
  - Hibernate Shards
- 미들 티어(middle tier)에서 제공
  - Spock Proxy
  - Gizzard
- 데이터베이스에서 제공
  - MongoDB
...
- 데이터가 하나의 샤드에 몰리는 경우 부하가 집중될 수 있다.
  - 적절하지 않은 샤드 키를 사용하면 데이터가 오히려 집중될 수 있다.
  - 데이터베이스 인스턴스 성능을 비대칭적으로 설계하는 방법으로 부하 집중에 대응할 수 있다.
- 여러 개의 데이터베이스 인스턴스와 샤드가 존재하기 때문에 관리가 복잡하다.

## 2. Sharding Types

샤딩에는 어떤 방식이 있을까? 간단히 살펴보자.

### 2.1. Hash Sharding

엔티티의 특정 컬럼 데이터와 해시 함수를 사용해 샤드를 구분한다. 해시 함수로 모듈러(modular) 연산을 사용할 수 있다. 해시 크기는 데이터베이스 개수로 정한다.
...
- 연속된 범위에 따라 데이터를 분산하므로 연속적인 데이터에 접근할 때 성능이 좋다.
