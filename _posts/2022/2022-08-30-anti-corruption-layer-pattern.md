...
- [퍼사드 패턴(Facade Pattern)][facade-pattern-link]
...
부패 방지 계층을 왜 사용할까? 신규 시스템으로 이전하는 과정에서 일부 기능만 옮기면 레거시 시스템의 기능이 필요한 경우가 있다. 시스템 연계는 다음과 같은 어려움을 만날 수 있다.
...
- Facade
  ...
  - 뒤죽박죽인 인터페이스들로 구성된 서브 시스템을 획일화된 하나의 인터페이스로 묶어 쉽게 사용하도록 제공한다.
