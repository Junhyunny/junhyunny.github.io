...
category:
  ...
  - async-await
  - asynchronous-task
last_modified_at: 2026-09-21T17:10:20+09:00
---
...
액터는 여러 동시 작업이 공유 상태에 동시에 접근하지 못하도록 직렬화해 데이터 경합(data race)을 방지하는 참조 타입이다. 변경 가능한 상태(mutable state)를 격리하고, 서스펜션 지점 사이의 액터 격리 코드를 한 번에 하나씩 실행하도록 보장한다. 다시 말해 **여러 태스크가 같은 액터의 격리된 상태에 동시에 접근할 수 없다.**

액터의 격리된 메서드 중간에 서스펜션 지점이 있다면 실행 중이던 태스크가 잠시 서스펜드(suspend)된 후 다른 태스크가 같은 액터에 진입할 수 있다. 이러한 특성을 재진입성(reentrancy)이라고 한다. 이 글에서는 자세히 다루지 않고 별도의 글에서 정리할 예정이다.
...
이처럼 데이터 경합으로 값이 유실되는 것을 방지하기 위해 스위프트는 액터를 사용한다. 위 예제 코드의 클래스를 액터로 변경해 실행해 보자.
...
격리 도메인은 특정 상태와 그 상태에 안전하게 접근할 수 있는 코드가 묶인 영역이다. 각 일반 액터 인스턴스는 독립적인 격리 도메인을 만들고, 글로벌 액터는 여러 선언을 하나의 공유 격리 도메인으로 묶는다. 앞선 컴파일 에러 예제를 통해 액터의 격리가 컴파일 타임에 검사된다는 사실을 살펴봤다.

그렇다면 직렬 실행은 어떻게 가능할까? 이 구조를 이해하려면 [스위프트의 비동기 처리 아키텍처][swift-asynchronous-task-architecture-link]를 알아야 한다. 태스크(task)-실행자(executor)-스레드(thread)로 이어지는 비동기 처리 방식이 낯설다면 이전 글을 먼저 읽어보는 것을 추천한다.

`await` 키워드는 **이 위치에서 태스크가 서스펜드 상태가 될 수 있다는 표시**다. 액터 외부에서 격리된 함수를 호출하는 것은 격리 경계를 넘는 비동기 호출이므로 `await` 키워드가 필요하다. 다른 태스크가 해당 액터의 격리된 코드를 실행 중이라면 호출한 태스크는 직렬 실행자가 작업을 스케줄링할 때까지 서스펜드될 수 있다.
...
**메인 액터는 특별하다.** 메인 액터의 태스크(또는 잡)는 메인 액터 실행자에 의해 반드시 메인 스레드에만 할당된다. iOS에서는 데이터 경합을 방지하기 위해 UIKit과 SwiftUI의 UI 갱신이 반드시 메인 스레드에서만 이루어진다. UI 프레임워크가 스레드 안전성을 보장하지 않기 때문에 여러 스레드가 동시에 뷰(view) 계층을 건드리면 내부 상태가 깨지기 때문이다.

메인 스레드는 다음과 같은 일을 한다.
