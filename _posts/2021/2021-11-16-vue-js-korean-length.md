---
title: "Vue에서 한글 입력 시 v-model 오류"
search: false
category:
  - vue.js
...
`Vue` 프로젝트에서 `v-model` 속성을 통해 한글을 입력받으면 정상적으로 처리가 되지 않는 현상을 발견했다. 이번 포스트에선 한글 입력 시 발생하는 문제점과 원인, 해결 방법에 대해 정리한다.

## 1. 문제 현상

`v-model`을 사용해 한글을 입력 받으면 다음과 같은 문제가 발생한다.

- 한 박자 늦게 한글 입력이 완성된다.
- 그에 따라 입력된 문자열의 길이가 정상적으로 확인되지 않는다.

### 1.1. 문제 코드

- `v-model` 속성에 스테이트(state)를 바로 연결한 양방향 바인딩을 통해 데이터를 입력 받는다.
...
설명에 따르면 `IME` 기능이 필요한 언어들을 `v-model` 속성으로 입력 받으면 IME 합성 과정 중 업데이트가 일어나지 않는다고 한다. 정상적인 업데이트를 위해 `input` 이벤트 사용을 권장하고 있다.
...
공식 홈페이지 설명에 따라 `input` 이벤트를 통해 한글 입력을 받는다.
