...
ES6(ECMAScript6) 이후로 함수나 블록(block) 영역을 기준으로 스코프를 만들 수 있습니다. 

`JavaScript`는 if-else, try-catch 구문 등으로 생기는 블록은 물론이고, 함수 내부에 새로운 함수를 선언함으로써 스코프의 중첩이 발생합니다. 
...
* if 블록 스코프 내부에서 `console.log(x)`를 수행하면 변수 `x`에 대한 참조가 발생합니다.
* 스코프가 중첩되어 있으므로 참조할 수 있는 변수 `x`의 후보들은 여러 개 존재합니다. 
* `x` 변수를 참조하는 if 블록 스코프 내에서 선언된 `x` 변수를 찾을 수 있습니다.
* `JavaScript` 엔진은 if 블록 스코프 내에서 선언된 변수 `x`를 참조하여 `'if block scope'` 문장을 출력합니다.
...
* 위 예시 코드에서 `console.log(y)` 실행은 다음과 같이 수행됩니다.
    * 변수 `y`가 if 블록 스코프에 존재하지 않으므로 상위 inner 함수 스코프를 탐색합니다.
...
* 위 예시 코드에서 `console.log(z)` 실행은 다음과 같이 수행됩니다.
    * 변수 `z`가 if 블록 스코프에 존재하지 않으므로 상위 inner 함수 스코프를 탐색합니다.
