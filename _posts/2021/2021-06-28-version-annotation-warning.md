---
title: "@Version 애너테이션에 의한 TransientPropertyValueException"
search: false
category:
  - spring-boot
  - jpa
...
동시성 문제를 해결하기 위해 낙관적인 락(optimistic lock) 방식으로 구현한 코드에서 문제가 발생했다. JPA 구현체가 엔티티(entity)를 영속성 컨텍스트에 저장하고 인스턴스를 반환하는 과정에서 몇 가지 조건들에 의해 의도와 다르게 동작하면서 예외가 발생했다.

## 1. Problem Context

문제가 발생한 코드와 상황을 최대한 유사하게 재현하였다.

### 1.1. ParentEntity Class

- 낙관적 락을 위해 versionNo 필드를 추가한다.
...
- @Transactional(propagation = Propagation.NOT_SUPPORTED)
  - @DataJpaTest 애너테이션에 적용된 트랜잭션으로부터 테스트 로직의 트랜잭션을 분리하기 위해 추가한다.
- 부모 엔티티를 저장한다.
- 자식 엔티티에 전달하여 둘 사이의 관계를 연결한다.
- 자식 엔티티의 상태를 변경한다.
- 자식 엔티티를 저장한다.
  - InvalidDataAccessApiUsageException 예외가 발생한다.
  - 원인은 TransientPropertyValueException 예외이다.
- 부모 엔티티의 아이디 값이 널(null)이다.
...
- 자식 엔티티에 대한 insert 쿼리가 수행되지 않는다.
...
엔티티에 필드를 하나 추가하면서 정상적으로 동작하던 비즈니스 로직들에서 예외가 발생하기 시작했다. 원인을 살펴보기 전에 엔티티의 라이프사이클(lifecycle)에 대해 간단히 정리할 필요가 있다.
...
이제 어떤 요소가 에러를 유발했는지 연관된 코드를 살펴보겠다. 먼저 SimpleJpaRepository 클래스를 살펴보겠다.

- 전달받은 엔티티 객체가 new 상태라면 영속화(persist)한다.
  - 영속화 후 전달받은 객체를 그대로 반환한다.
- 전달받은 엔티티 객체가 관리 중인 상태라면 영속성 컨텍스트에 병합(merge)한다.
  - 병합 후 결과를 반환한다.
...
JpaMetamodelEntityInformation 클래스의 isNew 메서드를 살펴보겠다.

- 버전 관련된 필드가 있는지 확인한다.
- 버전 관련된 필드가 있다면 해당 타입이 원시(primitive) 타입인지 확인한다.
  - 원시 타입이 아니라면 해당 값이 `null`이어야 `true`를 반환한다.
...
1. 영속성 컨텍스트에 새로 저장할 엔티티 객체인지 판단하는 로직이 다음과 같은 우선 순위를 가진다.
  ...
  2. 버전 관련된 필드가 존재하더라도 원시 타입이라면 엔티티 아이디 필드 값의 null 여부
  3. 버전 관련된 필드가 없다면 엔티티 아이디 필드 값의 null 여부
2. new 상태의 객체라면 영속화하고 전달받은 파라미터를 반환한다.
3. new 상태의 객체가 아니라면 병합하고 결과 객체를 새로 만들어 반환한다.

문제를 유발하는 코드들을 쭉 살펴보고 원인을 요약하면 다음과 같다.

1. 엔티티에 버전 관련된 필드가 래퍼 클래스 타입으로 추가되었다.
2. 버전 관련된 필드의 값이 `null`이 아니기 때문에 새로 생성한 객체임에도 병합 작업이 진행된다.
  - 영속화 작업에선 전달받은 엔티티를 변경하고, 해당 엔티티 객체를 결과로 반환한다.
  - 병합 작업에선 전달받은 엔티티를 변경하지 않고 쿼리 수행 결과를 새로운 엔티티 객체로 만들어 반환한다.

##### AS-IS

버전 필드를 추가하기 전 문제가 되지 않는 코드의 흐름이다.
...
엔티티 클래스에 추가된 버전 필드의 디폴트 값을 제거하거나 원시 타입을 사용한다.
