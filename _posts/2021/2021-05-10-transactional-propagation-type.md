...
- [트랜잭션(transaction) ACID 특징][transaction-acid-link]

## 0. 들어가면서

데이터베이스 트랜잭션은 기본적으로 원자성(atomic)을 만족해야 한다.
...
- JpaRepository 인터페이스 사용
  ...
  - save, saveAndFlush 메서드는 `REQUIRED` 전파 타입이 적용된다. 
  - 진행 중인 트랜잭션이 없다면 save, saveAndFlush 메서드가 완료됨과 동시에 커밋된다.
  - 쓰기 지연 특징으로 인해 삽입(insert) 쿼리가 나중에 실행되므로 테스트에선 saveAndFlush 메서드를 사용한다. 
- @DataJpaTest 애너테이션 사용
  - JPA 관련 컨텍스트만 사용하기 위해 @DataJpaTest 애너테이션을 사용하였다.
...
ChildService 객체는 데이터를 저장하고 ChildException 예외를 던진다.
...
ParentService 객체는 데이터를 저장하고 자식 서비스를 호출한다. 자식 서비스에서 예외를 던지더라도 부모 서비스에서 예외 전파를 차단한다.
...
- 부모와 자식이 모두 `REQUIRED` 전파 타입으로 지정한 경우
  ...
  - 부모와 자식은 하나의 트랜잭션으로 연결되어 있기 때문에 자식에서 예외가 발생했다면, 부모는 커밋 수행 중에 `UnexpectedRollbackException` 예외를 던지고 트랜잭션을 롤백한다.
- 부모는 트랜잭션을 시작하지 않고, 자식은 `REQUIRED` 전파 타입으로 지정한 경우
  - 부모는 트랜잭션을 시작하지 않았으므로 saveAndFlush 메서드를 호출하면 JpaRepository 인터페이스 트랜잭션에 의해 커밋된다. 
  - 자식에서 예외가 발생하면 자식 서비스의 트랜잭션만 롤백될 것을 예상한다.
...
부모와 자식이 모두 `REQUIRED` 전파 타입으로 지정한 경우 테스트는 정상적으로 통과한다. 부모 서비스 트랜잭션에서 생성된 `1127171622` 세션 객체를 통해 기존 트랜잭션을 이어가는 것을 확인할 수 있다. 다음과 같은 로그가 출력된다.
...
부모는 트랜잭션을 시작하지 않고, 자식은 `REQUIRED` 전파 타입으로 지정한 경우 테스트는 정상적으로 통과한다. 부모 서비스 트랜잭션에서 `480975330` 세션 객체, 자식 서비스 트랜잭션에서 `273401463` 세션 객체를 각각 따로 사용한다. 다음과 같은 로그가 출력된다.
...
부모는 `REQUIRED`, 자식은 `SUPPORTS` 전파 타입으로 지정한 경우 테스트는 정상적으로 통과한다. 부모 서비스 트랜잭션에서 생성된 `1127171622` 세션 객체를 통해 기존 트랜잭션을 이어가는 것을 확인할 수 있다. 다음과 같은 로그가 출력된다.
...
부모는 트랜잭션을 시작하지 않고, 자식은 `SUPPORTS` 전파 타입으로 지정한 경우 테스트는 정상적으로 통과한다. 부모 서비스 트랜잭션에서 `222122132` 세션 객체, 자식 서비스 트랜잭션에서 `684660636` 세션 객체를 각각 따로 사용한다. 실행 로그는 다음과 같다.
...
ChildService 객체는 데이터를 저장하고 별도의 예외는 던지지 않는다.
...
ParentService 객체는 데이터를 저장하고 자식 서비스를 호출한다.
...
- 자식 서비스는 예외 발생으로 비즈니스 로직이 실행되지 못하면서 데이터가 저장되지 않는다.
...
자식 서비스에서 조회하거나 데이터를 추가(insert)하는 쿼리가 실행되지 않는다. 테스트를 수행하면 다음과 같은 로그를 볼 수 있다. 
...
- 부모는 `REQUIRED`, 자식은 `REQUIRES_NEW` 전파 타입으로 지정한 경우
  ...
  - 자식 서비스는 데이터가 롤백되는 것을 예상한다.
- 부모 서비스의 비즈니스 로직은 동일하지만, 부모, 자식 모두 `REQUIRED` 전파 타입으로 지정한 경우와 결과가 사뭇 다르다.
  - `UnexpectedRollbackException` 예외가 발생하지 않는다.
  - 부모 트랜잭션은 정상적으로 데이터가 저장된다.
...
테스트는 정상적으로 통과한다. 부모 서비스 트랜잭션에서 `1254589807` 세션 객체, 자식 서비스 트랜잭션에서 `456897159` 세션 객체를 각각 따로 사용한다. 테스트 수행 로그는 다음과 같다.
...
- 자식 서비스에서 saveAndFlush 메서드 호출 후 예외를 던졌기 때문에 JpaRepository 인터페이스 트랜잭션에 의해 데이터가 저장된다.
- 자식 서비스에서 트랜잭션을 이어나가지 않은 상태에서 예외를 던졌기 때문에 부모 서비스가 예외를 상위로 던지지 않는다면 트랜잭션은 정상적으로 커밋된다.
...
`Suspending current transaction` 로그를 통해 `308998656` 세션 객체로 진행되던 트랜잭션은 멈췄음을 알 수 있다. `Resuming suspended transaction after completion of inner transaction` 로그를 통해 트랜잭션이 다시 시작됨을 알 수 있다. 테스트 수행 로그는 다음과 같다.
...
자식 서비스에서 조회하거나 데이터를 추가(insert)하는 쿼리가 실행되지 않는다. 아래는 테스트 수행 로그다.
...
중첩 트랜잭션이 지원 안 되는 관계로 부모, 자식 사이에 중첩된 트랜잭션으로 인해 발생하는 현상들은 이번 글에서 확인하지 못 했다. 부모는 `REQUIRED`, 자식은 `NESTED` 전파 타입으로 지정한 경우에 대해 테스트를 수행했다.

- `NestedTransactionNotSupportedException` 예외가 발생한다.
...
자식 서비스에서 조회하거나 데이터를 추가(insert)하는 쿼리가 실행되지 않는다. 테스트 수행 결과는 아래 로그를 통해 확인할 수 있다.
...
[transaction-acid-link]: https://junhyunny.github.io/information/database/acid/transaction/transcation-acid/ [unexpected-rollback-exception-link]: https://junhyunny.github.io/spring-boot/jpa/exception/unexpected-rollback-exception/