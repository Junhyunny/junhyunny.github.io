...
`A`라는 문자를 각 UTF 변환 규칙에 따라 바이트 시퀀스로 변경해 보자. 아래 숫자는 빅 엔디언(big endian) 기준의 바이트 시퀀스를 16진수로 표현한 것이다.

- UTF-8 방식: `41` (8비트 × 1)
- UTF-16 방식: `00 41` (16비트 × 1)
- UTF-32 방식: `00 00 00 41` (32비트 × 1)

같은 문자라도 UTF-8, UTF-16, UTF-32에서는 바이트 표현 방식이 다르다. 😀 이모지의 유니코드 번호는 `U+1F600`이다. 이를 각 UTF 변환 규칙에 따라 바이트 시퀀스로 변경하면 다음과 같다.
