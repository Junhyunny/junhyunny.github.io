...
선뜻 대답이 나오지 않는 어려운 질문이었다. 이 포스트에서 위 질문에 대한 답변을 정리해보겠다.

## 1. Wrapper 클래스

Java에는 기본형(primitive type) 데이터가 존재한다. 프로그램 언어를 처음 접할 때 주로 사용하는 자료형으로 int, char, byre, short, long, float, double, boolean 등이 이에 속한다. 이런 기본 타입의 값들은 JVM 내의 스택(stack) 메모리에 저장된다.

이런 기본형 데이터를 감싸서(wrapping) 만든 것이 바로 Wrapper 클래스이다. 각 기본형 데이터에 대응하는 Wrapper 클래스가 존재한다. 참조 타입인 wrapper 클래스 객체는 JVM 내의 힙(heap) 메모리에 저장된다.
...
- Boolean, Character 클래스를 제외하고 Number 클래스를 상속받는다.
...
- 타입 정보, 최대 값, 최소 값 등 부가적인 기능을 위한 값을 별도로 저장하고 있다.
...
Wrapper 클래스에 대해 이야기하면 빠지지 않는 내용이 있다. `Boxing / Unboxing`이다. `Boxing`은 기본형 데이터를 Wrapper 클래스의 인스턴스로 변환하는 과정을 의미한다. 반대로 `Unboxing`은 Wrapper 클래스의 인스턴스를 기본형 데이터로 변환하는 과정을 의미한다.
...
JDK 1.5 버전부터 `Boxing, Unboxing`이 필요하면 컴파일러에 의해 자동으로 수행되었다. 이를 `Auto Boxing, Auto Unboxing`이라고 한다.
...
Boxing, Unboxing 기능이 자동으로 수행되면서 컴파일 에러는 나지 않지만, 개발자의 의도치 않은 버그를 만들어내기도 한다. 아래 테스트 코드를 통해 예시를 들어보겠다.

### 3.1. 크기 비교 연산자 정상, 동일 판단 연산자는 비정상
- 크기 비교 연산자의 경우 auto unboxing 되면서 크기 비교가 정상적으로 수행된다.
- == 연산자를 이용하여 동일 여부 판단 시에는 정상적인 비교가 수행되지 않는다. == 연산자는 객체의 주소를 비교하기 때문이다.
- 객체에 담긴 값을 이용한 동일 판단은 equals 메서드를 이용해야 한다.
...
### 3.2. valueOf 메서드를 사용하여 만든 객체는 `==` 연산자가 정상적으로 동작하는가?
- Integer 생성자는 JDK 9 버전부터 사용을 권장하지 않는다.(@Deprecated(since="9"))
- Integer.valueOf 메서드를 사용하여 만든 객체를 `==` 연산자의 피연산자로 사용하는 경우 동일 여부 판단이 성공한다.
- 이는 valueOf 메서드 내부 캐시(cache) 처리에 의해 그렇게 보이는 것일 뿐 마찬가지로 equals 메서드를 사용하는 것이 좋다.
...
##### 결과
- valueOf 메서드를 통해 얻은 두 객체가 동일한 주소임을 알 수 있다.
...
#### 3.3. valueOf 메서드
- valueOf 메서드를 보면 특정 범위 내 값들에 한해서 캐시 처리를 수행한다.
- 필요한 경우 새로운 객체를 만들기 때문에 주소가 다른 객체가 생성될 수 있다.
...
- null 값을 가지는 wrapper 클래스 객체와 기본형(primitive type) 값을 `==` 연산의 피연산자로 사용하는 경우 NullPointException이 발생한다.
...
이유를 찾기 위해 관련된 내용들을 찾아보고 고민도 해보았다. 아래와 같은 몇 가지 이유로 추려지는 것 같다.
- List, Map 같은 Collection은 객체만 담을 수 있다.
- Java Generics 기능을 사용하려면 클래스를 명시해야 한다.
- 멀티 스레드 환경에서 동시성(concurrency)을 위한 코드 작성 시 동기화(synchronized) 기능은 객체만 지원된다.
