...
두서없이 JPA를 사용하다 지금에 와서 본질에 대해 공부하려다 보니 우선 JPA에 대한 정의를 해보려고 합니다.  
저는 아래 문장이 JPA를 가장 잘 설명한다고 생각되어 적어보았습니다. 
...
## 영속성(Persistence)
어떤 데이터를 생성한 프로그램이 종료되더라도 해당 데이터는 사라지지 않는 데이터 특성을 의미합니다. 
즉, 영속성을 지니지 못한 데이터는 메모리에만 존재하기 때문에 프로그램이 종료되면 그 즉시 소멸됩니다. 
...
Object-Relation Mapping 기법은 어플리케이션 설계와 데이터베이스 설계가 따로 이루어지면서 발생하는 객체 모델과 관계형 모델간의 불일치를 해소하기 위한 방법입니다. 
개발자가 너무 많은 SQL Mapping 작업에서 벗어나서 보다 더 객체 지향적인 프로그래밍(OOP, Object Oriented Programming)을 하기 위한 기법입니다. 
...
### OObject-Relation MappingRM 단점
Object-Relation Mapping을 사용하는 경우 다음과 같은 단점이 있습니다.
- 모든 비즈니스 케이스를 ORM 만으로 완벽하게 구현하기 힘듭니다.
...
- JPA(Java Persistence API)
  - Object-Relation Mapping을 쉽게하기 위해 Java 에서 사용하는 인터페이스 모음입니다.
