...
- 타이핑 이벤트 값을 보면 이전에 입력한 값들 뒤에 새로운 값이 추가되어 입려됨을 알 수 있습니다.
...
문제의 원인은 타이핑 이벤트에서 리-렌더링이 되지 않은 것인데 어떤 코드가 이런 현상을 일으키는지 디버깅해보았습니다. 
결과적으론 `waitFor` 함수의 콜백 함수 내에서 타이핑 이벤트를 수행하였기 때문에 문제가 발생했습니다.
...
디버깅을 하면서 몇 가지 새로운 개념에 대해 배울 수 있었기에 유익한 삽질(?)이었다고 생각됩니다.
