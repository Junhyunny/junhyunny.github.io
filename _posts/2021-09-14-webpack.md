...
## 0. 들어가면서
Java 언어를 주력으로 사용하는 백 엔드 개발자이지만, 일을 하다보면 프론트 엔드에 대한 이해도도 높아야한다고 느낍니다. 
...
## 2. Webpack 사용
이번 포스트는 `Webpack`에 대해 정리하는 글이기 때문에 매우 불편한 방법으로 `Webpack`을 사용하였습니다. 
다음 포스트에서 설정 파일을 이용하여 더 쉽게 `Webpack`을 사용하는 방법을 정리하겠습니다. 
