...
스프링 부트(spring boot)는 웹 서비스(WS, Web Service) 기술을 지원한다. [SOAP(Simple Object Access Protocol)][soap-link]에서 설명했지만, 여기서 말하는 웹 서비스는 통신 기술을 지칭한다. 월드 와이드 웹(WWW, World Wide Web)을 통해 사용자에게 서비스를 제공한다는 의미가 아니다. 일반적으로 웹 서비스는 다음과 같은 구조를 가진다.

1. 웹 서비스 제공자(provider)는 자신이 제공하는 API를 WSDL 문서로 만들어 UDDI 서버에 배포한다.
2. 웹 서비스 소비자(consumer)는 자신이 필요한 WSDL 문서를 UDDI 서버에서 탐색한다.
3. 소비자는 WSDL 문서를 파싱하여 요청에 필요한 정보를 생성한다.
4. 소비자는 생성한 정보를 기준으로 `SOAP` 메시지 형식에 맞는 요청 정보를 만든다.
5. 소비자는 HTTP를 통해 `SOAP` 메시지를 제공자에게 전달한다.
6. 제공자는 `SOAP` 메시지 요청을 받고 필요한 처리를 수행한다.
...
웹 서비스의 메시지 프로토콜인 `SOAP`을 사용해 두 개의 서비스가 통신하는 예제를 살펴본다. 다음과 같은 백그라운(background)로 인해 실습은 위에서 설명한 웹 서비스와 다른 흐름을 가진다.

- IBM, Microsoft, SAP에서 제공하는 공용 UDDI 서비스가 2007년에 종료되었다.
  - 아파치(apache)에서 UDDI 서버를 구축을 위한 오픈 소스 [JUDDI][juddi-link]를 제공한다.
- 스프링(spring) 프레임워크는 UDDI 서버 없이 웹 서비스를 제공한다.
  - 서비스 제공자가 직접 WSDL 파일을 호스팅한다.
- 웹 서비스 개발 시 `Contract Last` 방식과 `Contract First` 방식이 있다.
  - `Contract Last` 방식은 Java 코드를 먼저 작성하고 WSDL 파일을 만드는 방식이다.
  - `Contract First` 방식은 WSDL 파일을 통한 계약 성립 후 Java 코드를 통해 구현한다.
  - 스프링 프레임워크는 `Contract First` 방식만 지원한다.

### 1.1. Development Process

스프링 프레임워크를 사용한 웹 서비스의 제공자(provider)와 소비자(consumer)는 다음과 같은 개발 흐름을 가진다.

1. 서비스 제공자의 개발이 먼저 완료된다.
2. 제공자는 자신의 API를 정의한 WSDL 파일을 직접 호스팅한다.
3. 소비자는 제공자의 WSDL 파일을 가져온다.
4. 소비자는 제공자의 WSDL 파일을 기준으로 클래스(class) 파일을 생성한다.
  - 서비스 제공자가 WSDL 파일을 변경하는 것에 대해 소비자가 크게 영향을 받는다.
  - 소비자는 서비스 제공자와 강하게 결합되어 있다.
...
스프링 부트 프레임워크는 `spring-boot-starter-web-services` 의존성을 통해 쉽게 웹 서비스 애플리케이션을 구축할 수 있다.

- 내장 톰캣(embedded tomcat)이 서블릿 컨테이너를 사용한다.
- 스프링의 웹 서비스 코어 모듈이 사용한다.
  - 기본적인 스프링의 기능에 웹 서비스를 위한 기능들이 추가된다.
- WSDL 문서를 생성하기 위한 모듈을 추가한다.
  - 해당 의존성은 서비스 제공자만 필요하다.
- XML 문서를 Java 클래스나 객체(instance)로 변환하는 JAXB 모듈이 추가한다.
  - XML 문서를 JAVA 클래스나 객체로 변환하는 작업을 OXM(Object XML Mapping)이라고 한다.
  - 객체와 `SOAP` 메시지 사이의 변환을 처리한다.
...
1. 클라이언트는 서비스 소비자에게 책 정보 또는 저자(author) 정보를 요청한다.
  - REST 통신 구간
2. 서비스 소비자는 해당 요청을 서비스 제공자에게 재요청한다.
  - SOAP 통신 구간
3. 제공자는 요청 받은 정보를 소비자에게 응답한다.
  - SOAP 통신 구간
4. 소비자는 제공자로부터 받은 정보를 JSON 형식으로 클라이언트에게 응답한다.
  - REST 통신 구간
...
서비스 제공자부터 개발한다. 다음과 같은 프로젝트 구조를 가진다.
...
다음과 같은 의존성을 가진다. 주석으로 필요한 설명을 대체하였다.
...
XSD(XML Schema Definition)은 서비스 제공자가 노출하는 API 스키마 정보를 담은 XML 파일이다. `book.xsd` 파일도 있지만, 크게 다르지 않으므로 생략한다. 주석으로 필요한 설명을 대체하였다.
...
서비스 제공자는 XSD 파일을 기준으로 Java 클래스를 생성한다. 아래 명령어를 통해 생성한다.
...
서비스 요청을 받을 수 있는 엔드-포인트(end-point)를 생성한다. BookEndPoint 클래스도 존재하지만, 크게 다르지 않으므로 생략한다. `author.xsd`에 정보를 맞춰 코드를 작성한다.
...
API 스키마를 정의한 XSD을 기준으로 WSDL 파일을 생성하고 호스팅하기 위한 빈(bean)들을 정의한다. 서비스 소비자는 DefaultWsdl11Definition 빈 이름을 기준으로 WSDL 파일을 찾을 수 있다.
...
서비스 소비자를 개발한다. 제공자에서 제공하는 WSDL 문서를 기반으로 클래스를 생성해야하므로 제공자 애플리케이션 제작 이후 진행한다. 다음과 같은 프로젝트 구조를 가진다.
...
서비스 제공자의 WSDL 문서를 기반으로 클래스를 생성한다. 해당 태스크를 실행하기 전에 서비스 제공자 애플리케이션이 실행 중인지 확인한다.
...
`SOAP` 메시지와 Java 객체 사이의 변환을 도와주는 마샴러(marshaller)를 정의한다.

- `SOAP` 메시지에 대한 마샴(marshall)과 언마샴(unmarshall) 작업을 수행한다.
...
WebServiceGatewaySupport 클래스를 확장(extends)하여 `SOAP` 통신을 구현한다.

- 부모 클래스에서 제공하는 WebServiceTemplate 객체를 사용한다.
- 요청, 응답 객체는 WSDL 문서를 기반으로 생성한 클래스를 사용한다.
- 서비스 제공자에게 요청을 보낸다.
  - http://provider-service:8080/ws
- 소비자는 정보의 위치를 제공자가 정의한 WSDL 문서의 네임스페이스와 메시지를 기준으로 정의한다.
...
클라이언트로부터 REST 요청을 받을 수 있는 컨트롤러 객체이다.
...
도커 컴포즈(docker compose)를 통해 두 애플리케이션을 동시에 실행한다. 
