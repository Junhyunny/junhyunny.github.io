...
리액트(react) 프로젝트에서 한글을 입력할 때 마지막 문자가 중복으로 입력되는 현상이 발생하였습니다. 
한글 입력 시 발생하는 문제점과 원인, 해결 방법에 대해 정리하였습니다. 

## 1. 문제 현상

텍스트 박스(text box)에 한글을 입력하고 엔터(enter)를 눌러 입력한 값을 저장할 때 맨 마지막 문자가 한번 더 입력됩니다. 

### 1.1. 문제 코드

아래 코드를 크롬 브라우저에서 실행하여 한글을 입력하면 문제가 발생합니다.
...
이 문제는 IME(Input Method Editor) 과정에서 `KeyDown` 이벤트가 발생하면 운영체제와 브라우저가 해당 이벤트를 중복 처리하기 때문에 발생합니다. 
...
운영체제(operating system)가 라틴 계열 키보드로 한글 같은 조합이 필요한 문자의 입력을 지원하기 위한 컴포넌트(component) 혹은 프로그램입니다. 
한글처럼 IME 기능이 필요한 언어를 입력할 때 정상적인 처리가 안 될 수 있습니다. 
...
해결 방법은 `WebAPI`에서 제공하는 `CompositionEvent`와 관련 있습니다. 
...
각 세션 동안 이벤트 체인의 각 단계 사이에 속성 값들은 지속되며 `data` 속성에 유지됩니다. 
...
`isComposing` 속성은 IME 기능을 위해 컴포지션 세션이 시작될 때 `true`, 세션이 종료될 때 `false` 상태가 됩니다. 
컴포지션 세션 상태를 확인해 키보드 이벤트를 제어하면 한글 입력 문제를 방지할 수 있습니다. 

### 3.3. 문제 해결 코드

* 리액트 라이브러리 `KeyboardEvent` 이벤트 `nativeEvent` 객체의 `isComposing` 속성을 사용합니다.
...
`onKeyDown` 이벤트를 `onKeyPress` 이벤트로 변경하면 해당 문제가 해결되지만, 다음과 같은 문제점이 존재합니다.
