...
category:
  - information
  - java
last_modified_at: 2021-04-06T00:00:00
---
...
사전적 의미를 찾아보면 변경될 수 있다는 의미를 가졌습니다. 
이는 생성 이후에도 변수가 참조하고 있는 객체의 속성이 변할 수 있음을 의미합니다. 
아래 테스트 코드에 대한 설명을 보면 확인해보면 조금 더 이해가 쉽습니다. 
...
사전적 의미를 찾아보면 변경될 수 없다는 의미를 가졌습니다. 
MUTABLE 객체와 반대로 생성 이후에도 변수가 참조하고 있는 객체의 속성이 변할 수 없음을 의미합니다. 
대표적인 IMMUTABLE 객체인 String 클래스를 이용하여 테스트를 진행해보겠습니다.  
...
- 멀티스레드 환경에서 동기화 처리없이 객체를 공유할 수 있습니다.(Thread-safe) 
- 불변이기 때문에 객체가 안전합니다.

##### 단점
- 객체가 가지는 값마다 새로운 객체가 필요합니다. 메모리 누수의 위험이 존재합니다.
...
## OPINION
이직을 마음 먹고 기술 면접을 딱 한번 봤는데 정말 준비가 안되어 있으면 알고 있는 내용에 대해 제대로 설명하지 못한다는걸 깨달았습니다. 
그 중 하나가 MUTABLE, IMMUTABLE 객체에 대한 질문이었습니다. 
질문 세례를 받다보니 머리가 **'띵~'**한 상태에서 아는 내용을 질문 받아도 정작 핵심 키워드들이 떠오르지 않았습니다. 
질문받은 내용들 중에서 그나마 잘 대답한건 블로그에 정리해놓고 글을 다듬을 겸 항상 읽어보던 몇 주제들 뿐이었습니다. 
면접이 끝나고 나서 다시 한번 기록하는 습관의 중요함을 깨달았습니다.
