---
title: "Statement와 PreparedStatement 비교"
search: false
category:
  - spring-boot
  - java
...
[개발자가 반드시 알아야 할 자바 성능 튜닝 이야기][java-performance-tuning-book-link] 책의 내용 중 Java 애플리케이션에서 데이터베이스를 사용할 때 조금이나마 성능을 올릴 수 있는 방법에 대한 설명을 읽었다. 읽다보니 예전에 성능 분석 팀에서 SQL을 인라인(inline) 방식으로 작성하지 말라는 가이드를 받았던 기억이 떠올랐다. 호기심에 이유를 여쭤봤었는데 당시엔 정확하게 이해하지 못했었다. 글을 읽고 떠오른 기억을 계기로 삼아 관련된 내용을 정리해보았다.

## 1. Java 애플리케이션 SQL 실행 과정

책에 나온 설명이나 여러 레퍼런스들을 보면 Java 애플리케이션에서 SQL 문을 다음과 같은 과정을 통해 실행한다.

1. 쿼리 문장 분석
1. 컴파일
1. 실행

두 인터페이스의 구현체들은 이 과정에서 동작의 차이가 있다. 데이터베이스 종류 별로 `Statement`, `PreparedStatement` 인터페이스의 구현 클래스를 제공하기 때문에 동작은 각기 다르지만, 전반적인 내용은 같다.

- `Statement` 인터페이스 구현 클래스
  - 쿼리를 실행하는 `execute`, `executeQuery` 메서드 호출 시 3단계 과정을 모두 실행한다.
- `PreparedStatement` 인터페이스 구현 클래스
  - `PreparedStatement` 객체를 생성하는 시점에 쿼리 문장 분석과 컴파일을 처리하고 쿼리 혹은 커맨드 객체를 만든다.
  - SQL 문자열을 키, 쿼리 객체를 값으로 내부 메모리 캐시(cache)에 저장한다.
  - 쿼리를 실행하는 `execute`, `executeQuery` 메서드 호출 시 캐시에 저장된 쿼리 객체를 꺼내 사용한다.

### 1.1. H2 데이터베이스 SQL 쿼리 컴파일과 캐싱 과정

설명의 간편함을 위해 쿼리 문장 분석과 컴파일 과정을 합쳐 컴파일로 칭하고 설명을 이어나가겠다. `MySQL`, `PostgreSQL` 데이터베이스에서 제공하는 구현 클래스들의 코드들도 살펴봤지만, 전반적인 실행 흐름이 같았기 때문에 H2 데이터베이스의 구현 클래스들을 대표로 살펴보겠다.

H2 데이터베이스의 `JdbcPreparedStatement` 클래스 객체를 생성하면 아래 클래스들을 통해 컴파일과 캐싱이 수행된다.

#### 1.1.1. org.h2.jdbc.JdbcConnection 클래스

- `JdbcPreparedStatement` 객체를 만들기 전에 `translateSQL` 메서드를 통해 SQL 문장의 특수 문자들을 이스케이핑(escaping) 처리한다.
...
- prepareLocal 메서드에서 캐싱을 위한 객체를 만든다.
  - 캐시로 사용되는 `SmallLRUCache` 클래스는 LinkedHashMap 클래스를 상속받았다.
- 전달받은 SQL 문자열을 캐시의 키로 사용한다.
  - 문자열을 키로 사용하기 때문에 SQL 문을 인라인 방식으로 만드는 경우 값이 바뀔 때마다 키 값이 매번 바뀌므로 캐싱의 이점을 살리지 못한다.
- `Parser` 객체를 통해 쿼리를 수행할 수 있는 `Command` 객체를 생성한다.
- `Command` 객체를 캐시에 값으로 사용한다.
...
컴파일된 쿼리를 캐싱하여 재사용하는 `PreparedStatement` 구현체가 성능이 좋다는 의견이 많아 테스트를 작성하고 속도를 측정해보았다. 대상 데이터베이스는 다음과 같다.

- H2
- MySQL
- PostgreSQL

PC 사양에 따라 다르겠지만, 결과를 우선 살펴보면 3개의 데이터베이스 모두 `Statement`와 `PreparedStatement` 구현체의 쿼리 실행 속도가 확연하게 차이나진 않았다. 찾아보니 데이터베이스 별로 `Statement`와 `PreparedStatement` 구현체 사이의 성능 차이가 다르며 오라클(Oracle) 데이터베이스를 사용할 때 가장 효과적이라는 의견이 있었다.

### 2.1. H2 데이터베이스 테스트

- 테스트를 위한 10000건의 데이터를 추가한다.
- `name` 컬럼을 이용한 조회 쿼리를 각자 10000회씩 수행한다.
- 조회 쿼리 시작과 종료 시점에 시간 차이를 확인한다.
...
- 데이터베이스는 도커 컨테이너를 사용하였다.
- 프로젝트 경로에 `mysql-server.sh` 파일을 실행하면 MySQL 컨테이너가 실행된다.
- 테스트를 위한 10000건의 데이터를 추가한다.
- `name` 컬럼을 이용한 조회 쿼리를 각자 10000회씩 수행한다.
- 조회 쿼리 시작과 종료 시점에 시간 차이를 확인한다.
...
- 데이터베이스는 도커 컨테이너를 사용하였다.
- 프로젝트 경로에 `postgre-server.sh` 파일을 실행하면 PostgreSQL 컨테이너가 실행된다.
- 테스트를 위한 10000건의 데이터를 추가한다.
- `name` 컬럼을 이용한 조회 쿼리를 각자 10000회씩 수행한다.
- 조회 쿼리 시작과 종료 시점에 시간 차이를 확인한다.
...
SQL 삽입(SQL injection)은 외부 사용자가 전달한 검증되지 않은 값을 시스템이 쿼리에 그대로 삽입하여 사용함으로써 문제가 발생되는 공격이다. 두 인터페이스는 SQL 삽입 공격 취약성 여부가 다르다.

- `Statement` 인터페이스는 SQL 삽입 공격에 취약하다.
- `PreparedStatement` 인터페이스는 SQL 삽입 공격에 대한 방어가 가능하다.

해당 테스트는 조회 성능 테스트와 마찬가지로 3개의 데이터베이스들에 대해 진행하였고 결과는 다음과 같다.
...
테스트 코드가 크게 다르지 않으므로 H2 데이터베이스에 대한 테스트만 확인하겠다.

- 외부 사용자가 전달한 검증되지 않은 파라미터는 다음과 같다.
  - `''); delete from country; --`
  - 파라미터가 삽입되는 위치 앞에 쿼리는 종료시킨다.
  - `country` 테이블의 데이터를 모두 삭제한다.
  - 뒤에 오는 쿼리는 `--` 키워드를 통해 주석 처리한다.
- 테스트를 위한 10000건의 데이터를 추가한다.
- 쿼리를 실행하기 전 `country` 테이블에 저장된 데이터 카운트를 확인한다.
- `insert` 쿼리를 통해 데이터를 1건 추가한다.
- 쿼리가 종료된 후 `country` 테이블에 저장된 데이터 카운트를 다시 확인한다.
- `Statement` 구현체는 쿼리 실행 전에 10000건, 실행 후 0건일 것으로 예상한다.
  - SQL 삽입 공격에 의해 테이블에 모든 데이터가 삭제된다.
- `PreparedStatement` 구현체는 쿼리 실행 전에 10000건, 실행 후 10001건일 것으로 예상한다.
  - SQL 삽입 공격이 실패하므로 데이터가 1건 추가된다.
...
포스트 내용을 요약하면서 마무리하겠다.

- `Statement`, `PreparedStatement` 인터페이스의 구현체는 데이터베이스 별로 다른 방식으로 제공한다.
- SQL은 구문 분석, 컴파일, 실행 과정을 통해 실행된다.
- `Statement` 구현체는 모든 과정을 매번 실행한다.
- `PreparedStatement` 구현체는 구문 분석과 컴파일을 마친 쿼리를 캐시에 담아 반복 실행되는 경우 재사용한다.
- `PreparedStatement` 구현체는 캐시를 사용하기 때문에 성능이 좋아질 것을 예상하지만, 데이터베이스 별로 효과가 다르다.
- SQL 삽입 공격 관점에서 `Statement` 구현체는 취약하고, `PreparedStatement` 구현체는 방어가 가능하다.

확인해보진 않았지만, `MyBatis`나 `JPA` 같은 경우에도 내부적으로 데이터베이스 별로 제공하는 커넥션(connection) 클래스, `Statement`, `PreparedStatement` 인터페이스의 구현 클래스들을 사용할 것으로 예상되기 때문에 관련된 개념을 정리해두는 것은 도움이 될 것 같다. 관련된 내용을 확인하게 된다면 추후 포스트로 정리할 생각이다.
