---
title: "스프링에서 이미지를 PostgreSQL에 저장하는 방법"
search: false
category:
  - kotlin
  - spring-boot
...
일반적으로 이미지는 파일 시스템(file system)이나 AWS S3 같은 외부 저장소를 사용해 저장한다.

- 비용 측면에서 데이터베이스보다 파일 시스템이나 AWS S3가 저렴하다.
- 관계형 데이터베이스는 일반적으로 정형화된 텍스트 기반 데이터 처리에 최적화되어 있다.
- AWS S3 같은 외부 저장소를 사용하면 이미지 요청에 대한 서버의 부하를 감소시킬 수 있다.

하지만 현재 개발 중인 웹 애플리케이션은 클라우드 환경이라 서버의 파일 시스템을 사용할 수 없고, 개발 시간 단축과 외부 의존성을 최대한 줄이기 위해 데이터베이스에 이미지를 저장하기로 결정했다. 개발, 운영 환경은 다음과 같다.
...
이번 포스트는 파일을 업로드 후 파일을 데이터베이스에 저장하는 과정에서 겪은 문제와 테스트 방법에 대해 정리하였다. 스프링 프레임워크에서 파일을 업로드 하는 방법이나 단위 테스트에 대한 내용은 [How to test file upload in Spring][multipart-file-upload-in-spring-link] 포스트를 참고하길 바란다.

## 2. Solve the probloms

개발 과정에서 겪은 문제들을 하나씩 확인해보면서 해결 방법들에 대해서 정리해나가겠다. 문제의 원인은 요약하자면 테스트 환경과 개발, 운영 환경의 데이터베이스가 서로 다르기 때문에 발생한 것이다. H2 데이터베이스를 PostgresSQL 모드로 사용했더라도 JDBC 드라이버가 서로 다르기 때문에 애플리케이션이 다르게 동작한 것으로 예상된다.

처음 작성한 엔티티 모습은 다음과 같다.
...
PostreSQL 모드의 H2 데이터베이스에서 테스트를 실행한다.

- 엔티티 객체를 하나 생성 후 저장한다.
- 엔티티 매니저를 정리 후 데이터베이스에 엔티티를 조회한다.
- 조회한 결과와 예상 값들을 비교한다.
...
`binary_data` 컬럼을 `blob` 타입으로 테이블을 생성하는데 실패한다.
...
@Column 애너테이션의 columnDefinition 속성을 사용해 컬럼 타입을 지정한다.
...
개발 환경에 애플리케이션을 배포 후 해당 기능을 동작시키면 에러가 발생한다. 테스트 컨테이너(test container)를 적용해 개발 환경과 동일한 데이터베이스를 사용한 테스트 코드를 실행하면 동일한 에러 메시지를 확인할 수 있다.
...
로그를 살펴보면 `bytea` 타입 컬럼에 `bigint` 타입 형태의 데이터를 추가하려는 것으로 보인다.
...
@JdbcType 애너테이션을 사용해 타입에 하이버네이트(hibernate) 프레임워크에게 타입에 대한 힌트를 제공한다.
...
해결 방법을 찾는 과정에서 얻은 기타 정보들에 대해 함께 정리하였다.

### 3.1. @Type Annotation

스택 오버플로우(stack overflow)을 살펴보면 @Type 애너테이션에 타입을 명시하면 해결된다는 답변들이 많다. 이 답변은 하이버네이트 5 버전을 사용하는 경우 유효하지만, 하이버네이트 6 버전에선 사용할 수 없다. 스프링 부트 데이터 JPA 3.X 버전을 사용하는 경우 하이버네이트 6버전을 사용하므로 주의바란다.
...
PostgreSQL 데이터베이스를 사용하는 경우 엔티티에 다른 컬럼 정의나 타입 정의 없이 @Lob 애너테이션만 추가했다면 OID 타입으로 컬럼을 사용할 수 있다. 엔티티 필드와 테이블의 컬럼 타입을 OID로 정의하여 사용하는 경우 H2 데이터베이스를 PostgreSQL 모드로 사용하는 테스트에서 다음과 같은 예외가 발생한다. PostgreSQL 테스트 컨테이너를 사용하는 테스트나 개발, 운영 환경에선 정상적으로 동작한다.
