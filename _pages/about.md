...
* Google Analytics, Tag Manager 연결을 통한 사용 데이터 수집 기능 구현
...
* ACL(anti corruption layer) 구축으로 레거시 연계로 인한 도메인 오염 방지
    * Apache Nifi
...
* 실린더 번들 부적합 발생 시 이벤트 발행ㆍ구독을 통한 결합도 낮은 비즈니스 로직 구현
...
* 이송 물류 개선을 위한 차량 이송 물류 스케줄링 구현
    * 이송되어야 하는 물류들을 미리 그룹화 및 순번 지정
    * 타 운송 시스템으로 다음 이송 물류 전송
