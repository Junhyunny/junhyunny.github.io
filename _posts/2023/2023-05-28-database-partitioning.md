...
데이터베이스는 볼륨이 커질수록 읽기/쓰기 성능이 감소한다. 성능 향상을 위해 데이터를 분할하여 저장하는 파티셔닝(partitioning) 혹은 샤딩(sharding)이라는 기법을 사용한다. 이번 포스트는 파티셔닝 컨셉에 대해서 정리하였다.

## 1. Partitioning

> 파티션(partition)은 테이블을 논리적으로는 하나이지만, 물리적으로는 여러 개로 분리하여 관리하는 데이터베이스의 기능

데이터를 여러 개의 테이블로 분리하여 관리하는 파티션 기법을 적용하는 것을 파티셔닝(partitioning)이라고 한다. 대용량의 테이블을 물리적으로 여러 개의 소규모 테이블들로 분산하는 목적으로 주로 사용한다. 물리적인 데이터 분할이 있더라도 데이터베이스에 접근하는 애플리케이션 입장에선 이를 인지하지 못한다.

파티션을 사용하는 이유는 다음과 같다.

- 테이블이 너무 커서 인덱스의 크기가 물리적인 메모리보다 훨씬 큰 경우
  - 인덱스가 커지면 `INSERT`, `UPDATE`, `DELETE`, `SELECT` 작업도 함께 느려진다.
  - 인덱스 크기가 물리적으로 데이터베이스가 사용 가능한 메모리 공간보다 크면 영향을 더 크게 받는다.
...
- 데이터 특성상 주기적으로 삭제 작업이 필요한 경우
  - 로그 성격의 이력 데이터는 단기간에 대량으로 누적되고 일정 기간이 지나면 쓸모가 없어진다.
...
파티션 기법은 다음과 같은 장점들을 제공한다.

- 데이터베이스 확장성을 향상시킨다.
  - 새로운 데이터가 추가될 때마다 새로운 파티션을 생성하여 데이터를 분산시킬 수 있다.
- 데이터 관리를 용이하게 만들어준다.
  - 각 파티션 별로 독립적으로 관리될 수 있으며 백업, 복구, 인덱스 생성 등의 작업을 개별적으로 수행할 수 있다.
- 데이터 보안과 격리를 향상시킨다.
...
- 파티셔닝된 테이블의 특정 행의 데이터 변경은 해당 행이 위치한 파티션을 식별해야 한다.
  - 데이터 변경 관리의 복잡성이 증가된다.

## 2. Partitioning Method

파티셔닝 방법은 크게 두 가지로 구분된다.

### 2.1. Horizontal Partitioning

수평 파티셔닝(Horizontal Partitioning)을 먼저 살펴보겠다.

- 데이터를 행 단위로 분할하는 방법이다.
...
- 수직 파티셔닝(Vertical Partitioning)은 데이터를 열 단위로 분할한다.
- 테이블의 열들이 논리적 혹은 물리적으로 분할되어 각 파티션에는 특정 열 또는 열 그룹으로 포함된다.
...
`MySQL`을 기준으로 4가지 기본 파티션 기법에 대해 정리하였다.
...
키 파티션은 해시 파티션과 사용법과 특성이 같다. 해시 파티션은 해시 값을 계산하여 사용하지만, 키 파티션은 사용자가 명시한다. 최종적으로 `MySQL` 서버가 그 값을 MOD 연산을 수행하여 파티션을 결정한다.

해시 파티션과 다음과 같은 다른 점들이 있다.

- `MySQL` 서버가 내부적으로 MD5() 함수를 이용해 파티션하기 때문에 파티션 키가 반드시 정수 타입이 아니어도 된다.
- 해시 파티션에 비해 파티션 간의 레코드를 더 균등하게 분할할 수 있으므로 더 효율적이다.
