...
글 제목에 `전략`이라는 단어가 거창해보이지만, 고민에 대해 끄적거린 글이니 가볍게 읽어주시면 감사하겠습니다. 
...
- 미들웨어에서 비동기 처리 후 시나리오에 맞는 액션을 디스패치했는가?
    - API 요청 같은 비동기 처리 후 시나리오에 맞는 액션을 디스패치했다면 미들웨어의 역할은 종료됩니다. 
...
- setup 단계
    - `authentication` 모듈의 `authenticate` 액션이 수행되었는지 스파이(spy) 객체를 만듭니다. 
    - 이때 반환 값은 프로미스 객체를 반환하는 함수이으로 `mockReturnValue`를 통해 스터빙(stubing) 처리합니다.
    - 화면을 렌더링하고, 필요한 데이터들을 입력합니다. 
- act 단계
    - 사용자 클릭 이벤트를 발생시킵니다.
- assert 단계
    - `spyAuthenticationModule` 스파이를 이용해 원하는 액션이 적절한 파라미터로 수행되었는지 확인합니다.
...
화면 혹은 컴포넌트 테스트에서 원하는 액션 수행을 확인함으로써 사용자 인터랙션에 의한 액션이 미들웨어까지 전달되었다고 가정합니다. 
이번엔 미들웨어에서 원하는 액션을 수행했는지 확인합니다. 

### 3.1. 테스트 코드
- API 요청에 대한 성공, 실패로 인한 반환 값에 따라 개발자가 의도한 액션을 디스패치했는지 확인합니다.
- setup 단계
...
- assert 단계
    - `dispatch` 스파이가 적절한 액션을 디스패치했는지 확인합니다. 
...
원래 리듀서에 대한 테스트는 고민해보지 않았습니다. 
`'액션 타입에 따른 스테이트(state) 변경만 있는데 테스트할 가치가 있을까?'`라고 생각했었는데, 가이드를 보니 테스트의 필요성을 느꼈습니다. 

##### 가이드 라인 예시 구현 코드
- `todoAdded` 리듀서를 보면 이전 스테이트와 이번 액션에 따라 간단한 연산이 들어가 있습니다. 
- 개발자 실수로 인해 중첩된 `id`나 이빨 빠진 `id` 값을 가진 스테이트 리스트가 생성된다면 컴포넌트에서 의도치 않은 에러가 발생할 것 같습니다. 
- 이런 개발 실수를 막기 위해 초기 스테이트 값 확인과 적절한 스테이트 변경이 이루어졌는지 확인하는 작업은 필요해보입니다. 
...
예전에 참석했던 기술 컨퍼런스에서 테스트 커버리지 100% 달성률에 대한 이야기를 들었었는데, 당시에는 굉장히 멋진 일이라고 생각했었습니다. 

> 테스트 커버리지 100%이면, 에러가 있을수가 없겠는데?

최근들어 생각이 바뀌게 되었는데, 어플리케이션의 곳곳을 홅는 테스트 코드가 무슨 의미가 있을까에 대한 의문이 들었습니다. 
