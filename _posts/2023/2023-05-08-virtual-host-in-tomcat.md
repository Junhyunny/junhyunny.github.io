...
최근 톰캣(tomcat)의 `sever.xml` 파일 안에 `Host` 태그에 관련된 질문을 받았다. 한번도 사용해본 적 없는 기능이라 공부해보고 알려준다는 답변만 남겼다. 이후 배운 내용을 공유하기 위해 포스트 글로 정리하였다.

## 1. Virtual Host

`Host` 태그는 톰캣의 가상 호스트(virtual host)라는 기능에 사용된다. 물리 서버가 1대인 온-프레미스(on-premises) 운영 환경에서 여러 개의 도메인(domain)을 운영하는 경우 유용하게 사용된다. 사용 트래픽이 적은 경우처럼 물리 서버를 굳이 하나 더 늘릴 필요가 없다면 가상 호스트 기능을 통해 여러 서비스를 호스팅할 수 있다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/virtual-host-in-tomcat-01.png" width="100%" class="image__border">
</div>

### 1.1. server.xml in Tomcat

톰캣 폴더에 `conf/server.xml` 파일을 열면 다음과 같은 정보를 볼 수 있다. 이번 포스트에서 사용한 톰캣은 9 버전이다.

- 호스트 이름이 `localhost`인 경우 `webapps` 경로를 루트(root) 폴더로 설정한다.
  - `Context` 정보를 별도로 명시하지 않았으므로 `webapps/ROOT` 경로를 웹 애플리케이션의 컨텍스트 폴더를 설정한다.
...
다음과 같은 시나리오로 실습을 진행하였다.

- 스프링 부트(spring-boot) 프로젝트를 두 개 준비한다.
- 각 프로젝트를 `war` 패키지 파일로 빌드한다.
- 톰캣에 배포하고 가상 호스트를 통해 다른 도메인 주소로 서비스한다.
  - /pc 폴더 - `pc-service.com` 서비스 호스팅
  - /mobile 폴더 - `mobile-service.com` 서비스 호스팅

### 2.1. Build Projects

HTML 페이지의 내용만 다르고 같은 구조의 프로젝트를 배포하기 때문에 하나만 확인하겠다. 프로젝트 코드는 별다른 내용이 없으므로 자세한 설명 없이 글을 이어가겠다. 프로젝트의 모든 내용을 확인하고 싶은 분들은 포스트 맨 하단의 예제 코드 링크를 참고 바란다.

다음과 같은 환경을 가진 프로젝트를 빌드하였다.
...
스프링 부트는 내장 톰캣(embedded tomcat)을 사용하기 때문에 기본 패키지 방식이 `jar`이다.

- 일반 톰캣 서버에 배포하기 위해 `war` 형식의 패키징이 필요하다.
- 패키지 환경을 구성하는 자세한 방법은 아래 포스트를 참고 바란다.
- [스프링 부트(Spring Boot) 프레임워크에서 WAR 패키지로 배포하기][deploy-spring-boot-project-as-war-link]

다음 명령어를 통해 빌드하고, 패키징 된 파일을 톰캣에 이동한다.

- 빌드가 주된 목적이므로 테스트는 스킵한다.
...
`conf/server.xml` 파일을 다음과 같이 수정한다.

- `pc-service.com` 도메인 주소
  - 루트 폴더를 `pc`로 설정한다.
  - 웹 애플리케이션 컨텍스트 폴더를 `pc/ROOT`으로 지정한다.
- `mobile-service.com` 도메인 주소
  - 루트 폴더를 `mobile`로 설정한다.
  - 웹 애플리케이션 컨텍스트 폴더를 `mobile/ROOT`으로 지정한다.
...
빌드된 패키지 파일들을 톰캣 경로로 이동시킨다.

- 기본 루트인 `webapps` 폴더와 동일한 디렉토리 레벨에 두 개의 폴더를 만든다.
  - `pc` 폴더
  - `mobile` 폴더
- 빌드된 각각의 패키지 파일들을 `ROOT.war` 이름으로 변경하여 이동시킨다.
...
톰캣 서버를 시작하고 실행 로그를 확인한다.

- 루트 경로 폴더 내부에 `war` 패키지 파일이 있는 경우 톰캣에 의해 자동으로 압축이 풀리면서 서비스가 배포된다.
- `tail` 명령어를 통해 톰캣 실행 로그를 확인한다.
...
<div align="center">
  <img src="{{ site.image_url_2023 }}/virtual-host-in-tomcat-02.gif" width="100%" class="image__border">
</div>

### 2.4. Add Hosts at Local Machine

가상 호스트가 정상적으로 기능하는지 확인하기 위해선 도메인 주소가 필요하다. 로컬 호스트에 필요한 호스트를 등록한 후 브라우저를 통해 접근한다.
...
- 브라우저를 통해 각 도메인 주소에 접근한다.
- 서로 다른 화면이 보이는 것을 확인할 수 있다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/virtual-host-in-tomcat-03.gif" width="100%" class="image__border">
</div>

## CLOSING

스프링 부트 프레임워크에서 사용하는 내장 톰캣은 가상 호스트 기능을 활용하기 어렵다고 한다. nginx 같은 리버스 프록시(reverse proxy)를 사용하면 동일한 구조를 쉽게 구성할 수 있다. 관련된 내용은 다음 포스트를 통해 알아보겠다.
