...
현재 `Java` 어플리케이션에서 시간을 다루는 방법을 정리하고 있으며 이번 포스트에선 `JDK 8` 버전 전에 사용하던 날짜, 시간과 관련 API 기능들과 몇 가지 문제점들에 대해 살펴보려고 합니다. 

## 1. java.util.Date

`JDK 1.0`부터 사용한 시간, 날짜 관련 클래스입니다. 
이전 버전과 호환성을 위해 계속 제공하는 것 같지만, 현재 대부분의 기능이 더 이상 사용되지 않습니다.(deprecated) 
대부분 `Calendar` 클래스의 기능으로 대체되었습니다.

* 내부적으로 UNIX 시간을 관리합니다.
    * GMT 1970년 1월 1일 0시 0분 0초를 기준으로 지나온 시간을 ms(미리세컨드)로 표현한 시간입니다.
...
* `Date` 객체는 동시성에 취약합니다.
    * 캡슐화 실패로 `Date` 객체 내부에서 관리하는 유닉스 시간을 외부에서 변경할 수 있습니다.
...
### 1.2. Year starts based on 1900

* 생성자에 년도 파라미터에 122 값을 전달하면 2022년이 반환됩니다.
* 내부 코드를 확인하지 않으면 년도를 1900에 더해서 사용한다는 사실을 모릅니다.
...
* `Calendar` 객체는 동시성에 취약합니다.
    * 캡슐화 실패로 `Calendar` 객체 내부에서 관리하는 유닉스 시간을 외부에서 변경할 수 있습니다.
* 객체의 멤버 변수로 사용하는 것은 위험합니다.
...
### 2.4. Do not throw exception setting timezone

* 타임존을 지정할 때 잘못된 값이 들어와도 
...
`JDK 8`이 릴리즈되기 전엔 날짜, 시간 관련된 API 기능을 대체할 수 있는 라이브러리를 사용했다고 합니다.
