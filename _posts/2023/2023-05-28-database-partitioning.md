...
category:
  - information
  - database
last_modified_at: 2026-09-17T14:32:30+09:00
---
...
- [데이터베이스 인덱스(index)와 자료구조][db-index-data-structure-link]

## 0. 들어가면서

데이터베이스는 볼륨이 커질수록 읽기/쓰기 성능이 감소한다. 성능 향상을 위해 데이터를 분할하여 저장하는 파티셔닝(partitioning) 혹은 샤딩(sharding)이라는 기법을 사용한다. 이번 글에서는 파티셔닝 개념을 정리하였다.

## 1. Partitioning

> 파티션(partition)은 테이블을 논리적으로는 하나이지만, 물리적으로는 여러 개로 분리하여 관리하는 데이터베이스의 기능

데이터를 여러 개의 테이블로 분리하여 관리하는 파티션 기법을 적용하는 것을 파티셔닝(partitioning)이라고 한다. 대용량 테이블을 물리적으로 여러 개의 소규모 테이블로 분산하는 목적으로 주로 사용한다. 물리적인 데이터 분할이 있더라도 데이터베이스에 접근하는 애플리케이션 입장에선 이를 인지하지 못한다.

파티션을 사용하는 이유를 살펴보자. 우선 테이블이 너무 커서 인덱스의 크기가 물리적인 메모리보다 훨씬 큰 경우 파티셔닝을 고려한다.

- 인덱스가 커지면 `INSERT`, `UPDATE`, `DELETE`, `SELECT` 작업도 함께 느려진다.
- 인덱스 크기가 데이터베이스가 사용할 수 있는 물리적인 메모리 공간보다 크면 영향을 더 크게 받는다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/database-partitioning-01.png" width="80%" class="image__border">
</div>
<center>https://gngsn.tistory.com/203</center>

<br/>

데이터 특성상 주기적으로 삭제 작업이 필요한 경우에도 파티셔닝이 필요하다. 단기간에 대량으로 누적되고 일정 기간이 지나면 쓸모가 없어지는 로그 성격의 이력 데이터를 예로 들 수 있다. 이런 타입의 데이터는 시간이 지나면 별도로 아카이빙(archiving)하거나 백업한 후 삭제한다.

불필요해진 데이터를 백업하거나 삭제하는 작업은 일반 테이블에서는 상당히 고부하 작업이므로 미리 파티셔닝을 통해 데이터를 작업 단위로 나눠두고 편리하게 작업한다.
...
파티션 기법은 다음과 같은 장점을 제공한다.

- 데이터베이스 확장성을 향상시킨다.
  - 새로운 데이터가 추가될 때마다 새로운 파티션을 생성하여 데이터를 분산시킬 수 있다.
- 데이터 관리를 용이하게 만들어준다.
  - 각 파티션별로 독립적으로 관리할 수 있으며 백업, 복구, 인덱스 생성 등의 작업을 개별적으로 수행할 수 있다.
- 데이터 보안과 격리를 향상시킨다.
...
- 데이터가 균등하게 파티셔닝되지 않는 경우 특정 파티션에 부하가 집중될 수 있다.
  - 데이터 분산 전략과 로드 밸런싱 메커니즘을 고려해야 한다.
- 파티셔닝된 테이블에서 특정 행의 데이터를 변경하려면 해당 행이 위치한 파티션을 식별해야 한다.
  - 데이터 변경 관리의 복잡성이 증가한다.

## 2. Partitioning Method

파티셔닝 방법은 크게 두 가지로 구분된다. 먼저 수평 파티셔닝(Horizontal Partitioning)을 살펴보자. 데이터를 행 단위로 분할하는 방법이다. 일반적으로 분산 저장 기술에서 파티셔닝은 수평 분할을 의미한다.

- 데이터의 특정 열 값을 기준으로 데이터를 분할한다.
- 각 파티션에는 특정 범위 또는 조건에 해당하는 행들이 포함된다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/database-partitioning-03.png" class="image__border">
</div>

<br/>

다음으로 수직 파티셔닝(Vertical Partitioning)을 알아보자. 데이터를 열 단위로 분할한다.

- 테이블의 열이 논리적 혹은 물리적으로 분할되어 각 파티션에는 특정 열 또는 열 그룹이 포함된다.
- 주로 데이터베이스에서 자주 사용하지 않는 열이나 크기가 큰 열을 분리하여 저장 공간을 절약하고 액세스 성능을 개선하는 데 사용한다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/database-partitioning-04.png" class="image__border">
</div>

## 3. Partitioning Types

`MySQL`을 기준으로 보면 4가지 기본 파티션 기법이 존재한다.
...
레인지 파티션은 다음과 같은 용도에 사용하는 것이 적합하다.

- 날짜를 기준으로 데이터가 누적되고 연도나 월 또는 일 단위로 분석하고 삭제해야 하는 경우
...
리스트 파티션은 다음과 같은 용도에 사용하는 것이 적합하다.

- 파티션 키 값이 코드 값이나 카테고리와 같이 고정적인 경우
- 키 값이 연속되지 않고 정렬 순서와 관계없이 파티션을 해야 하는 경우
...
`MySQL`에서 정의한 해시 함수에 의해 레코드(record)가 저장될 때 파티션을 결정하는 방법이다. 파티션 표현식의 결과 값을 파티션 개수로 나눈 나머지로 저장될 파티션을 결정한다. 해시 파티션의 파티션 키는 항상 정수 타입이거나 정수 타입을 반환하는 표현식만 사용 가능하다.

해시 파티션은 다음과 같은 용도에 사용하는 것이 적합하다.

- 레인지 파티션이나 리스트 파티션으로 데이터를 균등하게 나누는 것이 어려운 경우
- 테이블의 모든 레코드가 비슷한 사용 빈도를 보이지만 테이블이 너무 커서 파티션을 적용해야 하는 경우
...
키 파티션은 사용법과 특성이 해시 파티션과 같다. 해시 파티션은 해시 값을 계산하여 사용하지만, 키 파티션은 사용자가 명시한다. 최종적으로 `MySQL` 서버가 그 값을 MOD 연산을 수행하여 파티션을 결정한다.

해시 파티션과는 다음과 같은 차이점이 있다.

- 키 파티션은 `MySQL` 서버가 내부적으로 MD5() 함수를 이용해 파티션하기 때문에 파티션 키가 반드시 정수 타입이 아니어도 된다.
- 해시 파티션에 비해 파티션 간 레코드를 더 균등하게 분할할 수 있어 효율적이다.
...
- [데이터베이스 샤딩(Database Sharding)][database-sharding-link]
