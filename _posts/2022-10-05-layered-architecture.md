...
* 사용자 인터페이스 계층(User Interface Layer)
    ...
    * 화면을 서비스하지 않는다면 클라이언트로부터 요청을 받고, 응답하는 API를 정의합니다.
...
* 도메인 계층(Domain Layer)
    ...
    * 세부적인 기술 스택 같은 외부 관심사에 의존하지 않고 격리된 채 순수한 비즈니스 로직을 수행합니다.
...
이와 달리 완화된 레이어 아키텍처는 이 제약을 완화시켜 자신의 하위 레이어는 모두 의존할 수 있습니다. 
...
이렇게 아무런 로직 없이 요청을 다음 계층으로 바이패스(bypass)하는 경우를 `싱크홀 안티 패턴`이라고 합니다. 
이는 불필요한 리소스(resource) 낭비를 만듭니다. 

모든 레이어 아키텍처는 몇 시나리오에 대해선 싱크홀 패턴에 빠지게 됩니다. 
여기서 `80-20 규칙(rule)`을 적용하는데, 전체 로직 중에 20%가 싱크홀 패턴라면 괜찮습니다. 
...
* 대상 계층의 컴포넌트를 제외하고 다른 계층의 의존성들은 테스트 더블(test double)로 만들어 테스트합니다.
* 배우기 쉽고 구현이 간단하여 거의 모든 어플리케이션의 시작점을 만들기에 적합합니다.
...
* 깔끔하고 얇은 뷰(clean and thin view)를 가져야 합니다.
    ...
    * 시스템의 상태를 변경시킬 수 있는 비즈니스 로직을 포함하면 안 됩니다.
* 도메인과 영속성 로직을 다른 계층으로 분리합니다.
    * 도메인과 영속성은 서로 다른 관심사를 가지므로 서로 다른 계층으로 분리합니다.
    * 이는 영속성 프레임워크가 변경될 때 핵심 비즈니스 로직에 주는 영향을 줄일 수 있습니다.
* 도메인 계층을 고립시켜야합니다. 
    * 도메인 계층은 비즈니스를 구성하는 핵심 개념과 중요한 정보, 준수해야하는 규칙들을 표현합니다.
    * 시스템을 단순하고 유연한 상태로 유지하기 위해서는 도메인 계층을 기술적인 이슈로부터 고립시켜야 합니다.
