...
* AOP
    ...
    * 스프링에서 관리하는 빈(bean)의 메서드 호출 전이나 후에 시점을 빼앗아 실행됩니다.
...
* 스프링 ``컨텍스트``에 빈으로 등록된 객체들의 특정 메서드 호출 전, 후 시점을 가로채어 공통적인 기능을 수행합니다.
...
* 스프링의 빈으로 등록된 컨트롤러와 서비스 객체의 특정 메서드 실행 전, 후에 AOP 로직을 수행합니다.
