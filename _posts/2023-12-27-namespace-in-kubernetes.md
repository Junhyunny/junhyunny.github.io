...
네임스페이스는 논리적인 구분이기 때문에 리소스를 완전히 격리하는 것은 아니다. 다른 네임스페이스에 위치한 오브젝트들과 통신할 수 있다. 통신하는 방법에 대해선 아래 실습을 통해 살펴볼 예정이다. 쿠버네티스 환경을 구축하면 기본적으로 4개의 네임스페이스가 만들어져 있다. 
...
시스템을 구성할 수 있는 상황에 따라 달라지겠지만, 리소스를 확실히 구분할 수 있도록 개발, 스테이지, 운영 같은 서비스 배포 환경 단위로 나눌 필요는 있어 보인다. 논리적인 수준의 격리를 통해 네임스페이스 구분할 수 있기 때문에 다음과 같은 장점을 얻을 수 있다.

- 네임스페이스 단위로 RBAC(Role-Based Access Control) 정책을 적용하여 보안 정책을 강화할 수 있다. 
- 네임스페이스마다 리소스를 다르게 지정하는 등 독립적인 정책 적용이 가능하다.
- 테스트 환경과 지속적인 개발이 이뤄지는 스테이징 환경을 구분할 수 있다.

## 2. Practice

간단한 장점들을 살펴봤으니 네임스페이스 컨셉을 확인하기 위해 가벼운 실습을 해본다. 다음과 같은 내용들을 확인한다.
...
도커 데스크탑(docker desktop)에서 지원하는 쿠버네티스 클러스터 환경에서 실습한다. 다음과 같이 간단한 환경을 구성한다. 

- 두 개의 네임스페이스 생성
  - foo-ns, bar-ns
  - foo-ns 네임스페이스는 리소스 쿼터를 통해 자원 제한이 걸려있다. 
- foo-ns 네임스페이스 파드와 서비스
  - foo-pod, foo-service
  - qux-pod, qux-service
- bar-ns 네임스페이스 파드와 서비스
  - foo-pod, foo-service

<p align="center">
  <img src="/images/namespace-in-kubernetes-02.png" width="80%" class="image__border">
</p>

### 2.1. Spring Application

파드를 배포하려면 컨테이너가 필요하다. 실습을 위한 스프링 애플리케이션 컨테이너 이미지를 하나 만든다. 전체 코드 중 중요한 부분만 살펴본다. 전체 코드를 보고 싶다면 아래 실습 코드 레포지토리(repository) 링크를 통해 확인하길 바란다. 

#### 2.1.1. application.yml

SERVICE_NAME 환경 변수를 통해 서비스 이름을 주입 받는다. 환경 변수는 파드가 정의된 yml 파일을 통해 주입된다. 기본 값은 `action-in-blog` 문자열이다.  
...
API 엔드-포인트(end-point)가 정의된 컨트롤러 클래스이다. 다음과 같은 요청을 처리한다.
...
스프링 부트 3.X 버전부터 지원하는 명시적인 HTTP 클라이언트 방식을 사용해 통신한다. 사용 환경을 구성법은 [Declarative HTTP Client in Spring Boot][declarative-http-client-in-spring-boot-link] 포스트를 참조하길 바란다. 명시적 HTTP 클라이언트 방식도 페인 클라이언트(feign client)처럼 동적 URL 할당이 가능하다. 동적 URL 할당에 대한 개념을 알고 싶다면 [Change URI with Openfeign when Runtime][dynamic-uri-using-openfeign-link] 포스트를 참고하길 바란다.

클리이언트의 getServiceName 메소드를 호출하면 파라미터로 전달 받은 URL 주소의 /service-name 경로로 GET 요청을 수행한다.
...
쿠버네티스 환경에서 사용할 수 있도록 도커 이미지를 생성한다. 다음과 같은 도커 파일을 사용한다.
...
다음 같은 yml 파일을 기준으로 쿠버네티스 네임스페이스를 생성한다.
...
네임스페이스 상세 내용을 각자 살펴본다. 먼저 foo-ns 네임스페이스 정보를 살펴본다. 지정된 리소스 쿼터를 확인할 수 있다.
...
다음으로 bar-ns 네임스페이스 정보를 살펴본다. 지정된 리소스 쿼터를 확인할 수 없다.
...
네임스페이스에 정의된 리소스 쿼터 정보를 확인하고 싶으면 다음 명령어를 사용한다.
...
foo-ns 네임스페이스에 배포할 파드 정보를 살펴본다. 
...
foo-ns 네임스페이스에 배포하는 파드는 실습을 위해 외부에서 호출해야하므로 NodePort 타입으로 지정한다. 해당 파드 스펙에 리소스 제약을 정의했다. foo-ns 네임스페이스가 리소스 제한이 설정되어 있기 때문에 배포될 파드들도 리소스 요청 정보가 필요하다. 리소스 요청 정보가 없는 경우 다음과 같은 에러 메세지를 볼 수 있다. 
...
foo-pod 파드와 크게 차이는 없다. 일부 차이점이 있다면 환경 변수 값을 `qux-pod-in-foo-ns`으로 설정한다. qux-pod 파드에 /service-name 경로로 GET 요청 시 해당 값을 응답 받는다. 서비스 타입은 외부와 통신할 필요가 없기 때문에 클러스터IP(ClusterIP) 타입을 사용한다. 정의 파일은 다음과 같다.
...
bar-ns 네임스페이스에 foo-pod 파드를 배포한다. 동일한 네임스페이스가 아니므로 동일한 이름을 가진 파드를 배포할 수 있다. bar-ns 네임스페이스는 리소스 제약이 없으므로 파드를 정의할 때 리소스 요청 정보를 작성할 필요가 없다. 환경 변수와 네임스페이스가 바뀐 것을 제외하면 크게 다르지 않다. 서비스도 외부와 직접 통신하지 않기 때문에 클러스터IP 타입을 사용한다.
...
foo-ns 네임스페이스에 배포된 foo-pod 파드를 통해 다른 서비스들과 통신해본다. 애플리케이션 코드를 보면 알 수 있듯이 `/external/{serviceName}` 경로로 GET 요청을 보내면 해당 서비스로 요청을 포워딩(forwarding)한다. 

cURL 명령어를 통해 foo-pod 파드를 통해 동일한 네임스페이스에 배포된 qux-pod 파드와 통신한다. 
...
다른 네임스페이스에 배포된 foo-pod 파드와 통신한다. 다른 네임스페이스에 위치한 서비스와 통신할 떄 주소는 조금 다르다. 먼저 결과를 확인하고 관련된 내용을 살펴보자.
