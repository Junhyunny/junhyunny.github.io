---
title: "도커 명명 볼륨(Docker Named Volume)"
search: false
category:
  - docker
...
[Docker Anonymous Volume][docker-anonymous-volume-link] 포스트에서 도커 볼륨의 전반적인 내용과 익명 볼륨(anonymous volume)에 대해 정리하였다. 이번 포스트에선 명명 볼륨(named volume)에 대해 알아보겠다. 글의 가독성을 위해 도커 볼륨에 관련된 내용을 이번 포스트에서도 빠르게 요약한 후 진행하겠다.

## 1. Docker Volume Quick Summary

### 1.1. Pain Point of Docker Container Layer

도커 컨테이너는 자신이 필요한 데이터, 파일들을 컨테이너 레이어(container layer)라는 영역에서 읽거나 저장한다. 컨테이너 레이어는 다음과 같은 한계점이 존재한다.

- 컨테이너 생성 시 초기화된 상태로 함께 새롭게 생성된다.
- 컨테이너 종료 시 함께 삭제된다.
...
- 이번 포스트에선 볼륨, 그 중에서도 명명 볼륨(named volume)에 대해서만 다룬다.
...
- `-v` 옵션으로 컨테이너의 `/app/volume` 경로를 명명 볼륨으로 연결한다.
  - 도커의 볼륨 이름은 `todos` 이다.
  - 콜론(:)을 기준으로 왼쪽이 볼륨 이름, 오른쪽이 컨테이너 내 경로이다.
- `-e` 옵션으로 파일 경로를 `/app/volume/todos`으로 지정한다.
- `--rm` 옵션을 사용하여 컨테이너의 실행을 멈추면 컨테이너를 함께 정리한다.
...
- `todos`라는 이름을 가진 볼륨이 생성되었음을 확인할 수 있다.
...
- 마운트 위치는 `/var/lib/docker/volumes/todos/_data` 경로로 지정된다.
...
[Docker Anonymous Volume][docker-anonymous-volume-link] 포스트에서 확인했듯이 볼륨을 사용하지 않으면 데이터가 저장되지 않는다. 동일한 테스트를 두 번 진행하는 것은 낭비이기 때문에 이번 포스트에선 컨테이너 사이에서 볼륨을 공유해보겠다. 다음과 같은 순서로 테스트를 진행한다.

1. 포트 8080 컨테이너를 실행한다.
1. 포트 8080 서비스에 접근한 후 화면에서 새로운 TODO 항목을 추가한다.
1. 생성된 명명 볼륨을 확인한다.
1. 포트 8081 컨테이너를 실행한다.
1. 포트 8081 서비스에 접근하면 포트 8080 서비스에서 저장한 데이터가 보이는지 확인한다.
...
`Docker CLI`를 사용하여 먼저 도커 볼륨을 생성하고 사용할 수 있다.
