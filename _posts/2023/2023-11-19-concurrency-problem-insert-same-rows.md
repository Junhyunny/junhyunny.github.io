...
1. 1번 트랜잭션 데이터를 조회하지만, 존재하지 않으므로 insert 쿼리를 수행한다. 하지만 아직 커밋(commit)을 수행하진 못했다.
...
DuplicatedCollectException 예외를 처리할 핸들러를 정의한다. 프론트엔드와 프로토콜을 정의한다. 이번 글에선 600 상태 코드와 예외 메시지를 전달한다.
