...
비관적인 락(pessimistic lock) 개념을 다시 정리한 후 글을 이어간다.

> 비관적인 락(pessimistic lock)<br/>
> 트랜잭션 충돌을 예상하고 미리 데이터에 대한 락을 선점하는 비관적인 락 방식이다. 비관적인 락은 특정 트랜잭션이 데이터에 대한 락을 선점하기 때문에 다른 트랜잭션들의 지연을 유발할 수 있다. 

## 1. How to use pessimistic lock in JPA?

### 1.1. LockModeType

잠금 모드를 지정하는 방식으로 비관적인 락 기능을 사용할 수 있다. 다음과 같은 모드들이 존재한다.

- `LockModeType.PESSIMISTIC_READ`
  - allows us to obtain a shared lock and prevent the data from being updated or deleted
  - `shared lock`이란 데이터 읽기 잠금이라고도 한다.
  - `shared lock`이 걸린 데이터는 다른 트랜잭션들에서 읽는 것이 가능하다. 
- `LockModeType.PESSIMISTIC_WRITE`
  - allows us to obtain an exclusive lock and prevent the data from being read, updated or deleted
  - `exclusive lock`이란 데이터를 변경할 때 사용한다.
  - `exclusive lock`이 걸린 데이터는 해제될 때까지 다른 트랜잭션(읽기 포함)들에서 접근할 수 없다.
- `LockModeType.PESSIMISTIC_FORCE_INCREMENT`
  - works like PESSIMISTIC_WRITE and it additionally increments a version attribute of a versioned entity
  - `exclusive lock`과 동일하며 추가적으로 버전 자동 증가 수행한다.

`PESSIMISTIC_READ`을 지원하지 않는 데이터베이스도 있지만, 그런 경우엔 `PESSIMISTIC_WRITE`으로 대체된다고 한다. 이번 포스트에선 `PESSIMISTIC_WRITE` 모드에 대해서만 다뤘다. 

### 1.2. @Lock Annotation

`spring-data-jpa` 라이브러리의 `JpaRepository` 인터페이스를 사용하는 경우 `@Lock` 애너테이션으로 잠금 모드를 설정한다. 
...
다음은 비관적인 락 기능 동작을 확인하기 위한 테스트 코드이다. 먼저 한 트랜잭션이 데이터 조회와 동시에 락을 선점한다. 다른 트랜잭션은 락이 풀려 데이터를 조회할 수 있기를 기다린다. 긴 시간 동안 데이터를 조회할 수 없다면 예외를 던진다. 

테스트 코드를 잘 이해하기 위해선 다음과 같은 내용을 미리 알면 좋다. 
...
먼저 `JpaRepository` 인터페이스를 사용한 테스트다. `AsyncTransaction` 빈을 사용해 테스트에 필요한 새로운 비동기 트랜잭션을 생성한다. `Propagation.REQUIRES_NEW` 속성을 지정하여 진행 중인 트랜잭션을 잠시 멈추고 새로운 트랜잭션을 만들어 낸다. 이를 통해 잠깐의 시간 차이가 발생하는 두 개의 트랜잭션을 실행한다.

- `트랜잭션1`는 다음과 같은 작업을 수행한다.
  - 제목(title)이 `Hello World`인 포스트(post) 엔티티를 찾는다.
  - 데이터 조회만으로 데이터 락을 점유한다.
  - 내용을 변경한다.
  - 7초 대기한다.
...
- `트랜잭션2` 처리 과정에서 예외가 발생하는 것을 예상한다.
  - 다른 트랜잭션에 락이 걸린 데이터를 조회하지 못하고 타임아웃(timeout) 예외가 발생한다.
  - `CompletionException` 예외가 발생한다.
  - `CompletionException` 예외의 원인은 `PessimisticLockingFailureException`이다.
  - `PessimisticLockingFailureException` 예외의 원인은 `PessimisticLockException`이다.
  - `PessimisticLockException` 예외의 원인은 `JdbcSQLTimeoutException`이다.
...
- 제목으로 조회하는 쿼리
  ...
  - 데이터 조회와 동시에 데이터에 락을 설정한다.
  - 늦게 시작한 `트랜잭션2`는 락이 풀려 조회가 가능해지길 기다린다.
- 타임아웃 에러
  ...
  - 데이터 조회를 위해 대기하는 중 시간이 초과되어 타임아웃 예외가 발생한다.
  - 해당 예외는 `트랜잭션2`에서 발생한 것으로 예상한다.
- 업데이트 쿼리 
  - `트랜잭션1`은 7초 대기 후에 업데이트를 수행한다.
  - `This is tx1 before sleep`, `This is tx1 after sleep` 로그 사이의 시간 차이는 약 7초이다.
- 제목으로 조회하는 쿼리
  - `where post0_.title=? for update`
  - 검증을 위한 조회 쿼리가 수행된다.
...
다음은 `EntityManager`를 사용한 테스트다. 위 테스트와 마찬가지로 각기 다른 트랜잭션을 만들어 실행하고 이를 커밋한다. 
...
- `트랜잭션2` 처리 과정에서 예외가 발생하는 것을 예상한다.
  - 다른 트랜잭션에 락이 걸린 데이터를 조회하지 못하고 타임아웃(timeout) 예외가 발생한다.
  - `CompletionException` 예외가 발생한다.
  - `CompletionException` 예외의 원인은 `PessimisticLockException`이다.
  - `PessimisticLockException` 예외의 원인은 `org.hibernate.PessimisticLockException`이다.
  - `org.hibernate.PessimisticLockException` 예외의 원인은 `JdbcSQLTimeoutException`이다.
- 포스트 엔티티는 커밋을 성공한 `트랜잭션1`의 업데이트 모습일 것으로 예상한다.
...
- 업데이트 쿼리 
  ...
  - `트랜잭션1`은 7초 대기 후에 업데이트를 수행한다.
  - `This is tx1 before sleep`, `This is tx1 after sleep` 로그의 시간 차이는 약 7초이다.
- 제목으로 조회하는 쿼리
