...
메시지 인증 코드(MAC, Message Authentication Code)는 메시지 인증과 위변조를 검출하기 위해 사용한다. 메시지 인증 코드를 통해 수신(receiving) 서비스는 다음과 같은 내용들을 확인할 수 있다.

- 비밀 키를 통해 암호화하므로 허가된 메시지인지 데이터 인증이 가능하다.
- 메시지를 사용해 인증 코드를 만들기 때문에 공격자에 의해 메시지가 위변조 되었는지 무결성 검증이 가능하다.

다음과 같은 과정을 통해 인증 과정이 이뤄진다.

1. 송신자는 메시지와 비밀 키를 함께 암호화 알고리즘에 통과시켜 MAC을 획득한다.
2. 송신자는 메시지와 MAC을 함께 송신한다.
3. 수신자는 전달 받은 메시지와 미리 공유하고 있던 비밀 키를 함께 암호화 알고리즘에 통과시켜 MAC을 획득한다.
  - 이때 사용한 비밀 키는 대칭 키(symmetric key)이다.
4. 수신자는 전달 받은 MAC과 새롭게 생성한 MAC을 비교한다.
  - 같은 MAC 값을 갖는 경우 정상적으로 처리한다.
  - 다른 MAC 값을 갖는 경우 메시지가 위변조 되었다고 판단한다.

<div align="center">
  <img src="{{ site.image_url_2023 }}/hmac-01.png" width="80%" class="image__border image__padding">
</div>
<center>https://en.wikipedia.org/wiki/Message_authentication_code</center>

## 2. HMAC(Hash-based Message Authentication Code)

`HMAC(Hash-based Message Authentication Code)`은 이름처럼 해시(hash)의 특징을 사용해 보안을 강화한 메시지 인증 코드이다. 다음과 같은 해시의 특징들을 사용한다.

- 단방향 암호화를 수행하므로 복호화가 불가능하다.
- 원본 데이터의 크기에 상관 없이 고정 길이의 데이터로 변환한다.
- 원본 데이터의 작은 변화에도 전혀 다른 해시 코드가 생성된다.
  - 작은 위변조에 전혀 다른 값이 출력되므로 데이터 무결성을 검증할 수 있다.

다음과 같은 방식을 통해 `HMAC`이 생성된다.

- H - 암호화를 위한 해시 함수
  - MD5, SHA-1, SHA-224, SHA-256, SHA-512 등이 사용된다.
- m - 인증이 필요한 메시지
- K - 비밀 키
  - 사전에 발급 받으며 대칭 키를 사용한다.
  - 유출 우려가 있을 경우 재발급해서 사용해야 한다.
...
<div align="center">
  <img src="{{ site.image_url_2023 }}/hmac-02.png" width="50%" class="image__border image__padding">
</div>
<center>https://en.wikipedia.org/wiki/HMAC</center>

## 3. Practice

`HMAC`은 데이터 무결성을 확인하기 위한 도구로 사용된다. 일반적으로 HTTPS 같은 보안 채널을 통해 메시지를 전송하고 혹시 모를 메시지의 위변조를 `HMAC`을 통해 검증한다. 다음과 같은 시나리오를 만족하는 `HMAC` 검증 필터를 만들어 위변조 된 메시지를 검증하는 기능을 구현해보았다.

1. 송신자는 비밀 키, 메시지, 타임스탬프(timestamp)를 사용해 `HMAC`을 생성한다.
2. 송신자는 API 요청 시 메시지, 타임스탬프 그리고 생성한 `HMAC`를 전달한다.
3. 수신자는 메시지, 타임스탬프, 미리 발급 받은 비밀 키를 통해 `HMAC`을 생성한다.
4. 수신자는 전달받은 `HMAC`과 생성한 `HMAC`을 비교한다.

### 3.1. TodoController Class

- 4개의 엔드포인트(endpoint)가 존재한다.
- GET, DELETE 요청은 HMAC 검증을 수행하지 않는다.
- POST, PUT 요청은 HMAC 검증을 수행한다.
  - POST 요청은 `application/json` 타입의 메시지 요청을 받는다.
  - PUT 요청은 `application/x-www-form-urlencoded` 타입의 메시지 요청을 받는다.
...
각 메서드 별로 어떤 기능을 수행하는지 살펴본다. 가독성을 위해 코드 설명은 주석으로 작성하였다.
...
`application/json` 타입 메시지는 입력 스트림으로 전달 받으므로 여러 번 읽을 수 없다. 필터에서 메시지를 읽는 경우 컨트롤러까지 메시지를 전달할 수 없다. 이 문제를 해결하기 위해 입력 스트림에서 읽은 바이트 배열은 임시 공간에 저장하여 재사용한다. 
...
송신자 역할은 테스트 코드로 대체한다. 필터 테스트를 위해 @WebMvcTest 애너테이션을 사용한다. 다음과 같은 케이스들을 테스트하였다.

- GET 요청은 정상적으로 응답을 받는다.
- DELETE 요청은 정상적으로 응답을 받는다.
- POST 요청은 다음과 같은 케이스를 확인한다.
  - 동일한 메시지로 요청하는 경우 정상적으로 응답을 받는다.
  - 변경된 메시지로 요청하는 경우 400(bad request) 응답을 받는다.
- PUT 요청은 다음과 같은 케이스를 확인한다.
  - 동일한 메시지로 요청하는 경우 정상적으로 응답을 받는다.
  - 변경된 메시지로 요청하는 경우 400(bad request) 응답을 받는다.
