...
스케일 업은 전체 트래픽이 꾸준히 많고, 지속적으로 CPU 사용률이 80% 이상을 넘는다면 효율적일 것이다. 스케일 아웃은 시간대 별로 혹은 특정 이벤트로 인해 급격하게 트래픽이 변동할 때 적합하다. 나는 대용량 파일 업로드를 위해 다음과 같은 시스템 디자인이 필요하다고 생각했다. 
