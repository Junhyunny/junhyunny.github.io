---
title: "Flyway CLI를 활용한 데이터베이스 마이그레이션"
search: false
category:
  - database
  - flyway
...
[Schema Migration][schema-migration-link]에서 이야기했듯이 스키마 마이그레이션 작업은 도구를 사용하는 것이 좋다. `Flyway`는 `Liquibase`와 같은 대표적인 오픈 소스 데이터베이스 마이그레이션 도구이다. 데이터베이스의 변경 사항을 추적하고 스키마 변경이나 롤백을 쉽게 도와준다. 깃(git)이 코드 형상 관리 도구라면 Flyway는 데이터베이스 변경에 대한 형상 관리를 지원한다.

다음과 같은 환경에서 사용할 수 있는 API를 제공한다.
...
CLI 환경에서 Flyway를 사용하는 방법에 대해 정리하였다.

### 1.1. Commands

주로 사용하는 명령어들은 다음과 같다.

- baseline
  - 현재 데이터베이스 스키마 상태를 시작점으로 새로운 버전 이력을 쌓는다.
  - 새로운 기준점 이전의 마이그레이션은 모두 무시한다.
  - 마이그레이션 이력 테이블을 생성한다.
...
- info
  - 현재까지의 마이그레이션 작업 이력에 대한 정보를 보여준다.
  - 로컬에 저장된 적용되지 않은 마이그레이션 스크립트 파일에 대한 정보를 보여준다.
...
- migrate
  - 가장 큰 버전을 가진 스크립트까지 스키마 마이그레이션 작업을 수행한다.
  - 변경 작업에 대한 내용을 마이그레이션 이력 테이블에 저장한다.
...
- repair
  - 실패한 마이그레이션 이력을 대기(pending) 상태로 변경한다.
  - 이전에 수행된 마이그레이션 스크립트가 없어진 경우 이력 테이블의 해당 마이그레이션 상태를 삭제(delete)로 변경한다.
  - 이전에 수행된 마이그레이션 스크립트가 변경된 경우 이력 테이블의 해당 체크섬(checksum)을 업데이트한다.
...
- validate
  - 마이그레이션 이력 테이블 상태와 로컬에 저장된 마이그레이션 스크립트 상태를 비교한다.
...
- clean
  - 마이그레이션 이력 테이블과 함께 데이터베이스의 모든 테이블을 드랍(drop)한다.
  - 운영 환경에서 사용되지 않도록 주의가 필요하다.
...
마이그레이션 스크립트 이름은 다음과 같은 규칙을 따라 작성한다.

- 스크립트 종류를 접두어(prefix)로 구분한다.
  - V - 버전 마이그레이션 파일
  - U - 롤백 마이그레이션 파일
  - R - 반복 수행 마이그레이션 파일
- 접두어 뒤에 버전 정보를 추가한다.
  - V, U 마이그레이션 파일에만 추가한다.
  - 1, 1.0, 1.0.1, 20230823(날짜) 등의 포맷을 자유롭게 사용 가능하지만, 반드시 순서를 지정할 수 있어야 한다.
- 구분자는 __(언더바 2개)를 사용한다.
  - 구분자는 접두어 혹은 버전과 설명(description) 사이를 구분한다.
  - _(언더바 1개)는 단어를 띄어쓰는 용도로 사용한다.
- 마이그레이션 스크립트가 어떤 동작을 수행하는지 간략한 설명(description)을 작성한다.
- 접미어는 .sql 확장자를 사용한다.
...
맥북(mac book)에서 실습을 진행하였다. 프로젝트 구조는 다음과 같다.
...
Flyway는 다음과 같은 경로에 작성된 설정 파일을 사용한다.
...
이번 실습은 프로젝트 경로의 `flyway.conf` 파일을 사용하였다. 설정 파일에 다음과 같은 정보를 작성한다.
...
빈 데이터베이스 상태에서 info 명령어를 수행한다.

- 설정 파일에서 지정한 migration, seed 경로에 저장된 스크립트만 적용 대상이다.
- 각 파일의 종류, 버전, 설명이 표시되며 두 파일 모두 현재 대기 상태이다.
...
- 설정 파일에서 지정한 데이터베이스에 마이그레이션을 수행한다.
...
1.0.3 버전 마이그레이션 스크립트를 정상적으로 수정하고 migrate 명령어를 수행하면 다음과 같은 에러가 발생한다. repair 명령어를 수행하고 마이그레이션을 재수행한다.

- 이전에 실패한 이력 때문에 마이그레이션을 진행하지 않는다.
- repair 명령어를 통해 실패한 이력을 대기 상태로 변경한다.
...
info 명령어를 통해 상태를 확인한다.

- 새로 추가된 1.0.2 버전의 스크립트 파일은 무시(ignored)된다.
- 마지막으로 버전보다 높은 버전의 스크립트만 적용된다.
...
1.0.2 버전 스크립트 파일을 1.0.4 버전으로 변경 후 작업을 수행한다. 마이그레이션을 진행하기 전에 이전에 사용한 마이그레이션 스크립트를 변경한다.
...
migrate 명령어 수행이 실패한다.

- Flyway는 이미 적용된 스크립트 파일을 관리한다.
...
프로젝트 상황에 맞게 해당 에러를 수정한다.

- 스크립트 파일을 이전 상태로 복구한다.
- repair 명령어를 통해 마이그레이션 이력 테이블의 체크섬을 최신화한다.
...
커뮤니티 버전은 롤백 기능이 제공되지 않는다. 커뮤니티 버전에서 문제가 발생했을 때 대처를 위한 롤백 전략이 필요해 보인다.
