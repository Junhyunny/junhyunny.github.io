...
`OAuth1.0`은 2009년 4월에 보안 결함에 관련된 내용이 발표되었으며, 현재는 `OAuth2.0` 프로토콜을 보통 사용합니다. 
...
이 과정은 클라이언트 어플리케이션이 리소스 오너의 권한을 위임 받을 수 있도록 미리 승인을 받는 것 입니다. 
...
* Homepage URL 
    ...
    * http://application-public-address
* Authorization callback URL
    * 클라이언트 어플리케이션이 인증 서버로부터 인증 코드를 받을 수 있는 URL
    * http://application-public-address/auth/callback
...
액세스 토큰으로 모든 리소스 서버의 기능을 사용할 수 없습니다. 
...
1. (B) Authorization Grant
    ...
    * 권한 부여 유형은 여러가지 존재하며 권한 부여 서버에서 지원하는 방식에 따라 달라집니다.
        * Authorization Code
        * Implicit
...
[redirect-and-forwar$ding-link]: https://junhyunny.github.io/information/redirect-and-forwarding/