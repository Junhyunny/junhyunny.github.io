...
- Job 인터페이스 - 실제 수행되는 execute 메서드를 명시합니다. 개발자는 해당 메서드를 구현합니다.
...
- `jobDataAsMap` - Job 역할을 수행항 클래스에게 전달할 파라미터를 정의합니다. `setter` 메서드를 통해 전달받습니다. 
...
- `executeInternal` 메서드 내부에 수행시킬 기능을 구현합니다.
- `setter` 메서드를 이용해 `blogService` 빈(bean) 객체를 주입받습니다. 
...
### 2.5. BlogDao 인터페이스
- selectTest 메서드 - TB_TEST 테이블 데이터를 조회합니다.
- updateTest 메서드 - TB_TEST 테이블 데이터를 업데이트합니다.
