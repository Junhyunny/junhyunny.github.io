...
* 블록킹 큐에서 작업을 꺼내어 작업자 스레드에게 전달합니다.
    * 블록킹 큐에서 작업을 꺼낼 때 타임아웃(timeout) 여부에 따라 poll 혹은 take 메서드가 호출됩니다.
...
* Executors 클래스
    * 정적 팩토리 메서드 패턴을 통해 스레드 풀 구현체 인스턴스를 생성합니다.
    * 이번 포스트에서 살펴볼 정적 팩토리 메서드들은 다음과 같습니다.
        * newCachedThreadPool
        * newFixedThreadPool
...
* ExecutorService 인터페이스
    ...
    * ExecutorService 인터페이스로 추상화 된 메서드들을 통해 스레드 풀 기능들을 활용할 수 있습니다. 
...
`newCachedThreadPool` 팩토리 메서드를 통해 생성합니다. 
...
* SynchronousQueue 인스턴스에 작업을 담습니다.
    * 타임아웃이 존재하므로 내부에서 poll 메서드가 수행됩니다.
...
`newFixedThreadPool` 팩토리 메서드를 통해 생성합니다. 
해당 스레드 풀은 다음과 같은 특징을 가집니다.
...
* LinkedBlockingQueue 인스턴스에 작업을 담습니다.
    * 타임아웃이 없으므로 내부에서 take 메서드를 통해 작업을 얻습니다. 
...
`newScheduledThreadPool` 팩토리 메서드를 통해 생성합니다. 
해당 스레드 풀은 다음과 같은 특징을 가집니다.
...
* DelayedWorkQueue 인스턴스에 작업을 담습니다.
    * 코어 풀 사이즈가 0인 경우 내부에서 poll 메서드를 통해 작업을 얻습니다.
    * 코어 풀 사이즈가 1 이상인 경우 내부에서 take 메서드를 통해 작업을 얻습니다.
...
* run_scheduled_task 메서드 수행 결과입니다.
* 테스트 로그
    * 10개의 작업을 5개의 스레드가 나눠서 수행하는 것을 확인할 수 있습니다.
...
* remove_thread_when_core_size_zero 메서드 수행 결과입니다.
* 테스트 로그
