---
title: "Google Tag Manager 커스텀 이벤트"
search: false
category:
  - information
  - data-science
...
구글 태그 매니저(GTM, google tag manager)는 애플리케이션에서 발생하는 사용자의 움직임을 태그로 정의하여 감지할 수 있다. 기본으로 제공하는 기능을 사용할 수도 있지만, 서비스 요건에 맞춘 이벤트를 정의할 수 있다. 이번 포스트는 구글 태그 매니저에서 사용자 정의 변수(variable)와 맞춤 이벤트(custom event)를 만들고 적용하는 방법에 대해 정리하였다.
...
- 변수 유형은 `데이터 영역 변수`를 사용한다.
- 데이터 영역 변수 이름은 `customData`이다.
  - 애플리케이션에서 `customData`라는 키(key)에 데이터를 매칭하여 전달한다.
- 데이터 영역 버전은 `버전 2`를 선택한다.
- 저장 버튼을 클릭한다.
...
- 사용자 정의 변수에 `Custom Data`라는 변수가 생성된다.
...
- 이벤트 이름을 `custom_event`으로 지정한다.
  - 애플리케이션에서 `custom_event`으로 이벤트를 발행하는 코드가 필요하다.
...
- 이벤트 이름을 `custom_click`으로 지정한다.
  - 지정한 이벤트 이름으로 구글 애널리틱스에서 확인 가능하다.
- 이벤트 매개변수를 추가한다.
- 키(key)는 `custom_data`로 정의한다.
...
- 이전 단계에서 생성한 `Custom Data` 변수를 선택한다.
...
- 이전 단계에서 생성한 `Custom Event` 트리거를 선택한다.
...
리액트 애플리케이션 코드를 살펴보겠다. 커스텀 이벤트는 일반 이벤트와 다르게 개발자와 협업이 더 많이 필요하다. 특정 이벤트에 대한 비즈니스적인 정의가 이뤄지면 이를 적절한 위치에서 호출해야 한다. 커스텀 이벤트를 애플리케이션에서 호출하는 방법에 대해 알아보겠다. 구글 태그 매니저를 설치하는 과정은 [Google Analytics with Google Tag Manager][google-analytics-with-google-tag-manager-link] 포스트를 참고하길 바란다.

### 5.1. global.d.ts

- 타입스크립트(typescript)를 사용하는 경우엔 전역 변수로 지정이 필요하다.
- 전역에서 `dataLayer` 객체를 사용할 수 있도록 `global.d.ts` 파일에 타입을 정의한다.
...
- 특정 클릭 이벤트가 발생했을 때 `dataLayer` 객체를 사용해 이벤트를 발행한다.
  - 이벤트 종류는 `custom_event`로 지정한다.
  - 변수로 지정되어 추적하는 `customData` 키에 랜덤한 값을 매칭하여 전달한다.
...
- 버튼을 클릭함에 따라 `custom_event`가 감지되는 것을 확인할 수 있다.
- `custom_event` 이벤트에서 `custom_data`가 수집됨을 확인할 수 있다.
