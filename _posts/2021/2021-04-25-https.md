...
* 다음과 같은 예를 들어보겠습니다.
    ...
    * `사용자_A`가 암호화 된 메세지를 `사용자_B`에게 전달합니다.
    * `사용자_B`는 암호화 된 메세지를 `PRIVATE_KEY_A`로 복호화 합니다.
    * `사용자_B`는 복호화 된 평문 내용을 확인할 수 있습니다.
...
* `공개 키(서비스 업체)`를 사용해 서버와 브라우저는 `대칭 키`를 공유합니다.
    ...
    * 서버는 암호화 된 메세지를 `비공개 키(서비스 업체)`로 복호화하여 `대칭 키`를 얻습니다.
* 서버와 브라우저는 `대칭 키`를 사용해 메세지를 암복호화합니다. 
    * 통신 메세지의 암복호화를 `비대칭 키`만 사용하는 것은 많은 연산이 필요하기 때문에 비효율적입니다.
