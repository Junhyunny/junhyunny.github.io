...
### 1.1. Advantages and Disadvantages of SOAP
...
* 기존 방화벽 및 프록시를 통해 쉽게 터널링이 되므로 HTTP 메세지 교환을 처리하기 위해 존재하는 광범위한 컴퓨팅 및 통신 인프라를 수정할 필요가 없습니다.
    * `SOAP`은 인터넷을 사용하는 모든 서버에 의해 지원되는 HTTP를 전송 매체로써 사용하기 때문에 쉽게 사용할 수 있습니다.
...
* 이름처럼 단순(simple)하지 않습니다.
    * 프로토콜 메세지 형식이 장황합니다.
* XML 메세지 구문을 분석하는데 비싼 비용이 듭니다.
    * HTTP 프로토콜을 직접 사용하는 REST 웹 서비스가 우세하게 됩니다. 
...
## 2. Web Service Protocol Stacks and Architecture

`SOAP`은 웹 서비스에서 사용되는 프로토콜입니다. 
웹 서비스 기술을 사용하기 위해선 다음과 같은 프로토콜들을 사용해 시스템을 구축해야합니다.
...
위 네 개의 프로토콜을 기반으로 웹 서비스는 다음과 같은 아키텍처를 구성합니다. 

1. 웹 서비스 제공자(provider)는 자신이 제공하는 API를 WSDL 문서로 만들어 UDDI에 공개합니다.
1. 웹 서비스 소비자(consumer)는 자신이 필요한 WSDL 문서를 탐색합니다.
...
웹 서비스 참여자들이 사용하는 `WSDL` 문서에 대해 살펴보겠습니다. 
`WSDL` 문서는 XML 기반으로 작성되었으며 .wsdl 확장자를 가지고 UDDI 레지스트리에 저장됩니다. 
