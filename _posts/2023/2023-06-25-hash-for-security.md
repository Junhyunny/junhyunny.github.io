...
해시(hash)는 단방향 암호화 기법이다. 평문을 암호화 된 텍스트로 만들어준다. 해시 함수(hash function)를 사용해 고정된 길이의 암호화 된 문자열로 변환한다. 고정된 길이의 암호화 된 문자열을 다이제스트(digest)라고 한다. 단방향이기 때문에 암호화 된 문자열을 다시 평문으로 복호화가 불가능하다. 일부만 변경되어도 전혀 다른 값이 된다는 특징 때문에 다음과 같은 용도로 사용하다.
