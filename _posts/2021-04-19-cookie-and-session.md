...
HTTP 통신은 요청을 보내는 측을 클라이언트(client), 요청을 받아 처리 후 적절한 응답을 반환하는 측을 서버(server)라고 합니다. 
웹 서비스의 경우 클라이언트는 보통 브라우저를 의미합니다. 
클라이언트와 서버는 HTTP(Hyper Text Transfer Protocol)을 기반으로 서로 통신합니다. 
HTTP 통신은 기본적으로 상태를 관리하지 않는 특징을 가지고 있습니다.

> 무상태 프로토콜(stateless protocol)은 어떠한 이전 요청과도 무관한 각각의 요청을 독립적인 트랜잭션으로 취급하는 통신 프로토콜 

무상태 프로토콜은 모든 요청이 독립적입니다. 
기본적으로 상태를 저장하지 않기 때문에 이전 요청이 현재 요청에 영향을 주지 않습니다. 
처음처럼 HTML 문서를 전달하는 용도로만 사용했다면 충분했겠지만, 현재는 HTTP 통신을 기반으로 다양한 서비스를 제공하기 때문에 무상태 프로토콜을 그대로 사용하기에 큰 어려움이 있습니다. 

실제로 웹 서비스들은 클라이언트의 동작으로 변경된 상태를 모두 유지합니다.

* 사용자가 로그인한 상태를 계속 유지
* 쇼핑몰 사용자가 로그인 없이도 장바구니에 담은 항목들을 유지

무상태 프로토콜 특징을 가진 HTTP 통신에서 서버가 사용자의 상태를 저장하는 방법이 바로 세션입니다. 
서버에 저장된 세션을 찾을 수 있는 키(key)가 브라우저에 저장되는 쿠키입니다. 
...
<p align="center">
    <img src="/images/cookie-and-session-4.JPG" width="80%" class="image__border">
</p>
<center>쿠키(Cookie)와 세션(Session)</center>
