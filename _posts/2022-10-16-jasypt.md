...
데이터베이스 유저와 비밀번호에 암호화 된 값을 사용한 예제 프로젝트를 통해 `Jasypt` 사용 방법을 알아보겠습니다. 
...
* `@SpringBootApplication`이나 `@EnableAutoConfiguration` 애너테이션을 기본적으로 사용하지 않습니다.
...
빈 이름은 기본적으로 `jasyptStringEncryptor`를 사용합니다. 
...
암호기를 만들 때 설정 값을 지정할 수 있습니다. 
비공개 키, 암호화 알고리즘, 인코딩(encoding) 타입, 솔트(salt) 값 생성기 등을 지정할 수 있습니다. 
비공개 키는 필수 값이고 나머지는 다음과 같은 기본 값들을 가집니다. 
...
* 암호 알고리즘은 편의상 기본 값이 아닌 `PBEWithMD5AndDES` 방식을 사용합니다.
    * 아래 사이트에서 `PBEWithMD5AndDES` 알고리즘 방식으로 암호화 값을 추출할 수 있습니다.
    * <https://www.devglan.com/online-tools/jasypt-online-encryption-decryption>
* 풀 사이즈 값은 기본 값이 1이라고 문서에 정리되어 있지만, 실제로 값을 지정하지 않으면 예외를 발생시키므로 1 값을 지정합니다.
...
암호화 된 값을 확인한 이후 코드는 제거합니다. 
비밀 키는 `HelloWorld` 문자열을 사용하였습니다.
...
* 비밀 키는 컨테이너 실행 시 환경 변수로 주입 받습니다. 
    * `jasypt.secrete-key=${JASYPT_SECRETE_KEY}`
    * 배포 환경 별로 이 값을 제어할 수 있습니다.
    * 운영 체제 환경 변수나 배포 스크립트로 `JASYPT_SECRETE_KEY` 값을 제어하여 컨테이너에 주입합니다.
...
* MySQL 데이터베이스 설정
    ...
    * 비밀번호는 `123`으로 지정합니다.
    * `healthcheck`를 통해 정상적으로 기동되었는지 확인합니다.
* 백엔드 어플리케이션 설정
    * 포트는 8080을 사용합니다.
...
MySQL 데이터베이스 기동 후 정상적으로 백엔드 서비스가 실행되는 것을 확인할 수 있습니다.
백엔드 서비스는 암호화 된 데이터베이스 접속 정보를 자동으로 복호화하여 데이터베이스에 연결하였음을 알 수 있습니다. 
...
* <https://www.devglan.com/online-tools/jasypt-online-encryption-decryption>
* [Jasypt를 사용한 암호화][jasypt-blog-link]
