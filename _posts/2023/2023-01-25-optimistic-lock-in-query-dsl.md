...
JPA(java persistence api)는 `@Version` 애너테이션으로 낙관적 락(optimistic lock) 기능을 제공한다. `spring-data-jpa`는 `save` 메서드로 추가(insert), 업데이트(update) 기능을 제공하면서 내부에 버전 확인 로직이 있기 때문에 쉽게 낙관적 락 기능을 사용할 수 있다.

반면에 `QueryDSL`은 업데이트에 낙관적인 락 방식이 자동으로 적용되지 않는다. 다음과 같은 방법으로 낙관적인 락 기능을 사용할 수 있다.

- 조회 쿼리와 엔티티의 오염 확인(dirty check)
  - 조회 쿼리에 락 타입(lock type)을 지정한다.
  - 조회된 엔티티를 변경하고, 엔티티의 오염 여부를 확인하는 과정을 통해 업데이트가 수행된다.
  - 업데이트를 수행할 때 버전 정보를 증가시킨다.
  - 다른 트랜잭션(transaction)에 의해 업데이트가 되었다면 버전이 달라지므로 낙관적 락 예외를 발생한다.
- 업데이트 쿼리 사용
  - 업데이트 쿼리 조건에 버전 정보를 추가한다.
  - 업데이트에 실패하면 버전 정보를 제외한 조회 조건으로 엔티티를 탐색한다.
  - 엔티티 존재함에도 업데이트가 실패했다면 버전 차이로 인식하여 낙관적 락 예외를 발생한다.

## 1. Using LockType and Dirty Check

엔티티 오염 확인을 통한 업데이트에서 낙관적 락을 발생시키는 방법에 대해 먼저 정리하였다.

### 1.1. Test Context

다음과 같은 테스트를 수행한다.

- `트랜잭션-1`은 다음과 같이 동작한다.
  - 엔티티를 조회한다.
  - 엔티티 필드 값을 변경한다.
  - 1초 대기한다.
  - 트랜잭션 종료 시 버전 정보가 맞지 않음을 확인한다.
  - 다른 트랜잭션에 의해 해당 엔티티가 업데이트 되었다고 인지하고, 커밋을 실패시키고 예외를 던진다.
- `트랜잭션-2`은 다음과 같이 동작한다.
  - 엔티티를 조회한다.
  - 엔티티 필드 값을 변경한다.
  - 정상적으로 커밋이 수행되고 엔티티 버전이 증가한다.
...
- `updateEntityWithLongTransaction` 메서드
  - 엔티티를 조회한다.
  - 조회 쿼리의 락 모드를 `OPTIMISTIC`으로 지정한다.
  - 엔티티의 값을 변경한다.
  - 1초 대기한다.
- `updateEntity` 메서드
  - 엔티티를 조회한다.
...
- 테스트를 위한 엔티티를 하나 생성한다.
- 각 트랜잭션 별로 스레드를 만들어 업데이트를 수행한다.
  - `tx1` - 중간에 대기 시간이 1초 있는 업데이트가 수행된다.
  - `tx2` - 일반적인 업데이트가 수행된다.
- `tx1` 처리 중간에 `OptimisticLockException` 예외가 발생할 것을 예상한다.
- `tx1`, `tx2`이 모두 종료된 후 저장된 모습은 `tx2`의 처리 결과 모습임을 확인한다.
...
- 업데이트 이전에 엔티티 조회 쿼리가 실행된다.
- 업데이트 쿼리의 조건으로 ID, 버전 정보를 사용한다.
- 변경한 필드 값들 이외에도 버전 정보를 업데이트한다.
...
해당 기능을 사용할 때 다음과 같은 것들을 고려해야 한다.

- 테스트가 어렵다.
  - 트랜잭션의 경합을 테스트하기 때문에 각 스레드를 만들고 트랜잭션을 커밋해야 한다.
  - H2 같은 임베디드(embedded) 데이터베이스를 사용하지 못하면 데이터베이스를 오염시킨다.
- 엔티티 객체의 캡슐화가 깨진다.
  - 엔티티의 오염 확인은 영속성 컨텍스트에서 관리 중인 엔티티를 기준으로 일어난다.
  - 필드 변경을 위해 엔티티에 `setter` 메서드를 만들어야 한다.
- 트랜잭션 경합이 발생하지 않는다면 낙관적인 락 메커니즘이 동작하지 않는다.
  - 트랜잭션 경합이 거의 발생하지 않는 느슨한 상황이라면 매 트랜잭션마다 조회한 엔티티를 기준으로 업데이트한다.

## 2. Update Query and Presence Check

`QueryDSL` 업데이트 쿼리를 사용하면서 자체적으로 낙관적인 락 메커니즘을 구현한다.

### 2.1. Test Context

- `트랜잭션-1`은 다음과 같이 동작한다.
  - 해당 ID, 버전 정보를 가지는 엔티티를 업데이트한다.
  - 업데이트 시 버전 정보를 현재보다 1 증가시킨다.
- `트랜잭션-2`은 다음과 같이 동작한다.
  - 해당 ID, 버전 정보를 가지는 엔티티를 업데이트한다.
  - 업데이트 시 버전 정보를 현재보다 1 증가시킨다.
  - 업데이트가 정상적으로 수행되지 않은 경우 이를 확인하고 예외를 던진다.
...
- 조회 조건을 만족하는 엔티티를 업데이트한다.
  - 버전 정보 확인을 제외한 다른 조회 조건은 `BooleanExpression` 객체로 만든다.
- 업데이트 된 데이터가 0건이 아니라면 해당 로직을 종료한다.
- 업데이트 된 데이터가 0건인 경우 다음과 같이 동작한다.
  - 버전 정보를 제외한 조건으로 데이터를 탐색한다.
  - 해당 데이터가 존재하는 경우 다른 트랜잭션에 의한 엔티티 변경이므로 `OptimisticLockException` 예외를 던진다.
  - 해당 데이터가 존재하지 않는 경우 `EntityNotFoundException` 예외를 던진다.
...
- 하나의 트랜잭션에서 2회 업데이트를 수행한다.
- 1차 업데이트는 정상적인 버전 정보를 지닌 엔티티로 업데이트를 수행한다.
- 2차 업데이트는 이전 버전 정보를 지닌 엔티티로 업데이트를 수행한다.
- 2차 업데이트에서 `OptimisticLockException` 예외가 발생할 것을 예상한다.
- 테이블에 저장된 모습은 1차 업데이트의 수행 결과일 것으로 예상한다.
...
- 업데이트 쿼리의 조건으로 ID, 버전 정보를 사용한다.
  - 업데이트 수행 시 엔티티의 버전을 함께 증가시킨다.
- 업데이트가 성공한다면 조회 쿼리를 수행하지 않는다.
- 업데이트가 실패한다면 조회 쿼리를 수행하고, 해당 결과에 따른 예외를 전달한다.
...
- 테스트가 단순해진다.
  - 테스트를 위해 트랜잭션 별로 스레드를 만들 필요가 없다.
  - 테스트 중간에 트랜잭션을 커밋할 필요가 없다.
- 데이터 정합성을 위한 업데이트를 하고자 엔티티의 버전 정보를 항상 들고 다녀야 한다.
- 업데이트 쿼리를 직접 작성하기 때문에 엔티티의 캡슐화를 깰 필요가 없다.
- 낙관적인 락에 대한 비즈니스 로직을 개발자가 직접 작성해야 한다.

## CLOSING

두 가지 방법에 대한 각 고려 사항들과 비즈니스를 고민하여 적절한 방법으로 구현한다.
