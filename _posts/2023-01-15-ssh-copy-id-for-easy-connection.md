...
* 클라이언트 PC에서 `ssh-keygen` 명령어를 사용해 공개키와 비밀키를 생성합니다.
* 공개키를 SSH 서버에 복사합니다.
* 비밀키를 사용해 SSH 서버에 접속합니다. 
* SSH Server IP - 192.168.78.132

### 1.1. Generate Public/Private Key

`ssh-keygen` 명령어를 사용해 공개키와 비밀키를 생성합니다. 
별도 설정이 없다면 `~/.ssh` 경로에 키가 생성됩니다. 
...
`ssh-copy-id` 명령어를 사용해 공개키를 SSH 서버에 복사합니다. 
