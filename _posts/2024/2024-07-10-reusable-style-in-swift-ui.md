---
title: "SwiftUI 재사용 가능한 스타일(Style)"
search: false
category:
  - iOS
  - swift-ui
...
프로덕트를 개발하면 공통으로 사용하는 디자인 컴포넌트들이 항상 존재한다. 작업 편의성을 위해선 공통적인 스타일을 재사용할 수 있는 코드 구현이 필요하다. SwfitUI 프레임워크는 웹 서비스처럼 공통 스타일 모듈을 만드는 CSS 같은 언어가 별도로 없다. SwiftUI는 이 문제를 어떻게 해결했을까? 
...
먼저 색상에 관련된 변수를 만들었다. 디자인 도구로 피그마(figma)를 사용하는 데 피그마는 색상 정보를 헥스(hex) 코드로 제공한다. SwiftUI는 헥스 코드로 색상을 표현하는 기능이 없기 때문에 Color 구조체의 생성자를 확장한다.
