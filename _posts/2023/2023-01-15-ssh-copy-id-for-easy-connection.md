---
title: "ssh-copy-id로 쉽게 SSH 연결하기"
search: false
category:
  - information
...
시스템 환경 작업을 하다보면 SSH(Secure Shell)을 많이 사용하는 데, 매번 인증하는 일이 상당히 불편하다. SSH 세션이 자주 끊기기도 하고, 쉘 스크립트에서 SSH 연결이 필요하면 인증 처리가 까다롭다. 이번 포스트에선 `ssh-copy-id` 명령어를 사용해 한번 인증된 클라이언트가 비밀번호 입력 없이 SSH 연결을 맺는 방법에 대해 정리하였다.

## 1. Practice

간단한 실습을 통해 사용 방법을 알아보겠다.

- 클라이언트 PC에서 `ssh-keygen` 명령어를 사용해 공개 키와 비밀 키를 생성한다.
- 공개 키를 SSH 서버에 복사한다.
- 비밀 키를 사용해 SSH 서버에 접속한다.
- SSH Server IP - 192.168.78.132

### 1.1. Generate Public/Private Key

`ssh-keygen` 명령어를 사용해 공개 키와 비밀 키를 생성한다. 별도 옵션을 주지 않는다면 `~/.ssh` 경로에 키가 생성된다.
...
`OpenSSH`의 일부 기능으로 매 로그인마다 비밀번호를 입력하지 않고 접근하기 위해 사용한다. `ssh-copy-id` 명령어를 사용해 공개 키를 SSH 서버에 복사한다.

- 클라이언트가 생성한 공개 키(public key)를 서버에 복사한다.
- `-i` 옵션으로 식별 공개 키를 지정한다.
  - 기본 값(default)이 `~/.ssh/id_rsa`이며 별도로 옵션 값을 지정하지 않으면 해당 키가 사용된다.
- SSH 서버의 사용자(user) 정보와 호스트 IP 정보를 추가한다.
  - `harbor@192.168.78.132`
  - SSH 서버 접속 ID - harbor
  - SSH 서버 호스트 IP - 192.168.78.132
- 비밀번호를 입력하여 인증 받으면 SSH 접속을 하라는 안내 메시지를 볼 수 있다.
