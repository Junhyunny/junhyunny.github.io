...
category:
  ...
  - spring-boot
  - security
last_modified_at: 2026-05-28T10:51:23+09:00
---
...
애플리케이션 설정에 노출되면 안 되는 민감한 정보들이 있다. 대표적으로 데이터베이스 접속 정보를 예로 들 수 있다. 환경 변수를 통해 주입해 주는 것이 안전하지만, 암호화된 값을 사용하는 방법도 존재한다. 이번 글에서는 암호화된 애플리케이션 설정값을 사용할 수 있도록 돕는 라이브러리를 소개한다.

## 1. Jasypt(Java Simplified Encryption)

자바(Java) 애플리케이션에서 설정 파일에 명시된 값들을 암호화, 복호화할 수 있는 라이브러리다. 스프링 프레임워크도 자바 애플리케이션이기 때문에 `Jasypt`를 사용할 수 있다. 평문(plain text)을 암호화한 값을 아래 형식으로 application.yml 파일에 추가하면 된다. `ENC(...)` 내부에 지정된 값은 스프링 애플리케이션이 실행될 때 자동으로 복호화된다.
...
데이터베이스 사용자 이름과 비밀번호에 암호화된 값을 지정하는 예제를 살펴보자. 메이븐(maven)을 의존성 관리 도구로 사용한다면 pom.xml 파일에 의존성을 추가한다. 스프링 부트(spring boot) 프레임워크에서는 `jasypt-spring-boot-starter` 의존성을 추가한다.
...
스프링 MVC 프레임워크를 사용한다면 `jasypt-spring-boot` 의존성을 추가한다.
...
스프링 MVC 프레임워크는 아래와 같이 @EnableEncryptableProperties 애너테이션으로 기능을 활성화해야 한다.
...
이제 암호화기를 만들어보자. 스프링 빈 객체의 이름은 `jasyptStringEncryptor`를 사용해야 한다. 변경하고 싶다면 아래와 같이 설정을 통해 변경한다.
...
스프링 빈 객체를 만들 때도 이름을 맞춰줘야 한다. @Bean 애너테이션에 이름을 `encryptorBean`으로 지정한다.
...
이번 예제는 기본으로 지정된 jasyptStringEncryptor 이름을 사용한다. 암호화기를 만들 때 옵션을 지정할 수 있다.
...
비공개 키는 필수로 입력받아야 한다. 나머지는 별도 설정이 없다면 다음과 같은 기본값을 사용한다. 기본값은 아래와 같다.
...
JasyptConfig 클래스에서 암호화기 빈(bean) 객체를 만든다.

1. 빈 이름은 Jasypt 라이브러리의 기본값을 사용한다. 별도로 지정하지 않으면 메서드 이름이 빈 이름이 된다. 메서드 이름을 `jasyptStringEncryptor`로 지정한다.
2. 비공개 키는 환경 변수로 주입받는다.
...
public class JasyptConfig {
    ...
    public StringEncryptor jasyptStringEncryptor( // 1
        ...
        config.setAlgorithm("PBEWithMD5AndDES"); // 3
...
[해당 사이트](https://www.devglan.com/online-tools/jasypt-online-encryption-decryption)에서 PBEWithMD5AndDES 알고리즘으로 암호화와 복호화가 가능하다.

- 왼쪽 창에서는 암호화, 오른쪽 창에서는 복호화를 수행할 수 있다.
- 양방향 암호화 타입을 사용한다.

<div align="center">
  <img src="{{ site.image_url_2022 }}/jasypt-01.png" width="100%" class="image__border">
</div>
<center>https://www.devglan.com/online-tools/jasypt-online-encryption-decryption</center>

<br/>

위 사이트에서 생성한 암호화된 값을 application.yml 파일에 설정한다. 필자는 비공개 키로 `HelloWorld` 값을 사용했다.

- `spring.database.username`, `spring.database.password` 설정에 암호화된 값을 사용한다.
  - `ENC()`로 감싼다.
- `jasypt.secret-key` 설정은 환경 변수를 사용한다.
  - 호스트 머신이나 컨테이너의 환경 변수를 사용해서 해당 값을 주입한다.
...
services:
  ...
  backend:
    ...
    ports:
      - '8080:8080'
    environment:
      - JASYPT_SECRET_KEY=${JASYPT_SECRET_KEY:?Set JASYPT_SECRET_KEY}
    depends_on:
      mysql:
...
도커 컴포즈를 실행해 정상적으로 데이터베이스에 연결되는지 살펴보자. 데이터베이스와 연결이 정상적으로 수행되고 오류 없이 백엔드 애플리케이션이 실행된다.
