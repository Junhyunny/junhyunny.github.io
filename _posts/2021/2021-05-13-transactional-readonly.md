...
어떤 뉘앙스인지 알 것 같지만, 기능에 대한 정확한 이해를 원했다. 검색 중 백기선님의 댓글을 발견했다.
...
관련된 내용들을 바탕으로 다시 정리해봤다.

- 의도치 않게 데이터를 변경하는 것을 막아준다.
- 하이버네이트(hibernate)를 사용하는 경우에는 플러시 모드를 매뉴얼(manual)로 변경한다. 오염 감지(dirty checking) 과정을 생략하면서 속도 향상 효과를 얻는다.
- 데이터베이스에 따라 데이터소스(datasource) 연결 수준에서 약간의 최적화가 가능하다.

## 2. Practice

데이터소스 연결 설정에 관련된 내용을 제외하고 요약한 내용들을 간단한 테스트 코드를 통해 살펴보겠다. 다음과 같은 환경에서 테스트를 수행한다.

- JDK17
- Spring Boot 3.1.0
- TestConatiner for MySql

application YAML 파일에서 로그 확인을 위해 필요한 패키지들의 로그 레벨을 트레이스(trace)로 변경한다.
...
readOnly 속성을 사용하는 PostService 클래스의 코드를 살펴보자. 클래스에 readonly 속성을 true 값으로 설정한다. 클래스 하위 모든 메서드들에게 적용된다.

- arhive 메서드
  - 데이터를 조회 후 엔티티의 상태를 변경한다.
  - 오염 감지 기능이 동작하기를 예상한다.
...
@Transactional 애너테이션에 readOnly 속성이 부여되면 어떤 동작을 하는지 살펴보기 위한 테스트 코드를 작성한다.

- throw_exception_when_create_in_readonly 테스트
  - readonly인 경우 새로운 데이터 삽입(insert)시 에러가 발생한다.
  - "Connection is read-only. Queries leading to data modification are not allowed" 에러 메시지를 확인한다.
- nothing_change_when_update_in_readonly 테스트
  - readonly인 경우 업데이트가 이뤄지지 않는다.
- nothing_change_when_dirty_check_in_readonly 테스트
  - readonly인 경우 업데이트가 이뤄지지 않는다.
...
각 테스트 별로 로그를 살펴보자. 먼저 throw_exception_when_create_in_readonly 테스트의 로그를 살펴보자.

- 삽입(insert) 쿼리가 출력된다.
- SQL Error: 0, SQLState: S1009 에러가 발생한다.
- "Connection is read-only. Queries leading to data modification are not allowed" 메시지가 출력된다.
...
nothing_change_when_update_in_readonly 테스트 결과를 살펴보자.

- 업데이트 쿼리 관련 로그가 출력되지 않는다.
...
마지막으로 nothing_change_when_dirty_check_in_readonly 테스트 수행 결과를 살펴보자.
...
관련된 글들을 확인하니 데이터베이스 제품에 따라 readOnly 기능 제공 여부가 다르다고 한다.
