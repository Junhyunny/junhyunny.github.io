...
* 마스터 노드에 문제가 발생하는 경우 이를 슬레이브 노드가 대체합니다.
    * 다만 슬레이브 노드는 읽기 연산에 대해서만 정상적인 동작을 보장합니다.
...
* 어플리케이션 화면을 통해 자기 자신에게 간단한 메세지를 전송합니다.
* 전송한 메세지는 레디스 리스트(list)에 저장됩니다.
* 레디스 리스트는 두 가지 존재합니다.
    * 읽지 않은 메세지들을 저장하는 리스트
    * 읽은 메세지들을 저장하는 리스트
* 메인 화면에서 간단한 API 호출을 통해 읽지 않은 메세지가 몇 개인지 확인할 수 있습니다.
* 메세직 확인 링크를 눌러 화면을 이동합니다.
* 이동하면 두 개의 리스트 상태를 확인할 수 있습니다.
...
다음과 같은 상황을 고려하여 테스트를 수행합니다.

* 메인 화면에서 읽지 않은 메세지 개수를 조회하는 기능은 읽기입니다.
* 새로운 메세지를 작성하는 기능은 쓰기 연산이 발생합니다.
* 메시지 리스트 현황 화면으로 이동할 때 쓰기 연산이 발생합니다.
    * 읽지 않은 메시지 리스트에서 모두 꺼내어(pop) 읽은 메세지 리스트로 이동합니다.
* 마스터 노드를 중지시켰을 때 다음 내용들을 예상합니다.
    * 읽기 연산이 가능한 메인 화면 새로고침은 정상적으로 수행됩니다.
    * 새로운 메세지를 작성 후 전송 버튼을 누르면 쓰기 연산으로 인해 정상적으로 수행되지 않습니다.
    * 리스트 상황 페이지로 이동하면 쓰기 연산으로 인해 정상적으로 수행되지 않습니다. 
