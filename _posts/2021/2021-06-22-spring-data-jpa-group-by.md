...
QueryDSL 같은 라이브러리를 사용하면 더 쉽게 쿼리를 작성할 수 있다. 쿼리가 점점 복잡해진다면 이를 고려해봐도 좋을 것 같다. `JDK 13`부터 텍스트 블록(""") 기능을 제공하여 쿼리를 작성하는 작업이 더 용이해졌다. 이를 활용하는 것도 복잡한 쿼리를 작성하는데 도움이 될 것 같다.
