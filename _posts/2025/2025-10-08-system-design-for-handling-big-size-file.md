...
1. 클라이언트가 최초 사전 서명된 URL을 요청할 때 서버는 `job_id`를 생성한다. 클라이언트가 만들어 서버에게 전달해도 무관하다. 중요한 것은 클라이언트가 job_id를 알고 있어야 한다는 것이다. job_id는 항상 다른 값을 갖도록 UUID를 사용한다.
2. 서버는 job_id와 함께 작업 상태를 `PENDING`으로 데이터베이스에 저장한다. 사전 서명된 URL을 만들 때 S3 오브젝트 경로에 job_id를 포함시킨다. 예를 들면, 원본 파일은 `{job_id}/uploads` 경로, 처리된 파일은 `{job_id}/processed` 경로를 사용한다.
3. 클라이언트는 사전 서명된 URL을 통해 S3의 {job_id}/uploads 경로로 파일을 업로드한다.
4. 파일 업로드가 완료되면 ECS 태스크가 실행되는데, 이때 S3 객체의 메타 데이터나 경로를 통해 `job_id`를 전달받는다.
5. ECS 태스크는 작업 상태를 시작할 때 `PROCESSING`, 완료 시 `COMPLETED`로, 실패 시 `FAILED` 등으로 업데이트한다.

S3 이벤트가 중복으로 발생하여 ECS 태스크가 여러 번 실행되는 케이스를 고려한다. 동일한 요청에 항상 같은 결과를 보장하는 멱등성(idempotency)이 필요하다. 데이터베이스에 저장된 `job_id` 상태를 확인한다. ECS 태스크가 시작될 때 상태가 PENDING이 아니면 작업을 중복 실행하지 않고 바로 종료한다.
...
이 팀은 큰 파일을 처리하는 아키텍처에 대한 고민뿐만 아니라 원인을 알 수 없는 서버 에러가 발생하고 있었는데, 이 부분은 시스템 버그였다. 시스템 버그를 찾아준 것만으로 일단 급한 불은 껐는지 새로 제안한 아키텍처에 대한 구현은 추후로 미뤄진 것 같다. 대안책을 전달한 미팅 이후에 별다른 연락은 없었다. 가능하다면 이 시스템 디자인을 함께 개발하고 싶었는데 아쉽다. S3-ECS 태스크 같은 AWS 리소스들 사이의 연결 부분이 궁금해서 구축해봤는데 이 내용은 다음 글을 통해 정리할 생각이다.
