...
- 엔티티 매니저(entity manager)의 `flush` 메서드를 명시적으로 호출
- 엔티티 매니저의 트랜잭션(transaction) 객체의 `commit` 메서드 호출
- `FlushModeType.AUTO` 설정인 경우 JPQL(Java Persistence Query Language) 쿼리(Query) 실행 시점

flush, commit 메서드처럼 명시적인 호출은 이해하기 쉽다. 기능을 제대로 파악하지 못 해서 발생하는 실수를 줄이기 위해 세 번째 조건에 대한 내용을 좀 더 자세히 살펴보자. 
...
3. 트랜잭션 마지막에 커밋이 발생하면서 영속성 컨텍스트의 오염된 엔티티가 오염 확인(dirty checking)에 의해 업데이트된다.
...
5. clear 메서드로 영속성 컨텍스트를 비우고 재조회한다.  
