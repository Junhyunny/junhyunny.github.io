...
순환 참조는 서로를 참조하고 있기 때문에 [참조 카운팅 알고리즘][reference-counting-gc-in-javascript-link]으론 사용되지 않는지 여부를 판단하기 어렵다. 순환 참조는 서로 연결되어 섬 같은 구조를 만드는데, 이를 `도달할 수 없는 섬`이라고 표현한다. 마크-앤-스윕 알고리즘을 사용하면 이렇게 도달할 수 없는 섬 전부를 메모리에서 삭제할 수 있다.
