...
`REST` API는 HTTP 메서드(method)를 통해 동작 방식을 정한다. 
...
PUT 메서드는 함께 전달한 엔티티(entity)를 요청 URI 아래 저장하길 기대한다. 다만, 요청 URI 하위에 리소스(resource) 존재 여부에 따라 다르게 동작한다.
...
데이터를 수정한다는 인식이 강한 PUT 메서드는 리소스 존재 유무에 따라 데이터를 생성하거나 수정한다. PUT 요청은 URI에 식별자 정보가 포함된다. 해당 식별자에 매칭되는 리소스의 존재 유무에 따라 응답이 달라진다.
...
데이터를 생성할 때 사용하는 POST 메서드를 살펴보자. POST 메서드는 함께 전달한 엔티티를 요청 URI 하위에 저장할 신규 리소스로 식별한다. POST 메서드는 반드시 URI로 식별될 수 있는 리소스를 생성하는 것은 아니다. 이런 경우 200 (OK) 또는 204 (No Content) 응답 상태 코드가 적절하며, 응답에 요청의 결과를 설명하는 엔터티가 포함되었는지 여부에 따라 선택하면 된다.
...
POST 메서드와 PUT 메서드의 근본적인 차이는 요청 URI에 있다. 
