...
구독-발행 메시지 프로토콜이기 때문에 일-대-다로 메시지를 전송할 수 있으며 애플리케이션들 사이의 결합도(coupling)를 낮추는 효과를 얻을 수 있다. 브로커(broker), 발행자(publisher) 그리고 구독자(subscriber) 컴포넌트가 이 프로토콜에 참여한다. 발행자는 IoT 디바이스, 구독자는 필자가 개발할 애플리케이션이다. 브로커 역할을 수행할 메시지 큐는 대표적으로 Mosquitto, HiveMQ, RabbitMQ, VerneMQ 등이 사용되는 것 같다. 
...
- 신뢰성
  ...
  - QoS(Quality of Service) 정의를 통해 메시지 전송에 대한 
- 보안
