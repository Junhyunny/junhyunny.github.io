---
title: "MySQL 커서를 사용한 시퀀스 키 업데이트"
search: false
category:
  - database
  - mysql
...
테이블 스키마 모습이 적절하지 않아 변경 작업이 필요했다.

- 비정형 문자열을 저장하는 두 개의 컬럼을 기본 키(PK, Primary Key)로 설정하고 있었다.
...
저널, 출판사 이름을 기본 키로 설정하는 경우 이름이 같은 경우가 발생한다면 문제가 발생할 수 있다. 전 세계 저널과 출판사를 대상으로 하기 때문에 가능성이 없는 것은 아니었다. 위 테이블을 다음과 같이 변경하고자 했다.
...
실제 데이터는 약 40000건 이상 존재했지만, 이번 포스트에선 간단하게 10000건 임의 데이터를 만들어 정리하였다. 원하는 작업을 수행하기 위해 일회성 애플리케이션을 만드는 일보다 SQL 스크립트 작성이 더 쉬운 길이라 판단했다. 새로운 ID 컬럼을 생성하는 것은 어렵지 않았지만, ID 컬럼을 기본 키로 변경하기 위해 모든 데이터가 서로 다른 값을 가지도록 업데이트해야 하는 문제가 있었다. 이 문제는 간단한 프로시저(procedure)를 작성하여 처리하였다.

### 2.1. Procedure

프로시저 개념에 대해 살펴보겠다. 프로시저는 데이터베이스 관리 시스템(DBMS, Database Management System)에서 실행할 수 있는 하나 이상의 SQL 문장을 그룹으로 만들어 사용하는 데이터베이스 객체이다. 프로시저는 일련의 작업을 수행할 수 있으며, SQL 코드 블록을 만들어 재사용할 수 있다. 다음과 같이 만들고, 호출할 수 있다.

- CREATE 명령어를 통해 프로시저를 정의한다.
  - 프로시저에 대한 파라미터가 추가될 수 있다.
- BEGIN, END 사이에 필요한 SQL 구문들을 작성한다.
- CALL 명령어를 통해 생성한 프로시저를 실행한다.
...
데이터를 조회한 후 조작을 수행하기 때문에 프로시저 내부에서 커서(cursor)를 사용했다. 데이터베이스 커서는 조회 결과 집합을 탐색하고 조작하기 위해 사용한다. 커서를 사용하면 일련의 데이터에 순차적으로 접근할 수 있다.

다음과 같은 일련의 과정을 통해 커서를 사용한다.

1. DECLARE
  - select 쿼리를 수행하여 조회한 결과를 사용하는 커서를 선언한다.
2. OPEN
  - 선언한 커서를 사용하기 위해 연다.
3. FETCH
  - 커서를 다음 행(row)으로 옮기는 작업을 수행한다.
  - INTO 키워드를 통해 커서가 가리키는 행의 데이터를 변수에 주입한다.
4. CLOSE
  - 커서 사용이 종료되면 닫는다.
...
데이터 조작을 위한 UPDATE_JOURNAL_PUBLISHER_MAPPING 프로시저를 정의한다. 스크립트 설명은 가독성을 위해 주석으로 작성하였다.
...
남은 작업들을 수행한다. 이전 기본 키는 삭제하고 ID 컬럼을 새로운 기본 키로 선언한다. 저널 이름과 출판사 이름에 걸린 NOT NULL 제약 조건은 제거한다.
...
기본 키 ID 컬럼에 자동 증가 속성을 추가하고 초기 값을 지정한다.
...
저널 이름, 출판사 이름에 걸린 제약사항은 정리한다. 저널 ID, 출판사 ID를 조합하여 유니크 키를 생성한다.
