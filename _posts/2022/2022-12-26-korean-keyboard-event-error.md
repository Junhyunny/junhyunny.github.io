...
리액트(react) 프로젝트에서 한글을 입력할 때 마지막 문자가 중복으로 입력되는 현상을 발견하였다. 한글 입력 시 발생하는 문제점과 원인, 해결 방법에 대해 정리하였다.
...
- 입력된 값을 저장하고 텍스트 박스의 값은 초기화한다.
- 입력된 값의 마지막 문자가 추가로 저장된다.
...
[Error on v-model with Korean in Vue][vue-js-korean-length-link] 포스트에서 다뤘던 개념이다. 이 문제는 IME(Input Method Editor) 과정에서 `KeyDown` 이벤트가 발생할 때 운영체제와 브라우저가 해당 이벤트를 중복 처리하기 때문에 발생한다. 위키피디아(wikipedia)에선 IME를 다음과 같이 정의한다.
...
해결 방법은 `Web API`에서 제공하는 `CompositionEvent`와 관련 있다. `CompositionEvent`는 키보드에서 사용할 수 없는 문자를 입력 받기 위한 보조적인 방법을 제공한다.

- US 키보드엔 존재하지 않지만, 문자에 강조(accent)를 줄 때 사용
- 아시아 언어의 기본 컴포넌트인 로고그램(logogram)들을 빌드-업(build-up)할 때 사용

문자를 합성하는 컴포지션 세션(composition session)은 `compositionstart`, `compositionupdate`, `compositionend` 이벤트로 구성된다. `compositionupdate` 이벤트는 여러 번 발생할 수 있다. 각 세션 동안 이벤트 체인 각 단계 사이의 값들은 지속되며 `data` 속성에 유지된다. 브라우저는 컴포지션 세션을 통해 IME 기능을 제공한다.

### 3.2. isComposing Property

키보드 이벤트를 살펴보면 `isComposing`라는 속성이 존재한다.
...
`isComposing` 속성은 컴포지션 세션이 시작될 때 `true`, 세션이 종료될 때 `false` 상태가 된다. 컴포지션 세션의 상태를 확인해 키보드 이벤트를 제어하면 한글 입력 문제를 방지할 수 있다.

### 3.3. 문제 해결 코드

- 리액트 라이브러리 `KeyboardEvent` 이벤트 내부 `nativeEvent` 객체의 `isComposing` 속성을 사용한다.
  - 바닐라 자바스크립트가 아니기 때문에 이벤트 객체 내부엔 `isComposing` 속성이 존재하지 않는다.
- `isComposing` 상태가 `true`인 경우에 키보드 이벤트를 막는다.
...
`onKeyDown` 이벤트를 `onKeyPress` 이벤트로 변경하면 해당 문제가 해결되지만, 다음과 같은 문제점들이 존재한다.

- 리액트에서 `onKeyPress` 이벤트는 더 이상 지원하지 않는다.(deprecated)
- `onKeyPress` 이벤트는 한/영, Shift, Backspace 등의 키를 인식하지 못 한다.
