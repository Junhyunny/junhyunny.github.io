...
⚠️ Vue.js 코드에서 `{ { } }`으로 표기된 코드는 띄어쓰기를 붙여야지 정상적으로 동작합니다.(Theme 예약어로 인한 표기 에러)
...
### 1.2. App.vue
- 조회 버튼을 클릭 시 `search` 함수를 데이터를 조회합니다.
...
#### 2.3.2. API End-Point Pageable 인터페이스 파라미터(parameter) 사용
...
작성하다보니 내용이 단순하여 살을 붙이기 시작했는데 `Vue.js`와 `Spring Boot`를 활용한 페이징 테이블 구현으로 포스트가 마무리되었습니다. 
페이징 처리는 단순할 것 같으면서도 상당히 번거로운 작업입니다. 
사용하는 기술 스택에 따라 구현 방법이 달라지고, 공통으로 사용하는 컴포넌트로 뽑아내면 입 맛에 맞게 사용하기 어려울 때도 있습니다. 
