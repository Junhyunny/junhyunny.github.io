...
영상 녹화나 사진 촬영 기능을 테스트하면 다음과 같은 문제들이 발생한다.

- CI/CD 파이프라인에는 실제 연결된 카메라가 없으므로 촬영이 불가능하다.
- 테스트마다 촬영 모습 매번 다르다면 결과를 검증하는 것이 어렵고, 테스트는 매번 실패할 것이다.
...
FFmepg 설치가 완료되었으면 다음 명령어를 통해 비디오 포맷을 변경한다. 입력 비디오는 맥북에서 유튜브 영상을 짧게 녹화한 것을 사용했다. y4m 포맷의 영상은 압축 과정 없이 각 프레임의 모든 픽셀 데이터를 그대로 기록하기 때문에 데이터의 양이 압출 파일보다 훨씬 많다. 해상도와 프레임을 낮춰 파일 사이즈를 제한한다. 변환된 output.y4m 파일은 E2E 프로젝트의 media 경로에 위치시킨다.
...
3. "녹화 종료" 버튼을 클릭한다.
4. 다운로드 경로에 파일이 해당 이름으로 파일이 다운로드 되었는지 확인한다.
...
1. 화면에 접속 후 사진 촬영 기능으로 이동한다. 즉시 녹화를 시작하지 않고, 미디어 스트림이 연결되는 것을 500ms 기다린다. 
2. "촬영" 버튼을 클릭한다.
3. 다운로드 경로에 파일이 해당 이름으로 파일이 다운로드 되었는지 확인한다.
