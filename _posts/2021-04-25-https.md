...
* 어플리케이션이 메세지를 수신할 때 보안 계층은 전송 계층에서 받은 메세지를 복호화하여 어플리케이션 계층으로 올려줍니다. 
...
대칭 키/비대칭 키 암호화 알고리즘을 통해 메세지를 암복호화한다는 것은 알았습니다. 
...
<p align="center">
    <img src="/images/https-6.JPG" width="80%">
</p>

##### Delivery SSL Certificate Between Server and Browser

서버와 브라우저 사이에서 발생하는 SSL 인증서 전달 과정은 다음과 같습니다.

1. 브라우저가 서버에게 데이터 요청을 합니다.
1. 서버는 브라우저에게 SSL 인증서를 전달합니다.
1. 브라우저는 자신이 관리하는 공인된 CA 목록 중에 해당 SSL 인증서를 암호화한 CA가 있는지 확인합니다.
1. 목록에 존재하는 CA라면 브라우저는 **이미 알고 있는 해당 CA의 공개 키**를 이용해 해당 인증서를 복호화합니다.

<p align="center">
    <img src="/images/https-7.JPG" width="80%">
</p>
