...
* 숫자 2.625는 32비트 고정 소수점 표현으로 0 000000000000010 1010000000000000 입니다.
    * 부호 0
    * 정수부 10
    * 실수부 101
    * 나머지는 0으로 패딩(padding)
...
부동 소수점 방식의 가장 큰 단점은 실수 연산이 부정확하다는 것 입니다. 
`Java`에서 실수를 표현하는 `float`, `double`은 부동 소수점 방식을 사용하는데, 이를 사용한 연산은 부정확한 결과를 내놓는 경우가 있습니다. 
