---
title: "CSS 트리 뷰(Tree View)"
search: false
category:
  - html
  - css
  - javascript
last_modified_at: 2026-09-17T14:07:25+09:00
---
...
디렉토리 구조를 표현할 때 많이 사용하는 트리(tree)를 구현하였다.

### 1.1. HTML 코드

우선 HTML 구조를 살펴보자.

- 각 노드별로 `.node` 클래스를 가진다.
- 하위 노드가 없는 경우 `.leaf` 클래스를 가진다.
- 각 라벨별로 매칭되는 체크 박스가 존재한다.
- 라벨 클릭 시 매칭된 체크 박스가 체크됨에 따라 하위 트리가 열리거나 닫힌다.
...
다음 스타일링 코드를 살펴보자. 영역 표시나 크기를 설정하는 속성은 설명에서 제외하였다. 가독성을 높이기 위해 설명은 주석에 작성하였다.
...
자바스크립트 코드를 통해 사용자 인터랙션으로 트리 뷰를 열고 닫아보자.
