...
시스템 구축이 끝나면 운영으로 넘어가기 전에 로깅(logging) 시스템 구축이 반드시 필요하다. 동작 중인 애플리케이션은 디버거(debugger)가 없기 때문에 시스템 운영자는 장애나 문제 상황을 로그를 통해 확인해야 한다. 예전 회사는 공통 컴포넌트 팀에서 로깅을 맡아줬지만, 현재 팀은 직접 구축해야 하기 때문에 관련된 지식을 탐구했다. 이번엔 Java 진영에서 많이 사용되는 로깅 프레임워크에 대한 이야기이다.

## 1. SLF4J(Simple Logging Facade for Java)

> The Simple Logging Facade for Java (SLF4J) serves as a simple facade or abstraction for various logging frameworks (e.g. java.util.logging, logback, log4j) allowing the end user to plug in the desired logging framework at deployment time. 

`SLF4J`는 퍼사드(facade) 패턴을 통해 다양한 로깅 프레임워크를 지원하는 추상화 라이브러리이다. 별도의 추상화 계층은 개발자가 코드 수정 없이도 원하는 로깅 시스템을 실행하는 시점에 선택할 수 있도록 돕는다.

- `SLF4J`을 직접 구현하거나 어댑터를 사용하는 등 구현체로 연결되는 방법이 다르기 때문에 대문(Façade)이라는 용어가 사용된 것 같다.
  - 어떤 프레임워크는 `SLF4J`의 로거(logger) 인터페이스를 직접 구현하였다.
  - 어떤 프레임워크는 `SLF4J`의 로거 인터페이스를 구현한 어댑터(adapter) 클래스를 사용한다.
- `SLF4J` 추상 레이어를 통해 사용할 수 있는 로깅 프레임워크들은 다음과 같다.
...
`SLF4J`은 세 가지 모듈을 제공한다. 이를 통해 여러 개의 로깅 프레임워크를 하나로 통합하여 사용할 수 있다.

- Bridge Module
  - 브릿지(bridge)는 다른 로깅 프레임워크를 `SLF4J`로 연결하기 위해 사용한다.
  - 레거시 코드나 외부 라이브러리에서 다른 로깅 프레임워크를 호출하는 경우 이를 `SLF4J`로 연결해준다.
  - jcl-over-slf4j.jar, log4j-over-slf4j.jar, jul-to-slf4j.jar 라이브러리 등이 있다.
...
- API Module
  ...
  - API를 사용하는 클라이언트(client)는 실제 로깅을 제공하는 구현체를 모른다.
...
- Binding Module
  - 바인딩(binding)은 `SLF4J` API와 로깅 프레임워크의 실제 구현체를 연결하는 어댑터 역할을 수행한다.
  - logback-classic.jar, logback-core.jar, slf4j-reload4j.jar, slf4j-jdk14.jar, slf4j-simple.jar 라이브러리 등이 있다.
...
간단한 실습 코드를 통해 관련된 내용을 살펴보겠다. 예제 코드를 통해 다음과 같은 환경을 구성한다.

- 세 가지 종류의 로거를 사용해 로그를 출력한다.
  - JUL(Java Util Logging)
  - JCL(Jakarta Commons Logging)
  - SLF4J
- JUL, JCL은 브릿지를 통해 SLF4J API로 연결한다.
- 바인딩을 통해 실제 구현체인 Logback 로깅 프레임워크로 연결한다.
...
- logback 의존성을 추가하지 않는 경우 다음과 같은 에러를 만나게 된다.
  - `SLF4J` 구현체를 찾을 수 없어서 발생한다.
...
- 각 로거 별로 "Hello World" 문자열과 로그를 출력하는 인스턴스의 클래스 이름을 함께 출력한다.
...
- `slf4jLogger` 인스턴스의 구현체 클래스 이름은 `ch.qos.logback.classic.Logger`이다.
- `javaUtilLogger` 인스턴스의 부모 로거의 핸들러 이름은 `org.slf4j.bridge.SLF4JBridgeHandler`이다.
  - JUL 로거는 내부적으로 핸들러를 사용해 로그를 출력한다.
  - 내부 코드를 살펴보면 부모 로거의 핸들러까지 탐색하여 로그를 출력한다.
- `apacheCommonsLogger` 인스턴스의 구현체 클래스 이름은 `org.apache.commons.logging.impl.SLF4JLocationAwareLog`이다.
