...
이전에 작성했던 [Spring Security JWT(Json Web Token) 인증 예제][spring-security-example-link] 글을 참조했는데, 마음에 들지 않는 부분들이 있어서 다시 정리하였습니다.
...
##### Spring Security JWT(Json Web Token) 인증 예제 서비스 구조
