...
Java 가상 머신이라고 불리는 소프트웨어로 Java 애플리케이션이 동작할 수 있는 환경을 제공합니다. 
JVM은 애플리케이션과 운영체제(OS, Operating System) 사이에서 중재자 역할을 수행합니다. 
...
JVM은 이 영역에 애플리케이션에서 사용하는 데이터들을 적재합니다. 
...
가비지 컬렉터 동작 시 JVM 애플리케이션 실행이 멈추게 되는데 이를 `STOP-THE-WORLD` 라고 표현합니다. 
