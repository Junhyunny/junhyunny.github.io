---
title: "자바 스레드 풀(Java Thread Pool)"
search: false
category:
  - java
last_modified_at: 2026-09-23T00:24:35+09:00
---
...
스레드 풀(thread pool)은 프로그램의 동시성 실행을 지원하는 디자인 패턴이다. 복제된 작업자(replicated workers) 또는 작업자-크루(worker-crew) 모델이라고 불리기도 한다. 스레드 풀은 여러 스레드를 유지·관리하고, 풀 내의 스레드는 작업(task)이 할당되기를 기다린다.
...
- 짧은 작업을 위해 스레드를 생성하고 수거하는 데 드는 비용을 줄일 수 있다. 스레드 생성과 수거는 운영체제 자원을 사용하기 때문에 비용이 크다.
...
자바(Java)에서 제공하는 스레드 풀의 구조를 아래 이미지를 통해 살펴보자.
...
<div align="center">
  <img src="{{ site.image_url_2023 }}/thread-pool-in-java-02.png" width="100%" class="image__border">
</div>

## 2. Types of Thread Pool in Java

JDK 1.5부터 java.util.concurrent 패키지를 통해 동시성 관련 기능을 제공했다. 자바에서 스레드 풀 기능을 제공하는 주요 인터페이스와 클래스를 살펴본 후 각 스레드 풀의 특징을 살펴보자.

- Executors 클래스
  - 정적 팩토리 메서드 패턴을 통해 스레드 풀 구현체 인스턴스를 생성한다.
  - 이번 글에서 살펴볼 정적 팩토리 메서드들은 다음과 같다.
    - newCachedThreadPool
    - newFixedThreadPool
    - newScheduledThreadPool
- ExecutorService 인터페이스
  - 스레드 풀 기능을 제공하는 클래스들은 ExecutorService 인터페이스를 구현한다.
  - 스레드 풀로서 제공해야 하는 API가 정의되어 있다.
  - ExecutorService 인터페이스로 추상화된 메서드를 통해 스레드 풀 기능을 활용할 수 있다.
- ThreadPoolExecutor 클래스
  - ExecutorService 인터페이스의 구현체 클래스이다.
  - 생성자를 살펴보면 다음과 같은 파라미터를 전달받는다.
  ...
  - `handler` - 큐 용량이 초과되어 작업 실행이 거부될 때 사용하는 핸들러 객체이다.
...
먼저 캐시 스레드 풀(Cached Thread Pool)에 대해 살펴보자. `newCachedThreadPool` 팩토리 메서드를 통해 생성한다. 해당 스레드 풀은 다음과 같은 특징을 가진다.

- ThreadPoolExecutor 인스턴스이다.
- 스레드 풀에서 반드시 유지되어야 하는 코어 스레드 개수는 0이다.
- 스레드 풀에 담을 수 있는 최대 스레드 개수는 `Integer.MAX_VALUE`이다.
- 60초 동안 작업을 할당받지 못하면 스레드가 종료된다.
...
간단한 예시 코드를 통해 동작 모습을 살펴보자. 다음과 같은 설정으로 스레드 풀을 생성한다.
...
위 테스트 코드를 실행했을 때 로그를 살펴보자. 각 작업이 들어올 때마다 풀에서 사용되는 스레드가 모두 다른 것을 확인할 수 있다. 모든 테스트 코드는 통과한다.

- 작업을 전달한 직후 스레드 풀 사이즈가 10이다.
- 60초 뒤에 스레드 풀 사이즈가 0이다.
...
다음은 고정 크기 스레드 풀(Fixed Thread Pool)이다. `newFixedThreadPool` 팩토리 메서드를 통해 생성하며, 다음과 같은 특징을 가진다.

- ThreadPoolExecutor 인스턴스이다.
- 스레드 풀에서 반드시 유지되어야 하는 코어 스레드 수는 파라미터로 전달받은 `nThreads` 값이다.
- 스레드 풀에 담을 수 있는 최대 스레드 수는 파라미터로 전달받은 `nThreads` 값이다.
- 0초 동안 작업을 할당받지 못하면 스레드가 종료된다.
...
마찬가지로 간단한 예시 코드를 통해 동작 모습을 살펴보자.
...
위 테스트 코드를 실행했을 때 로그를 살펴보자. 10개의 작업을 5개의 스레드가 나눠서 수행하는 것을 확인할 수 있다.

- 작업을 전달한 직후 스레드 풀 사이즈가 5이다.
- 5초 뒤에 스레드 풀 사이즈가 5이다.
...
마지막으로 스케줄 스레드 풀(Scheduled Thread Pool)을 살펴보자. `newScheduledThreadPool` 팩토리 메서드를 통해 생성한다. 해당 스레드 풀은 다음과 같은 특징을 가진다.
...
- 스레드 풀에서 반드시 유지되어야 하는 코어 스레드 수는 파라미터로 전달받은 `corePoolSize` 값이다.
- 스레드 풀에 담을 수 있는 최대 스레드 수는 `Integer.MAX_VALUE`이다.
- 10ms 동안 작업을 할당받지 못하면 스레드가 종료된다.
...
위 테스트 코드 중 run_scheduled_task 메서드를 실행했을 때 로그를 살펴보자. 10개의 작업을 5개의 스레드가 나눠서 수행하는 것을 확인할 수 있다. 첫 번째 작업을 예약한 지 1초 뒤에 `Run at last` 로그가 출력된 것을 확인할 수 있다.
...
다음은 remove_thread_when_core_size_zero 메서드 수행 결과이다. 10개의 작업을 1개의 스레드가 수행하는 것을 확인할 수 있다.

- 작업을 수행하는 동안 스레드 풀 사이즈가 1이다.
- 50ms 뒤에 스레드 풀 사이즈가 0이다.
...
ForkJoinPool 클래스도 스레드 풀의 종류 중 하나이다. Executors 클래스를 사용하면 ForkJoinPool 인스턴스를 생성할 수 있다. 스레드 풀을 개선하기 위해 JDK 1.7에서 추가되었으며, 이번 글에서 함께 정리하기에는 내용이 많아 별도로 정리할 예정이다.
