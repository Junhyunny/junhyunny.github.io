---
title: "DTO 클래스, MultipartFile 활용 파일 업로드 (feat. @ModelAttribute 애너테이션)"
search: false
category:
  - spring-boot
  - vue.js
...
컨트롤러(controller) 클래스의 @RequestMapping 애너테이션이 붙은 메소드를 지원하는 애너테이션입니다. 
요청 파라미터나 반환 값을 명명된 모델 속성(model attribute)에 바인딩한다고 합니다. 
...
- 파라미터 앞에 `@ModelAttribute` 애너테이션을 붙힙니다.
...
이번 예제 코드에서는 N개의 파일을 리스트가 아닌 맵(Map)에 담아서 전달합니다. 
`FileUpload.vue` 파일은 프론트 엔드 프로젝트, 나머지 클래스 파일들은 백 엔드 프로젝트의 예제 코드입니다.
...
DTO 클래스 안에 파일을 N개 담아서 전달하는 예제 코드입니다. 
이번 예제 코드에서는 N개의 파일을 맵(Map)에 담아서 전달합니다. 
