...
람다식 내부에서 사용하는 변수는 `final`이거나 `effectively final`이어야 합니다. 
...
`Java` 컴파일러는 메소드 내 지역 변수가 람다식 내부에서 사용될 때 변경되길 바라지 않습니다. 
그렇기 때문에 람다식 내부에서 사용하는 지역 변수는 앞에 `final` 키워드를 붙혀서 변경되지 않음을 개발자가 보장하던지, `final` 키워드가 붙은 변수처럼 초기화 이후에 읽기만 가능하도록 컴파일러가 제어합니다. 
...
또, 병렬 처리를 수행하는 코드에서도 람다식이 많이 사용됩니다. 
...
특정 메소드 호출 시 스택 메모리에 올라간 지역 변수는 메소드 호출이 종료되면서 함께 삭제됩니다. 

이런 문제를 람다 캡처링(혹은 캡쳐링 람다)라는 개념이 적용됩니다. 
다른 스레드에 의해 제어될 수 있는 람다식은 상위 스코프의 지역 변수를 사용할 때 상위 스코프의 지역 변수가 메모리에서 제거되더라도 문제가 되지 않도록 자신의 스코프에 값을 그대로 복사하여 사용합니다. 
컴파일러는 람다식으로 인해 생성되는 익명 인스턴스(anonymous intsance)를 만들 때 자동으로 만들어진 생성자를 사용해 값을 복사합니다. 
...
인스턴스 변수는 힙(heap) 영역에 할당되고, 스태틱(static) 변수는 메소드(method) 영역에 선언되므로 값 복사가 불필요합니다. 
