...
실제 구현된 기능은 정상적으로 동작하지만, 테스트 코드가 실패하는 것이 문제였다. 문제가 발생한 원인을 탐색해보았다.

### 1.1. 테스트 시나리오

문제 현상을 살펴보기 전에 우선 테스트 시나리오를 살펴보겠다. 다음과 같은 시나리오로 테스트 코드를 작성하였다.
- useEffect 훅을 통해 화면 렌더링 시 필요한 데이터를 가져온다.
- 받은 데이터를 이용해 상태(state)를 변경하면 화면이 리-렌더링된다.
- 사용자 타이핑 이벤트를 통해 검색 문자열을 입력받는다.
- 리-렌더링된 화면에서 사용자가 타이핑한 문자열을 포함하는 데이터만 화면에 보인다.
...
테스트 코드를 동작 시 출력되는 에러 로그를 통해 텍스트 박스에 입력 문장의 맨 마지막 문자만 할당됨을 확인하였다. 현상의 원인을 확인하기 위해 많은 시간을 소요했지만, 뚜렷한 이유를 찾지는 못했다.

##### 에러 로그
- 텍스트 박스에 `value` 값을 보면 "o"만 할당되어 있다.
...
코드를 변경해봐도 테스트가 통과되지 않아 로그를 출력해보았다. 테스트 로그와 서비스 동작 로그의 차이점을 보고 문제 원인을 재정의할 수 있었다. 테스트가 실패한 원인은 다음과 같다.
- 테스트 코드에선 타이핑할 때마다 상태가 변경되었음에도 리-렌더링이 동작하지 않았다.
- 리-렌더링이 되지 않았기 때문에 텍스트 박스 `value` 속성에 타이핑하는 값들이 제대로 할당되지 않았다.
...
- 타이핑 이벤트 값을 보면 이전에 입력한 값들 뒤에 새로운 값이 추가되어 입력됨을 알 수 있다.
...
문제는 타이핑 이벤트에서 리-렌더링이 되지 않았기 때문이다. 어떤 코드가 이런 현상을 일으키는지 디버깅해보았다. 결과만 보면 `waitFor` 함수의 콜백 함수 내에서 타이핑 이벤트를 수행하였기 때문에 문제가 발생했다.

디버깅 모드로 콜 스택을 살펴보면 `waitFor` 함수와 `userEvent.type` 함수 내부에서 모두 `batchedUpdates` 함수를 호출한다. `batchedUpdates` 함수 내부 `try-finally` 구문의 `finally` 블록에서 리-렌더링을 수행하기 위한 함수를 호출하는데, 현재 실행 중인 컨텍스트가 `NoContext` 이어야 리-렌더링을 수행한다. 함수 내부와 콜 스택을 살펴보겠다.

##### react-dom.development.js 파일 batchedUpdates 함수
- `executionContext`는 `react-dom.development` 모듈의 전역 변수이다.
- 임시 변수에 담아 두었던 `executionContext`가 `NoContext` 이어야 리-렌더링을 수행한다.
...
##### 디버깅 콜 스택
- `waitFor` 콜 스택에서 `batchedUpdates` 함수를 호출한다.
  - 이 시점에 `react-dom.development` 모듈의 `executionContext` 값이 `BatchedContext` 상태로 변경된다.
- `userEvent.type` 콜 스택에서 `batchedUpdates` 함수를 호출한다.
  ...
  - 타이핑 이벤트에 대한 리-렌더링 작업들이 수행되지 않는다.
- `waitFor` 콜 스택 `batchedUpdates` 함수의 `finally` 블록에서 `executionContext === NoContext` 조건이 만족된다.
- 리-렌더링을 1회 수행한다.
...
문제의 원인을 알고나니 해결할 방법이 바로 떠올랐다.

### 3.1. 렌더링 대기하기

`axios` 모듈에 스터빙한 데이터가 화면에 렌더링되었는지 확인 후 타이핑 이벤트를 처리한다. 이 방법은 문제를 해결할 수는 있지만, 좋지 않은 방법이라고 생각하였다. 테스트 코드를 통해 확인하고 싶은 내용은 `"사용자가 입력한 값으로 필터링된 데이터만 화면에 출력되는가?"`인데, 중간에 `expect` 함수를 호출하기 때문에 의미가 모호해진다.

##### 테스트 코드
- `axios` 모듈에 스터빙한 값이 화면에 보이는지 확인한다.
- 타이핑 이벤트를 처리한다.
- 필터링된 값들만 보이는지 확인한다.
- `expect` 구문이 두 개가 되면서 `setup > act > verify` 구분이 모호해진다.
...
필자가 생각하는 BP(best practice)이다. `find-` 구문으로 시작하는 함수를 사용하면 비동기적인 동작을 대기한다.

##### 테스트 코드
- `findByPlaceholderText` 함수를 사용한다.
  - 원하는 엘리먼트(element)를 찾을 때까지 대기한다.
- `expect` 구문이 한 개이므로 `setup > act > verify` 구분이 명확해진다.
...
### 3.3. 테스트 결과
- 최초 에러가 발생한 테스트와 새로 만든 두 개의 테스트의 실행 결과이다.
...
리-렌더링을 유발하는 이벤트를 `waitFor` 함수 내부에서 수행할 경우 예상하지 못한 결과를 얻을 수 있다. 디버깅을 하면서 몇 가지 새로운 개념에 대해 배울 수 있었기에 유익한 삽질이었다고 생각된다.
