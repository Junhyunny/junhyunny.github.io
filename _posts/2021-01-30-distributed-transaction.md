...
* 메세지 중복 전달이나 수신을 막기 위한 멱등성(idempotence)
...
분산 트랜잭션을 다시 정리해보면 어떤 비즈니스 프로세스가 네트워크를 통해 연결된 분산 환경에 놓인 시스템(혹은 서비스)들을 두루 걸쳐 실행되는 것입니다. 
각 서비스들은 로컬 트랜잭션을 수행하며, 비즈니스 관점에서 바라보았을 때 분산 트랜잭션에 연관된 서비스들이 일으킨 로컬 트랜잭션들은 의미있는 연관 관계를 가집니다. 
...
1. 로직 중간에 알 수 없는 에러가 발생하면서 차감된 크레딧 정보는 롤백(rollback)되어 이전 상태로 돌아갑니다.
...
사가 컴포넌트는 자신이 책임지고 있는 로직을 수행하고 다음 참가자에게 메세지를 전달합니다. 
다른 사가 패턴 참가자의 메세지 응답에 따라 필요한 다음 처리를 선택적으로 수행합니다. 
...
1. 주문 사가 컴포넌트는 메세지 브로커를 통해 크레딧 생성을 메세지를 전달합니다.
    1. 주문이 생성되었음을 클라이언트에게 응답합니다.
1. 크레딧 서비스의 커맨드 핸들러가 메세지를 받아 크레딧 예약을 요청합니다.
1. 크레딧 서비스는 주문한 금액만큼 크레딧을 예약합니다.
1. 크레딧 서비스는 크레딧 예약 성공 여부를 메세지로 전달합니다.
...
* 메세지 혹은 이벤트의 중복 송수신을 없애야 합니다.
    * 중복된 메세지, 이벤트 송수신이 발생하는 경우 잘못된 데이터 처리가 발생하기 때문에 이에 대한 처리를 고민해야합니다.
    * Kafka 같은 솔루션을 사용하는 경우 `Idempotence Publisher`를 사용합니다.
    * 크리스 리차든의 오케스트레이션 사가 예제 코드는 데이터베이스를 사용한 `Transactional Outbox Pattern`을 적용하였습니다.
* 메세지 혹은 이벤트 재송신(retry)이 가능해야 합니다.
    * 불안정한 인프라 상황에 메세지나 이벤트가 누락되는 경우가 발생합니다.
    * 누락된 메세지, 이벤트를 재송신 할 수 있어야 합니다. 
* 상황에 맞는 사가 패턴을 적용합니다.
