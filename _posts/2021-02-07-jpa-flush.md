...
## 2. Flush Mode Options and JPQL(Java Persistence Query Language)

### 2.1. Flush Mode Options
...
`JPQL`은 테이블을 대상으로 질의를 하는 것이 아닌 엔티티 객체를 대상으로 질의문을 작성합니다. 
그렇기 때문에 일반적인 `SQL` 문법과 조금 다른 요소를 가집니다. 
