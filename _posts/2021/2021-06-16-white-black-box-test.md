...
화이트 박스 테스트(white box test)는 내부 구조나 애플리케이션 동작 방식에 대해 테스트를 수행한다. 다음과 같이 정리할 수 있다.

- 모듈의 정상적인 작동을 코드 레벨에서 확인한다.
  - 컴파일 된 원시 코드의 문장을 여러번 수행한다.
...
블랙 박스 테스트(black box test)는 내부 구조나 동작 방식에 대해 확인하지 않고 애플리케이션의 기능을 테스트한다. 다음과 같이 정리할 수 있다.

- 요구사항 명세서를 보면서 구현된 기능을 테스트한다.
  - 테스트를 진행하는 사람은 모듈 내부에서 어떤 동작이 있었는지 신경쓰지 않는다.
...
- 한계값 분석(경계값 분석, Boundary Value analysis)
  - 입력값의 경계값을 중심으로 예외 발생 검사한다.
- 원인-결과 그래프(Cause-effect Graphing)
  - 입력 데이터 간의 관계, 출력에 미치는 영향의 분석 그래프 이용한다.
