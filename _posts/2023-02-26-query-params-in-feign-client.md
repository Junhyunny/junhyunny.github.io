...
`FeignClient`를 사용하면서 다음과 같은 장점들을 느낍니다. 

* 애너테이션과 인터페이스 선언만으로 API 요청을 수행할 수 있는 객체가 생성됩니다.
...
이번 포스트에선 쿼리 파라미터 수가 늘어나는 경우 DTO(data transfer object) 객체로 대체하는 방법을 정리하였습니다. 
`FeignClient`의 사용법이나 테스트 방법은 이번 포스트에서 자세히 다루지 않으므로 이전 포스트를 참고하시길 바랍니다. 

## 1. @RequestParam Annotation

일반적으로 URL 쿼리를 만들 때 사용하는 애너테이션입니다. 
`FeignClient`를 구현한 인터페이스와 테스트 코드를 통해 사용 방법을 살펴보겠습니다.
...
* getBlogResponsesWithParams 메소드
    ...
    * `?name=jun&age=20&address=Seoul` 쿼리문이 포함된 요청이 생성될 것을 예상합니다.
...
* GET 메소드 요청이 수행된 것을 확인합니다.
* 요청 URL 값이 `http://localhost:11962/search?name=jun&age=20&address=Seoul`임을 확인합니다.
...
이를 사용하는 클라이언트 코드는 파라미터 위치가 헷갈려 파라미터 값을 정확한 위치에 넣지 못할 수 있습니다. 

* 예시 코드는 사람 이름이 3개나 들어갑니다.
...
이런 패인 포인트를 개선하기 위해 `@SpringQueryMap` 애너테이션을 사용할 수 있습니다. 
...
이전 단계에서 수행한 테스트와 동일합니다. 
다만 요청 시 `BlogQuery` 객체를 사용합니다. 

* `action.in.blog.client` 패키지에서 발생하는 디버그(debug) 로깅(logging) 레벨을 활성화합니다.
* `FeignClient` 요청 시 발생하는 로그를 모두 보기 위해 로거 레벨을 `FULL` 레벨로 지정합니다.
* `FeignClient` 요청 시 빌더 패턴(builder pattern)으로 `BlogQuery` 객체를 만듭니다.
    * URL 요청을 만들 때 어떤 키에 어떤 값이 매칭되는지 확인하기 좋습니다.
* `FeignClient` 메소드에 파라미터가 1개로 더 이상 늘어날 필요가 없습니다.
* `WireMock` 객체를 사용해 가상 서버를 구축하여 테스트를 수행합니다.
    * 스터빙(stubbing)한 값에 맞는 요청이 들어왔는지 확인합니다.
    * 적절한 요청을 받은 경우 스터빙한 응답을 반환합니다.
* `FeignClient` 응답을 통해 받은 데이터를 검증합니다.
