...
[How to save image into PostgreSQL with Spring][how-to-save-image-into-database-with-spring-link] 포스트에선 이미지를 데이터베이스 BLOB 형태로 저장하는 방법에 대해 정리했다. 이번 포스트에선 BLOB으로 저장된 이미지를 브라우저에 표시하는 방법에 대해 정리하였다. 실습을 위한 개발 환경은 다음과 같다.
...
이번 포스트는 브라우저에서 이미지를 다운로드 받는 방법에 대해서만 집중적으로 다룬다. 이미지를 업로드하는 기능이나 데이터베이스에 저장하는 방법은 이전 글들을 참고하길 바란다. 이미지 반환 작업은 두 단계를 거쳐 이뤄진다. 서버가 이미지 바이너리(binary)를 클라이언트로 그대로 내려주고 JavaScript로 이를 처리하는 방법도 있지만, 코드 복잡성을 줄이고 캐시 컨트롤(cache control) 적용을 위해 리소스 URL을 전달하는 방식을 사용했다.

1. 클라이언트는 이미지를 업로드한다.
2. 서버는 이미지 리소스 URL 경로를 응답한다.
3. 클라이언트는 `img` 태그에서 `src` 속성으로 이미지 리소스 URL 경로를 참조한다.
4. 서버는 해당 URL 정보를 바탕으로 데이터베이스에서 바이너리 데이터를 조회 후 반환한다.
...
파일을 업로드하고 데이터베이스에 이미지 바이너리 데이터를 저장하는 기능은 이미 구현되어 있다고 가정한다. 해당 저장된 데이터를 기반으로 이미지 리소스 URL 정보를 만들어 반환한다. 전달받은 URL 정보를 바탕으로 데이터베이스를 조회한다.

### 1.1. FileEntity Class

엔티티 클래스는 다음과 같은 모습을 가진다.

- 바이너리 데이터를 저장할 수 있는 엔티티 클래스이다.
  - 바이너리 데이터는 바이트 배열 형태로 저장한다.
  - 데이터베이스마다 동작이 다르므로 추가적으로 필요한 컬럼 정보를 추가한다.
- resourceLocation 메서드
  - 메서드 파라미터로 전달받은 호스트 이름을 기준으로 리소스 위치 URL을 만들어 반환한다.
  - 엔티티 ID, 이름이 사용된다.
...
- `hostname` 변수 값은 application.yml 파일 설정으로 지정한다.
  - 이번 포스트에서 지정된 값은 `http://localhost:8080`이다.
- 업로드된 파일을 데이터베이스에 바이너리 형식으로 저장한다.
- 저장된 엔티티 정보를 기준으로 이미지 리소스 URL 위치를 만들어 반환한다.
...
간단하게 타임리프(thymeleaf)를 사용해 페이지를 구성하였다. JavaScript 코드에 대한 설명은 가독성을 위해 주석으로 작성하였다.
...
이미지 태그의 `src` 속성에 값이 설정되면 브라우저는 서버로 리소스 요청을 보낸다. 상대 주소라면 해당 페이지를 받은 서버로 보내고, 절대 주소라면 해당 서버로 요청을 보낸다. 서버는 클라이언트가 이미지를 다운로드 받을 수 있도록 엔드-포인트(end-point)를 준비한다. 이번 예제에선 getImage 메서드를 만들어 이미지를 클라이언트에게 내려준다.

- 경로 변수(path variable)로 받은 아이디로 파일 정보를 데이터베이스에서 조회한다.
  - 이 예제에서 파일 이름은 사용되지 않지만, Content-Disposition 헤더에 사용하는 등의 용도로 사용할 수 있다.
- 데이터베이스에서 조회한 정보를 기반으로 다음과 같은 응답 데이터를 반환한다.
  - 컨텐츠 타입을 설정한다.
  - 헤더에 캐시 컨트롤을 추가한다. URL 값이 변경되지 않는다면 `max-age`로 지정한 시간 동안 이미지를 캐시로 사용한다. 단위는 초(second)이다.
  - 바이너리 데이터는 ByteArrayResource 인스턴스에 담아서 반환한다.
...
요즘 응답 데이터는 보통 JSON 형식을 사용하기 때문에 컨트롤러 테스트에서 `jsonPath` 메서드를 많이 사용한다. 바이트 배열 형식의 데이터 반환 여부를 테스트하기 위해선 `MvcResult` 클래스를 사용하거나 `content` 메서드를 사용한다. FileRepository 인스턴스를 테스트 더블(test double)로 사용한다.
...
애플리케이션을 실행 후 `http://localhost:8080`에 접속하면 다음처럼 동작하는 모습을 확인할 수 있다.
