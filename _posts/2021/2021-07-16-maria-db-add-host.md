...
이번 포스트에서는 Maria DB 서버에 새로운 사용자 정보를 등록하는 방법과 외부에서 접속하는데 발생했던 문제를 해결한 방법을 공유해보겠다.
...
`flush privileges` 명령어를 통해 변경된 사용자 정보를 반영한다.
...
변경된 내용이 잘 반영되었는지 추가된 사용자로 접속을 해본다.
...
설치한 데이터베이스에 접근을 시도하면 다음과 같은 에러가 발생한다. host 정보를 `'%'`로 지정하여 모든 IP에서 접근을 허용하였는데 이상하다.
...
`netstat -lntp` 명령어를 통해 문제의 원인을 확인할 수 있다. 3306 포트(port) 번호를 가지는 프로세스 정보를 보면 Local Address가 `127.0.0.1` IP 주소를 가진다. Local Address IP 주소가 `0.0.0.0`이라면 모든 인터페이스를 허용하겠다는 의미이며, `127.0.0.1`이라면 자기 자신만 호출이 가능한 상태이다. 현재는 IP 주소가 `127.0.0.1`이므로 로컬 호스트만 접근이 가능한 상태이다.
...
모든 IP에 대해 접근을 허가하려면 설정 파일을 변경해줘야 한다.
...
서비스를 재시작 후 `netstat -lntp` 명령어를 통해 IP 정보를 다시 확인한다.
