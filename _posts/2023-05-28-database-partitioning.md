...
이번 포스트는 파티셔닝 컨셉에 대해서만 정리하였습니다.

## 1. Partitioning

> 파티션(partition)은 테이블을 논리적으로는 하나의 테이블이지만, 물리적으로는 여러 개의 테이블로 분리하여 관리하는 기능

데이터를 여러 개의 테이블로 분리하여 관리하는 파티션 기법을 적용하는 것을 파티셔닝입니다. 
주로 대용량의 테이블을 물리적으로 여러 개의 소규모 테이블로 분산하는 목적으로 사용합니다. 
...
* 데이터 특성상 주기적으로 삭제 작업이 필요한 경우
    * 로그라는 이력 데이터는 단기간에 대량으로 누적되고 일정 기간이 지나면 쓸모가 없어집니다.
    * 시간이 지나면 별도로 아카이빙하거나 백업한 후 삭제합니다.
