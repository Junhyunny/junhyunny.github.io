...
`공격 벡터`는 취약점 악용이 가능한 접근 경로(물리적, 논리적 거리)를 평가한다. 원격에서 공격이 가능할수록 더 많은 잠재적 공격자에게 노출되므로 위험도가 높아진다.
