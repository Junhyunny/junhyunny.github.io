...
세션은 다양한 기술을 통해 구성될 수 있다. 서버 고가용성, 성능, 시스템 요구 사항에 따라 적절한 기술을 사용한다.
...
- 서비스를 위한 기타 정보
  - 시스템에서 사용하는 코드 데이터
  - 사용자 인터렉션(interaction)에 의해 발생한 임시 변경 사항
...
- `이름`은 각각의 쿠키를 구별하는 데 사용된다.
- `값`은 쿠키의 이름에 매칭되는 정보이다.
- `유효시간`은 쿠키의 유지 시간이다.
- `도메인`은 쿠키를 전송할 도메인 주소를 의미한다.
- `경로`는 쿠키가 유효한 리소스 경로 정보를 의미한다.
