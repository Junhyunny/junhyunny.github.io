...
Java는 멀티 스레드 환경에서 스레드 안전(thread-safe)를 위해 `Atomic` 키워드가 붙은 클래스를 제공한다. java.util.concurrent.atomic 패키지에 위치한다. `Atomic`이라는 키워드가 붙은 것처럼 원자적 연산(atomic operation)이라는 개념과 연관있어 보인다. 이 개념에 대해 먼저 살펴보겠다.

## 1. Atomic Operation

> 원자성(atomicity) - 어떤 것이 더 이상 쪼개질 수 없는 성질을 말한다.

원자적인 연산은 "더는 쪼개질 수 없는 연산"을 의미한다. 코드가 한 줄이더라도 원자적인 연산은 아니다. 가장 대표적인 예시가 단항 연산자 `++` 이다.

##### ++ 단항 연산자

- 코드 한 줄이더라도 기계어로 변경되면 원자적이지 않은 연산이 될 수 있다.
- 현상의 원인은 프로세스 내 스레드들이 변수를 사용해 데이터를 공유하기 때문이다.
...
Java는 동시성 문제를 해결하기 위한 기능들을 제공한다. 그 중 하나가 `Atomic` 클래스들이며 이번 포스트에서는 대표적인 AtomicInteger 클래스를 살펴보겠다.
...
`Atomic` 클래스가 어떻게 원자성을 보장하는지 이해하려면 CAS(compare and swap) 메커니즘에 대해 이해해야 한다. 위키(wiki)는 다음과 같이 정의하고 있다.
...
단순하게 설명하면 다음과 같이 동작한다.

- 이전 값(old value)과 새로운 값(new value)을 전달한다.
- 이전 값과 현재 메모리에 저장된 데이터가 같은 경우에 새로운 값으로 메모리의 데이터를 변경한다.
- 성공 여부에 따라 true, false 값을 반환한다.

`Atomic` 클래스들은 compareAndSet 이라는 메서드를 사용한다. 이름에서 볼 수 있듯이 CAS 메커니즘을 구현했을 것이라 예상할 수 있다. AtomicInteger 클래스를 기준으로 내부 메서드를 탐험해보겠다.

#### 2.1.1. AtomicInteger Class

- getAndSet 메서드는 이전 값을 반환하고 새로운 값을 메모리에 업데이트한다.
- compareAndSet 메서드는 이전 값을 새로운 값으로 변경하고 성공 여부를 반환한다.
  - 메모리에 저장된 값과 expectedValue 변수의 값이 같은 경우에만 수행 완료된다.
- 두 메서드 모두 Unsafe 객체에게 동작을 위임한다.
...
### 2.2. Performance Between synchronized keyword and Atomic classes

Java는 동시성을 제어하기 위해 synchronized 키워드를 제공한다. 이미 동시성 제어 기능이 있음에도 `Atomic` 클래스들을 제공하는 이유는 성능 때문이라는 글을 읽은 기억이 난다. 관련된 내용을 직접 구현을 통해 살펴보겠다.
...
- NormalInteger 클래스를 생성한다.
- CompletableFuture 클래스를 사용해 두 개의 스레드를 경합시킨다.
  - 한 스레드는 값을 증가, 한 스레드는 값을 감소시킨다.
- 총 소요되는 시간과 최종 값을 확인한다.
...
- 총 소요되는 시간은 16393 이다.
- 최종 값은 0으로 정상적인 동기화가 이뤄졌음을 확인할 수 있다.
...
- 총 소요되는 시간은 8862 이다.
  - synchronized 키워드를 사용했을 때보다 2배 정도 빠름을 확인할 수 있다.
- 최종 값은 0으로 정상적인 동기화가 이뤄졌음을 확인할 수 있다.
