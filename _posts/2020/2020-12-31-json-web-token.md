...
헤더와 페이로드의 데이터 무결성과 변조 방지를 위한 서명 정보다. 서명은 헤더 정보의 인코딩 값과 페이로드 정보의 인코딩 값을 합친 후 비밀 키로 해싱(hashing)하여 생성한 값을 사용한다. 서버에서 서명 정보를 비교해 위조된 토큰인지 아닌지 여부를 판단할 수 있다.
...
<https://jwt.io/> 사이트에서 헤더, 페이로드, 시그너처 정보를 추가하면 JWT 객체를 생성할 수 있다. 반대로 JWT 객체가 있다면 이를 파싱해 볼 수 있다. 내부에 어떤 값이 들었는지 확인할 때 유용하다.
...
2. 토큰 요청을 받은 인증 서버는 사용자 계정을 확인하고 토큰 지급이 가능한지 여부를 판정 후 토큰을 발급한다.
3. 인증 서버는 사용자 정보를 Json 데이터로 만들고, Base64 인코딩을 수행한다. 서명은 암호화하여 토큰을 생성한다.
  - 이 과정에서 만들어진 토큰은 일반 방식과 다르게 스토리지에 저장하지 않는다.
4. 인증 서버는 클라이언트에게 토큰을 전달한다.
5. 클라이언트는 토큰과 함께 리소스 서버에 API 요청을 수행한다.
6. 리소스 서버는 토큰이 정상적인지 확인한다. JWT의 서명이 정상적인지 확인한다. 이때 필요하다면 인증 서버와 협력한다. 이후 사용자의 권한 정보 등을 확인 후 응답한다. 
