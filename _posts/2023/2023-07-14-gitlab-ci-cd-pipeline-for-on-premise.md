...
최근 시작한 프로젝트는 코드 저장소로 GitLab을 사용했다. 별도의 자원 없이 효율적인 CI/CD 파이프라인을 구축하기 위해 GitLab에서 제공하는 파이프라인 기능을 구축한 내용에 대해 정리하였다.

## 1. GitLab CI/CD Pipeline

GitLab CI/CD 파이프라인에서 사용되는 용어들을 정리하였다.
...
- 정상적으로 수행되었는지 서버 ~/.ssh 경로에 authorized_keys 파일을 통해 확인할 수 있다.
...
- 화면 왼쪽에 `Settings` 카테고리에서 CI/CD 탭을 선택한다.
...
- ssh 작업을 위해 이미지 내부에서 `openssh-client` 패키지를 설치한다.
- GitLab 변수로 등록한 파일을 복사한다.
  - $SECRET_ACCESS_KEY - 비공개 키
  - $KNOWN_HOST - known_hosts
- ssh 접속을 위해선 각 파일과 폴더의 권한이 올바라야 한다.
- 패키지 파일의 이름을 ROOT.war로 변경한다.
- ROOT.war 파일을 온-프레미스 서버에 배포한다.
...
실제 개발 중인 프로젝트에 적용된 파이프라인을 기준으로 설명했기 때문에 별도로 테스트 코드 업데이트는 불가능하다.
