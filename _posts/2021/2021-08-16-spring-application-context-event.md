...
다른 비즈니스 도메인의 의존성이 침투하면 서로 결합도(coupling)이 강해지기 시작한다. 깨진 창문 효과에 따라 한번 침투하기 시작한 의존성은 시간이 지날수록 두 도메인 사이의 결합도를 강하게 연결하게 될 것이다. 시스템이 커지게 되어 비즈니스 별로 서비스를 분리해야하는 시점이 오면 빠른 변화를 만들기 힘들게 된다. 두 비즈니스 사이에 의존성을 만들지 않을 방법이 필요했다.

## 1. Spring Application Context Event

스프링에서 기본으로 제공하는 기능이다. 커스텀 이벤트(custom event)를 정의하고, 이를 발행(publish), 구독(subscribe)할 수 있다. 애플리케이션 컨텍스트(application context)를 사용해 다른 빈(bean)들 사이의 정보를 교환할 수 있다. 이벤트 처리는 두 컴포넌트(component) 사이의 결합도를 낮출 수 있다. 여기서 말하는 컴포넌트는 모듈(module)나 서비스 등이 될 수 있다.

##### 모듈 사이의 이벤트 처리

- 하나의 서비스 내에 분리된 모듈 사이의 정보 교환이 이뤄진다.
- 동일 서비스이므로 애플리케이션 컨텍스트를 통해 이벤트 처리를 수행한다.
...
이번 포스트에선 커스텀 이벤트를 요청하고 받는 방법에 대한 예시를 다룰 예정이지만, 기본적으로 어떤 이벤트들이 있는지 살펴보겠다. 기본 이벤트를 사용하면 개발자는 자신의 기능을 애플리케이션의 라이프사이클(lifecycle)에 연결할 수 있다.

- ContextRefreshedEvent
  - 애플리케이션 컨텍스트를 초기화하거나 리프레시(refresh)할 때 발행된다.
- ContextStartedEvent
  - 전형적으로 명시적인 정지 이후 빈들을 재실행할 때 발행된다.
- ContextStoppedEvent
...
두 모듈 사이의 정보를 커스텀 이벤트로 주고 받는 간단한 예시 코드를 살펴보겠다. 배달이나 주문과 관련된 비즈니스 도메인에 대해선 잘 모르지만, 정말 간단한 프로세스를 예시로 다뤄보았다.

1. 배달 정보가 "배달 완료" 상태로 업데이트된다.
1. 배달 모듈에서 "배달 완료" 이벤트를 발행한다.
1. 주문 모듈에서 "배달 완료" 이벤트를 전달 받는다.
1. 주문 모듈은 해당 주문 상태를 "배달 완료" 상태로 변경한다.
...
- base 패키지
  - `DeliveryCompleteEvent`는 정보를 주고 받을 때 사용하는 이벤트 객체이다.
- delivery 패키지
  - DeliveryService 객체를 통해 배달 정보를 변경한다.
  - DeliveryEventProxy 구현체를 통해 order 패키지와 정보를 주고 받는다.
- order 패키지
  - OrderEventListener 객체를 통해 delivery 패키지와 정보를 주고 받는다.
...
- finishDelivery 메서드는 배달 상태를 완료로 변경한다.
...
- 배달 정보를 조회 후 배달 완료 상태로 변경한다.
- 프록시(proxy) 객체를 통해 배달 완료 이벤트를 발행한다.
  - `DeliveryEventProxy` 인터페이스를 통해 구현 상세 정보를 차단한다.
...
- `DeliveryEventProxy` 인터페이스를 구현한다.
  - 클라이언트에게 배달 완료 이벤트 발행에 대한 서비스를 제공하지만, 실제 구현은 숨긴다.
  - 현재는 스프링 애플리케이션 컨텍스트를 사용하지만, 향 후에는 다른 기술을 사용하여 이벤트를 발행할 수 있다.
- 배달 완료 이벤트를 발행한다.
...
- startDelivery 메서드는 주문의 배달 상태를 시작으로 변경한다.
- finishDelivery 메서드는 주문의 배달 상태를 완료로 변경한다.
...
- 이벤트를 수신하면 주문 서비스에게 배달 정보를 전달한다.
...
- 신규 배달 정보를 생성한다.
  - 배달 상태는 "시작"이다.
- 해당 배달의 상태를 "완료"시킨다.
- 해당 배달과 연관된 주문 정보를 조회한다.
  - 주문의 배달 상태가 "배달 완료"인지 확인한다.
...
주문, 배달 관련된 도메인을 직접 경험해보지는 않아서 테스트 시나리오가 좋지 않을 수 있다. 해당 포스트를 작성하면서 생긴 궁금한 주제들로 다음 포스트를 정리할 생각이다.
