...
리액트 애플리케이션 배포 시 개발, 운영 환경 별로 다르게 동작하도록 변수 설정이 필요했습니다. 
CI/CD 파이프라인의 젠킨스 파일을 직접 수정할 수 없다는 제약 사항이 있었지만, 
각 배포 환경 별로 사용하는 도커 파일(Dockerfile)이 다른 점을 이용하여 배포 환경에 맞는 환경 변수를 설정하였습니다. 
이번 포스트에선 배포 환경 별로 도커 파일을 사용하여 리액트 애플리케이션에 환경 변수를 설정하는 방법에 대해 정리하였습니다. 
...
우선 도커 파일 별로 애플리케이션에서 사용할 변수를 다르게 설정합니다.
...
* `localhost:8080` 접속 시 기본 리액트 애플리케이션 화면이 보입니다.
