---
title: "메이븐(Maven) 프로젝트 GitLab CI/CD 파이프라인 최적화"
search: false
category:
  - gitlab-ci
  - maven
...
- [온-프레미스 환경을 위한 GitLab CI/CD 파이프라인][gitlab-ci-cd-pipeline-for-on-premise-link]
...
- 각 잡마다 원격 레포지토리에서 필요한 의존성을 다운로드 받는 작업이 발생한다. 각 잡이 10초 이상씩 수행된다.
...
파이프라인 로그를 살펴보면 다음과 같은 경고를 볼 수 있다. 

- processPath: artifact path is not a subpath of project directory: /root/.m2/repository
...
깃랩 CI는 `$CI_PROJECT_DIR` 디렉토리 내부에서만 아티팩트(artifact)를 찾을 수 있다. 프로젝트 외부에 캐시를 둘 수 없다. `$CI_PROJECT_DIR`는 깃랩 CI에서 예약된 변수로 프로젝트 경로를 의미한다. 이 문제를 해결하기 위해 다음과 같이 `.gitlab-ci.yml` 스크립트를 변경한다.

- `$CI_PROJECT_DIR/.m2/repository` 경로를 캐싱한다.
- `MAVEN_OPTS` 옵션을 통해 로컬 레포지토리 경로를 `$CI_PROJECT_DIR/.m2/repository`로 설정한다. `MAVEN_OPTS` 환경 변수는 메이븐 빌드 시 사용되므로 선언만으로도 적용된다.
...
파이프라인 스크립트를 변경 후 애플리케이션 코드를 푸시하면 다음과 같이 파이프라인이 동작한다. 각 잡 별로 캐시 경로를 공유할 뿐만 아니라 매 파이프라인마다 다운로드 받던 의존성을 다시 받지 않는다. 또한 캐시가 파이프라인마다 공유된다.
