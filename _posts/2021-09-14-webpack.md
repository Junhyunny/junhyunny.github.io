...
- `root` ID를 가진 div 태그 안의 HTML을 `message`로 지정합니다.
...
#### 1.1.2. 모듈을 사용한 코드
위와 같은 현상을 전역 스코프(scope)가 오염되었다고 표현합니다. 
...
- `root` ID를 가진 div 태그 안의 HTML을 `hello.message + world.message`로 지정합니다.
...
## 2. Webpack 사용
이번 포스트는 Webpack에 대한 간단한 정리하는 글이기 때문에 매우 불편한 방법으로 `Webpack`을 사용하였습니다. 
다음 기회에 설정 파일을 이용하여 더 쉬운 방법으로 `Webpack`을 사용하는 방법을 작성할 예정입니다. 

이제부터 작성한 모듈들을 `Webpack`을 사용한데 한데 모아주겠습니다. 
이런 행위를 번들링(bundling)이라고 합니다. 

### 2.1. Webpack 설치하기
우선 사용하기 위해선 설치를 해야합니다. 
...
Java 언어의 클래스에 `public static void main(String[] args)` 메소드가 필요한 것과 같은 느낌이 듭니다. 
이를 엔트리 포인트라고 합니다. 
...
### 2.3. CLI(Command Line Interface)를 사용한 번들링
...
다음과 같은 명령어를 수행합니다.
- 엔트리 포인트는 index.js 파일로 지정합니다.
- 번들링한 파일은 현재 디렉토리에 만듭니다.
...
Webpack Document를 다짜고짜 읽기만 했을 때는 안 보였던 것들이 이제는 조금 더 이해가 되기 시작했습니다. 
