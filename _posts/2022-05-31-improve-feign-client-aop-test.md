...
그런 관점에서 봤을 때 처음 작성한 테스트 코드는 상당히 네트워크 상황에 따라 결과가 달리질 수 있었고, 실제 데이터베이스에 삽입되는 데이터 값에 대한 검증도 이뤄지지 않았습니다. 
...
실제 네트워크 호출에 대한 의존성을 제거한 것에 타협하고 남은 테스트를 작성하였지만, 참을 수 없는 문제점을 하나 발견했습니다. 
메소드 위 요청 방법을 결정하는 애너테이션들은 요청 경로 값을 넣는 방법에 따라 다른 속성에 값이 저장됩니다. 
