...
최근 레디스(redis)를 탐구하면서 캐시(cache)를 활용한 전략에 대해 관심이 생겼습니다. 
이에 캐싱 전략을 효과적으로 펼칠 수 있는 캐시 패턴과 관련된 내용을 포스트로 정리하였습니다. 
...
* 캐시 조회(cache lookup)와 캐시 쓰기(cache write)가 캐시 서버를 통해 이뤄집니다.
...
* 클라이언트(client) 어플리케이션은 캐시에서 데이터를 확인합니다.
* 캐시에서 데이터가 없음을 인지하면, 데이터 저장소로부터 데이터를 조회 후 캐시를 최신화합니다.
* 캐시를 최신화한 이후에 클라이언트에게 데이터를 응답합니다.
...
* 캐시 제공자(provider)가 데이터 저장소에 데이터를 저장하는 책임을 가집니다.
* 클라이언트는 캐시 서버에 데이터를 저장하거나 변경합니다.
...
* 캐시과 데이터 저장소 사이에 별도 통신은 수행하지 않습니다.
...
* 클라이언트는 어플리케이션에게 원하는 데이터를 요청합니다.
* 어플리케이션은 우선 캐시를 조회합니다.
    * 데이터가 존재하면 이를 반환합니다.
* 어플리케이션은 원하는 데이터가 존재하지 않으면 데이터 저장소에서 이를 조회합니다.
* 어플리케이션은 조회한 데이터를 캐시 서버에 저장합니다.
* 어플리케이션은 조회한 데이터를 클라이언트에게 반환합니다.
