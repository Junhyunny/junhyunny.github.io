---
title: "부동 소수점과 자바(Java) 숫자 계산 문제점"
search: false
category:
  - java
...
올 여름에 읽었던 [이펙티브 자바 3/E][effective-java-book-link]에서 정확한 숫자 연산과 관련된 주의사항이 최근에 도움을 주었다. 금융 관련된 계산에 특히 주의하라는 조언이 있었는데, 마침 금융 프로젝트를 진행하게 되면서 잘못된 설계를 피해갈 수 있었다. 이번 글에서는 `Java`에서 정확한 숫자 연산과 관련된 내용을 간략하게 정리해 보려 한다.

## 1. Binary Number

0과 1만으로 값을 표현하는 컴퓨터는 숫자를 이진법으로 표현한다. 예를 들어 숫자 `21`을 이진수로 표현하면 `10101`가 된다.

> 10101 = (2^4 * 1) + (2^3 * 0) + (2^2 * 1) + (2^1 * 0) + (2^0 * 1)

실수 표현은 헷갈릴 수 있는데, 원리는 동일하다. 예를 들어 숫자 0.625를 이진수로 표현하는 방법은 다음과 같다.

- 0.625에 2를 곱하는 연산을 수행하여 1.25 값을 얻는다.
- 이전 연산의 소수부인 0.25에 2를 곱하는 연산을 수행하여 0.5 값을 얻는다.
- 이전 연산의 소수부인 0.5에 2를 곱하는 연산을 수행하여 1.0 값을 얻는다.
- 이전 연산의 소수부는 0이므로 연산을 멈춘다.
- 각 연산에서 얻어진 정수부(자리 올림수) 1, 0, 1을 순서대로 나열하면 소수부 이진수 표현이 된다.
- 숫자 `0.625`는 이진수로 표현하면 `0.101`이 된다.
...
컴퓨터는 자신의 연산 비트들을 갖고 최대한 다양한 값들을 표현하고자 다음과 같은 방법을 사용한다. 컴퓨터가 실수를 표현하는 방법 중 고정 소수점(fixed point) 방식에 대해 먼저 알아보자. 32비트 기준으로 고정 소수점 방식에 대해 정리해 보았다.

- 부호(1비트), 정수부(15비트)와 소수부(16비트)로 나눈다. 
- 정수부는 일반적인 이진수 표기법을 따른다.
- 소수부는 다음과 같은 규칙에 따라 표현된다.
- 숫자 7.625는 32비트 고정 소수점 표현으로 0 000000000000111 1010000000000000 이다.
  - 부호 0
  - 정수부 111
  - 실수부 101
  - 나머지는 0으로 패딩(padding)
...
고정 소수점 방식은 표현할 수 있는 수의 범위가 좁기 때문에 컴퓨터는 실제로 부동 소수점(floating point) 방식을 사용한다. 고정 소수점 방식과 다르게 소수점이 고정되어 있지 않다. 지수부(exponent)와 가수부(mantissa)를 만들고, 가수부의 소수점을 정규화를 통해 이동시킨다. 예를 들어 7.625를 이진수로 표현한 111.101을 정규화하면 다음과 같다.

> 1.11101 * 2^2

부동 소수점 방식은 가수부와 지수부를 사용해 실수를 표현하는 방법이다. 가수, 지수라는 용어가 익숙하지 않을 수 있으니 간단하게 정리해 보자.

- 예를 들어 0.000012를 정규화하면 `1.2 * 10^-5` 값을 가진다.
  - 가수는 1.2 이다.
  - 밑수는 10 이다.
  - 지수는 -5 이다.
- 위에서 구한 이진수 표현인 `1.11101 * 2^2`를 기준으로 설명하면 다음과 같다.
  - 가수는 1.11101 이다.
  - 지수는 2 이다.
  - 밑수는 2 이다.
...
32비트 기준으로 부동 소수점 방식을 자세히 살펴보자.

- 부호(1비트), 지수부(8비트), 가수부(23비트)로 나눈다.
- 지수부는 음의 지수를 처리하기 위해 편향(bias) 값을 127 더한다.
  - 지수부 0 ~ 127 구간은 음의 지수를 의미한다. (-127 ~ 0)
  - 지수부 128 ~ 255 구간은 양의 지수를 의미한다. (1 ~ 128)
- 정규화된 가수부를 표시한다.
- 숫자 7.625는 32비트 부동 소수점 표현으로 0 10000001 11101000000000000000000 이다.
  - 부호 0
...
부동 소수점 방식은 고정 소수점 방식에 비해 표현할 수 있는 수의 범위가 굉장히 크다. 

- 고정 소수점 방식
  - 정수부가 15비트이므로 최대 65535 까지의 숫자만 표현 가능하다.
  - 실수부는 16비트이므로 131071개의 실수만 표현할 수 있다.
- 부동 소수점 방식
  - 가장 작은 실수는 1.175494351E-38 까지 표현 가능하다.
  - 가장 큰 실수는 3.402823466E+38 까지 표현 가능하다.

## 3. Floating Point Number in Java 

컴퓨터는 실수를 정확하게 표현하지 못하는 한계를 가지고 있다. 비단 부동 소수점만의 문제는 아니지만, 어쨌든 부동 소수점 방식은 실수 연산이 부정확하다는 단점을 가지고 있다. (1/2)^n 의 합으로 정확하게 표현할 수 있는 실수는 많지 않다. `Java`에서 실수를 표현하는 `float`, `double`은 부동 소수점 방식을 사용하는데, 이를 사용한 연산은 부정확한 결과를 내놓는다. 

예를 들어 0.1을 이진수로 표현하면 0.0001100110011... 값을 가지는데, 소수점 아래 숫자가 정확하게 나눠 떨어지지 않고 순환된다. 컴퓨터는 숫자를 무한정으로 표현할 수 없기 때문에 가장 근사치 값이 사용된다. 아래 예제 코드처럼 0.1을 1000회 더하면 숫자 100이 되어야 하지만, double 타입을 사용하는 경우 99.99999... 같은 이상한 결과를 얻게 된다.
...
`Java`에서 정확한 계산 결과가 필요한 경우 `BigDecimal`, `int`, `long` 타입을 권장한다. `BigDecimal` 클래스는 다음과 같은 상황에 사용할 수 있다.

- 소수점 연산이 필요한 경우
- 성능에 크게 문제가 없는 경우
- 19자리 이상 숫자를 다뤄야 하는 경우

`int`, `long` 타입은 다음과 같은 상황에 사용할 수 있다. 

- 성능이 민감한 경우
- 소수점 계산을 직접 추적할 수 있고, 숫자가 크지 않은 경우
- `int` 타입은 9자리, `long` 타입은 18자리 미만의 숫자를 다루는 경우

위와 다르게 정확한 계산이 가능하다. 0.1 값을 1000회 더하면 정확히 100 이라는 값을 얻는다.
...
BigDecimal 생성자를 사용하는 방법은 값이 부정확할 수 있다. `valueOf` 메서드를 사용하는 것을 권장한다.
