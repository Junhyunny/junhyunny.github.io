---
title: "동일 로우(row) 중복 삽입 동시성(concurrency) 문제"
search: false
category:
  - java
  - spring-boot
  - jpa
last_modified_at: 2026-04-25T16:30:40+09:00
---
...
동시성 문제는 서버 개발자 커리어를 시작했던 MES(Manufacturing Execution System) 개발 및 운영할 때 처음 경험했다. 크레인, 트레일러, 컨베이어 벨트에 설치된 센서들에서 밀리초(ms) 단위로 메시지를 수신하면서 트랜잭션 경합이 너무 심하게 발생했다. 데이터베이스 락(lock) 메커니즘을 이용한 비관적인 락(pessimistic lock) 방식이나 애플리케이션에서 제어하는 낙관적인 락(optimistic lock) 방식을 통해 데이터 정합성을 지키기 위한 코드를 많이 작성했다.
...
1. 1번 트랜잭션 데이터를 조회하지만, 존재하지 않으므로 insert 쿼리를 수행한다. 하지만 아직 커밋(commit)을 수행하지 않았다.
...
@Table 애너테이션을 통해 유니크 키 제약 조건을 명시한다. 개발, 스테이지, 운영 환경 데이터베이스에 테이블이 존재했기 때문에 제약 조건 추가는 DDL(Data Definition Language)을 통해 직접 변경하였다.
...
다음 비즈니스 로직을 수행하는 CardService 클래스의 코드를 살펴보자.
...
트랜잭션 경합이 발생했을 때 정상적으로 1개의 아이템만 생성되는지 테스트 코드를 통해 살펴보자. 트랜잭션 경합으로 인한 DuplicatedCollectException 예외를 재현하기 위해 CompletableFuture 클래스를 사용해 비동기 처리를 수행한다.
...
DuplicatedCollectException 예외를 처리할 핸들러를 정의한다. 프론트엔드와의 프로토콜을 정의한다. 이번 글에선 600 상태 코드와 예외 메시지를 전달한다.
...
글로벌 핸들러에서 처리한 응답 코드가 예상대로 적용되는지 테스트 코드를 통해 살펴보자. @WebMvcTest 애너테이션을 사용한다. 컨트롤러를 지정하여 테스트 컨텍스트 스코프를 최소화한다.
...
- 세 번의 호출 중 하나는 200, 나머지는 600 상태 코드를 응답받는다.
- `Already collected card` 예외 메시지를 응답받는다.
...
- [잘못된 데이터 업데이트 동시성(concurrency) 문제][concurrency-problem-update-wrong-data-link]
