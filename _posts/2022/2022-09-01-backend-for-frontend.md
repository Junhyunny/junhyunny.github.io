...
오래 전 사용자들은 웹 UI 기반의 애플리케이션 서비스들을 이용하였습니다. 
스마트 폰의 등장과 함께 모바일 애플리케이션 시장이 빠르게 성장했습니다.  
기존 웹 UI 애플리케이션의 기존 백엔드 서비스가 모바일 애플리케이션의 요구사항까지 함께 지원하는 범용 서비스가 되면서 문제가 발생합니다. 
...
* `BFF`에는 프레젠테이션(presentation) 로직만 포함되는지 확인합니다.
    * 애플리케이션은 보통 다음과 같은 로직들을 가지고 있으며, `BFF`는 프레젠테이션 로직만 가지도록 합니다.
