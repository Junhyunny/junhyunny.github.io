---
title: "커스텀 라디오 버튼(Custom Radio Button)"
search: false
category:
  - html
  - css
...
브라우저에서 기본 제공하는 라디오 버튼은 사용하지 않고, 커스터마이징(customizing)하는 방법을 정리하였다.

### 1.1. HTML 코드

- 같은 그룹으로 묶일 라디오 버튼들은 같은 이름을 부여한다.
  - 1번 라디오 버튼 그룹 이름 - `grades`
  - 2번 라디오 버튼 그룹 이름 - `terms`
- 라벨의 `for` 속성 값과 라벨이 눌리면 선택되는 라디오 버튼의 `id` 속성 값을 맞춘다.
  - 예를 들어 1학년 선택 버튼을 보면 다음과 같다.
  - 라벨 for 속성 - `grade-1`
