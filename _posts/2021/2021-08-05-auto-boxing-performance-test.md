---
title: "자바 래퍼 클래스(Java Wrapper Class) 오토 박싱(auto boxing)과 성능 이슈"
search: false
category:
  - java
last_modified_at: 2026-05-07T23:47:39+09:00
---
...
- [자바 래퍼 클래스(Java Wrapper Class)][wrapper-class-link]

## 0. 들어가면서

성능 분석을 위해 [VisualVM][visualvm-link] 모니터링 툴(tool)을 사용했다. `VisualVM`에 `Visual GC` 플러그인(plugin)을 설치하여 가비지 컬렉션(Garbage Collection, GC)도 확인해 보았다.
...
다음은 Runnable 인터페이스를 구현한 MikeTyson 클래스를 살펴보자. MikeTyson 객체의 run 메서드를 보면 snoopIntCopy.getId() 메서드에서 반환한 primitive 타입의 int 값이 containsKey 메서드에 전달되면서 오토 박싱이 발생한다. 테스트를 위해 다음과 같은 main 메서드에서 8개의 데몬 스레드를 생성한 후 작업을 실행한다.
...
- CPU 사용률은 크게 특이 사항이 없다.
- Heap 메모리 사용률을 보면 3300MB의 75% 수준인 2500MB까지 높아졌다가 떨어지는 현상이 반복된다. Heap 사용률이 떨어지는 것은 GC(Garbage Collection, 가비지 컬렉션)가 발생했기 때문이다.
...
Visual GC 플러그인 화면을 보면 객체가 처음 생성될 때 위치하는 에덴(eden) 영역의 메모리가 높아졌다 떨어지는 현상이 자주 반복된다. 에덴 영역의 메모리가 떨어지는 시점에 `GC Time`이 올라가는 것을 보아 가비지 컬렉션이 동작했음을 확인할 수 있다.
...
성능을 최적화한 테스트 코드를 살펴보자. SnoopInt 클래스의 멤버 변수를 래퍼(wrapper) 클래스로 변경하여 이전 코드처럼 containsKey 메서드에서 오토 박싱이 발생하지 않도록 수정했다. SnoopInt 클래스는 아래처럼 래퍼 클래스 타입의 멤버 변수를 지닌 클래스이다.
...
MikeTyson 클래스도 일부 변경이 필요하다. MikeTyson 생성자에서 맵(map) 객체를 준비할 때 Integer 클래스의 valueOf 메서드로 래퍼 클래스 객체가 키에 포함되도록 한다. snoopIntCopy 객체의 getId 메서드로 꺼낸 값은 이미 래퍼 클래스 객체이므로 containsKey 메서드를 수행할 때 오토 박싱이 발생하지 않는다. 나머지 동작은 동일하다.
...
- Heap 메모리 사용률을 보면 3700MB의 33% 수준인 1250MB까지 높아졌다가 감소한다.
...
오토 박싱 테스트에 비해 가비지 컬렉션 수행 빈도가 현저히 적은 것을 확인할 수 있다. 이번에는 Visual GC 모니터링 화면을 살펴보자.

- 에덴 영역의 메모리가 높아졌다 떨어지는 주기가 매우 길다.
- 오토 박싱 테스트에 비해 가비지 컬렉션 수행 빈도가 매우 적다.
...
참고한 글에 따르면 가비지 컬렉션 중에는 시스템이 일순 멈추는 치명적인 문제가 발생한다. 그러므로 가비지 컬렉션 빈도를 최대한 줄이기 위한 노력이 필요하다.
