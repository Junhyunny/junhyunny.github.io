...
애플리케이션 설정에 노출되면 안 되는 민감한 정보들이 있다. 대표적으로 데이터베이스 접속 정보를 예로 들 수 있다. 환경 변수를 통해 주입해주는 것이 안전하지만, 암호화 된 값을 사용하는 방법도 존재한다. 이번 글에선 암호화 된 애플리케이션 설정 값을 사용할 수 있도록 돕는 라이브러리를 소개한다.

## 1. Jasypt(Java Simplified Encryption)

자바(java) 애플리케이션에서 설정 파일에 명시된 값들을 암호화, 복호화 할 수 있는 라이브러리다. 스프링 프레임워크도 자바 애플리케이션이기 때문에 `Jasypt`를 사용할 수 있다. 평문(plain text)을 암호화 한 값을 아래 형식으로 application.yml 파일에 추가하면 된다. `ENC(...)` 내부에 지정된 값은 스프링 애플리케이션이 실행될 때 자동으로 복호화된다.
...
비공개 키는 필수로 입력 받아야 한다. 나머지는 별도 설정이 없다면 다음과 같은 기본 값을 사용한다.
...
1. 빈 이름은 기본 값을 사용한다.
  - 별도로 지정하지 않으면 메서드 이름이 빈 이름이 된다.
  - 메서드 이름을 jasyptStringEncryptor 으로 지정한다.
...
<div align="center">
  <img src="{{ site.image_url_2022 }}/jasypt-01.png" width="80%" class="image__border">
</div>
