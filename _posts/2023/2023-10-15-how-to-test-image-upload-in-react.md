...
HTML `input` 태그의 타입을 `file`로 지정하면 호스트 장치의 지원을 받아 파일을 선택할 수 있다. 파일 선택 후 서버로 업로드하거나 자바스크립트(JavaScript) 코드로 HTML 문서를 조작할 수 있다. 이번 글은 리액트(react) 애플리케이션으로 간단한 파일 업로드 기능을 구현할 때 이를 테스트하는 방법에 대해 정리하였다.

테스트 대상 컴포넌트는 다음과 같다.

- `file` 타입의 `input` 태그를 사용하여 이미지를 선택한다.
- FileReader 객체를 사용해 화면 `img` 태그의 소스(src)를 선택한 이미지로 변경한다.
...
RTL(React Testing Library) userEvent 모듈의 upload 함수를 사용하면 이미지 파일 선택 기능을 테스트할 수 있다. 검증 방법은 화면에 보이는 `img` 태그의 소스 값이 변경되었는지 확인한다. 파일 선택과 동시에 서버로 업로드한다면 axios 같은 Rest API 요청 모듈을 테스트 더블(test double)로 사용한다.

- 테스트를 위한 파일 더미(dummy)를 생성한다.
- App 컴포넌트를 렌더링(rendering)한다.
- userEvent 모듈의 upload 함수를 사용해 파일 선택 이벤트를 발생시킨다.
- 화면의 이미지 태그의 소스가 변경되었는지 확인한다.
  - FileReader 객체의 readAsDataURL 메서드를 사용하면 이미지 바이너리(binary) 정보가 Base64 방식으로 인코딩된다.
