---
title: "맥용 Sourcetree에서 GitHub 공개 데이터만 접근되는 문제"
search: false
category:
  - information
...
새로운 회사에서 받은 맥북으로 개발 환경을 만드는 과정에서 문제가 발생했다. 난생 처음 사용하는 맥북에 익숙하지 않은 탓도 컸지만, 다른 원인이 있어 보였다. 형상 관리 툴(tool)로 사용하는 소스트리(Sourcetree)가 정상적으로 동작하지 않았는데, 윈도우즈에서 발생했던 문제와는 또 달랐다.
...
맥북에서 소스트리와 GitHub 연동 시 권한을 충분히 부여하지 못하는 문제가 있는 것으로 보인다. 맥북의 보안상의 문제인지 원인은 확인할 수 없었다.
...
해당 문제는 GitHub 토큰(token)을 발급받아 사용하면 해결할 수 있다.
...
- 발급받은 토큰을 복사한다.
- 소스트리 애플리케이션에서 Sourcetree > 설정 > 계정 > 추가(혹은 편집) 선택
- 인증 방식 - 베이직(Basic)
