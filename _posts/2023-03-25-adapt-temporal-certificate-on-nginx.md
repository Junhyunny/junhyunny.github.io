...
* 프로젝트에 `ssl` 폴더를 생성합니다.
* `openssl` 명령어를 통해 인증서를 생성합니다.
    * `req -x509` 
        * `-x509` 옵션을 통해 인증서 요청(CSR, certificate Signing Request)이 아닌 PKCS#10 형식의 인증서를 즉시 발급합니다.
        * 테스트 용으로 많이 사용하는 방법입니다.
    * `-days 30` 
...
* 공인 `CA(certificate authority)`에서 발급한 인증서가 아니기 때문에 경고 메세지가 보여집니다.
