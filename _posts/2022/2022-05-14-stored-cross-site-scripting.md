...
지난 포스트에 이어서 저장형 XSS(Stored Cross Site Scripting) 공격과 방어에 대해 알아보겠다. 중복된 내용들은 간단하게 정리하고 본론으로 들어가겠다.
...
저장형 XSS 공격은 보안이 취약한 서버에 악의적인 사용자가 악성 스크립트를 저장함으로써 발생한다. 비정상적인 방법이 아니라 서버에서 제공하는 게시판, 사용자 프로필에 악의적으로 동작하는 스크립트가 그대로 저장된 후 클라이언트의 브라우저로 전달되어 문제가 발생한다. 간단한 시나리오를 바탕으로 예제 코드를 살펴보겠다.

### 1.1. 저장형 XSS 공격 시나리오

1. 악의적인 사용자가 보안이 취약한 사이트를 발견했다.
1. 보안이 취약한 사이트에서 제공하는 게시판에 사용자 정보를 빼돌릴 수 있는 스크립트를 작성하여 올린다.
1. 일반 사용자는 악의적인 사용자가 작성한 게시글을 읽으면, 서버로부터 악성 스크립트가 담긴 게시글 응답을 전달받는다.
...
보안이 취약한 서비스의 코드를 살펴보겠다. 불필요한 코드는 제외하고 문제를 일으키는 코드만 확인해보겠다.

#### 1.2.1. Posts JSP

- 화면의 왼쪽은 게시글 목록이다.
- 화면의 오른쪽은 글을 작성할 수 있는 입력 폼(form)이다.
- 왼쪽의 게시글 하나를 선택하면 상세 게시글 화면으로 이동한다.
...
- `/post` 경로 요청 시 사용자 입력을 검증 없이 그대로 데이터베이스에 저장한다.
- `/post/{postId}` 경로 요청 시 작성된 게시물 정보를 그대로 화면으로 전달한다.
...
1. 악의적인 사용자는 악성 스크립트가 담긴 게시글을 작성한다.
1. 일반 사용자는 악의적인 사용자가 작성한 게시글을 보기 위해 클릭한다.
1. 게시글 조회 시 작성된 악성 스크립트가 실행된다.
...
- `removeTag` 메서드에서 `<`, `>` 문자를 HTML에서 사용하는 특수 문자로 변경한다.
...
- 게시글을 저장하기 전에 `removeTag` 메서드를 통해 내용을 정리한다.
...
사용자의 입력을 검증하지 않고 그대로 사용하는 경우 XSS 공격에 취약한 모습을 보이는 것 같다. 안정적인 서비스 운영을 위해선 사용자 입력을 검증 후 변경하고, 화면 출력도 안정적인 방법으로 수행할 필요가 있다. 이번 포스트에선 간단한 방법으로 사용자의 입력을 변경 후 저장하였다. 필터(filter)나 컨버터(converter)를 사용하면 예시보다 효율적인 방법으로 검증 및 변경 작업이 가능할 것이다. 기회가 된다면 간단한 예시 코드를 작성하고 소개하는 포스트를 작성해봐야겠다. 다음 포스트는 DOM 기반 XSS(DOM based XSS) 공격과 방어에 대한 내용을 다뤄보겠다.
