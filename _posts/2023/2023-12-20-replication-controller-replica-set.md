---
title: "쿠버네티스(Kubernetes) ReplicationController와 ReplicaSet"
search: false
category:
  - container
  - kubernetes
...
고가용성(high availability)이란 시스템이 오랜 기간 동안 지속적으로 정상 운영이 가능한 성질을 의미한다. 이를 가능하게 만드는 가장 쉬운 방법은 백업(backup)을 두는 것이다. 둘 중 하나에 문제가 생기더라도 나머지가 지속적인 서비스를 가능하게 해준다. 레플리케이션 컨트롤러는 여러 개의 파드를 그룹지어 관리하기 때문에 시스템의 고가용성을 보장한다.
