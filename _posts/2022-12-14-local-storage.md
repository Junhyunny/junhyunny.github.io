...
* `localStorage` 객체에 저장한 데이터는 브라우저에 의해 자동으로 서버에게 요청되지 않습니다. 
* `localStorage` 객체에 저장한 데이터는 브라우저를 다시 실행해도 사라지지 않습니다. 
* 대부분의 브라우저들은 최소 2MB, 최대 5BM 정도의 데이터를 저장할 수 있습니다.
* 서버가 HTTP 응답 헤더를 통해 `localStorage` 객체에 저장된 데이터를 제어할 수 없습니다. 
