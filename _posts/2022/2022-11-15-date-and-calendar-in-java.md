...
요즘 시간에 예민한 애플리케이션을 개발하면서 시간 도메인에 대해 공부하는 중이다. 이전 포스트에선 프론트엔드에서 시간을 다루는 방법에 대해 정리하였다. 이번 포스트에선 `JDK 8` 버전 전에 사용하던 날짜, 시간과 관련 API 기능들과 몇 가지 문제점들에 대해 살펴보려고 한다.

## 1. java.util.Date

`JDK 1.0`부터 사용한 시간, 날짜 관련 클래스이다. 이전 버전과 호환성을 위해 계속 제공하는 것 같지만, 현재 대부분의 기능이 더 이상 사용되지 않는다. `Calendar` 클래스의 기능으로 대부분 대체되었다.

- 내부적으로 UNIX 시간을 관리한다.
  - GMT 1970년 1월 1일 0시 0분 0초를 기준으로 지난 시간을 ms(밀리 세컨드)로 측정한 값이다.
- `Date` 클래스는 UTC 시간을 반영하지만, JVM 호스트 환경에 따라 정확하지 않을 수 있다.
  - 윤초(leap second) 계산이 정확하게 이뤄지지 않는다.
- 시간을 문자열로 변경하거나 문자열을 시간으로 파싱(parsing)할 수 있다.
- 내부적으로 로컬 타임존에 따라 시간이 계산된다.
- 메서드 대부분이 `deprecated` 되었다.
  - <https://docs.oracle.com/javase/8/docs/api/java/util/Date.html>

## 2. Pain Points of Date Class

`Date` 클래스는 다음과 같은 문제점들을 가지고 있다.

### 2.1. Thread Not Safe

- 해당 테스트 코드는 항상 성공하지 않는다.
- `Date` 객체는 동시성에 취약하다.
  - 캡슐화 실패로 `Date` 객체 내부에서 관리하는 UNIX 시간을 외부에서 변경할 수 있다.
- 객체의 멤버 변수로 사용하는 것은 위험하다.
...
- 생성자에 년도 파라미터에 122 값을 전달하면 2022년이 반환된다.
- 코드 내부를 확인하지 않으면 년도를 1900에 더해서 사용한다는 사실을 모른다.
...
`JDK 1.1`에 생긴 시간, 날짜 관련 추상 클래스이다. 추상 클래스이므로 생성자가 아닌 팩토리 메서드 패턴이 적용된 `getInstance` 메서드를 사용해 객체를 생성한다.

- 내부적으로 UNIX 시간을 관리한다.
- 지역에 따라 적절한 구현체를 `getInstance` 메서드를 통해 생성한다.
...
`Calendar` 클래스는 다음과 같은 문제점들을 가지고 있다.

### 4.1. Thread Not Safe

- 해당 테스트 코드는 항상 성공하지 않는다.
- `Calendar` 객체는 동시성에 취약하다.
  - 캡슐화 실패로 `Calendar` 객체 내부에서 관리하는 UNIX 시간을 외부에서 변경할 수 있다.
- 객체의 멤버 변수로 사용하는 것은 위험하다.
...
- 정수로 정의된 상수를 사용한다.
  - `DATE`의 정수 값은 5이다.
  - `THURSDAY`의 정수 값은 5이다.
- 같은 상수 값을 사용하는 경우엔 컴파일 시점에 에러를 잡지 못한다.
- 아래 코드는 일 기준으로 1일을 더하고, 목요일 기준으로 1일을 더하는 로직으로 보인다.
- 실제로 일(date) 기준으로 2일이 더해진 값이 반환된다.
...
- `Date` 클래스의 0부터 달을 지정하는 방식을 유지하였다.
- 코드는 10월 객체를 생성한 것 같지만, 실제로 11월 객체를 생성한다.
...
- 타임존을 지정할 때 잘못된 값을 사용해도 에러가 발생하지 않는다.
  - 존재하지 않는 `Seoul/Asia` 타임존 ID를 지정하면 `GMT`를 사용한다.
...
`JDK 8`이 릴리즈되기 전엔 기본으로 제공되는 날짜, 시간 API 기능을 대체할 수 있는 라이브러리를 사용했다고 한다.
...
`JDK 8`부터 날짜와 시간과 관련된 API 기능들이 많이 개선되었고, 관련된 클래스들은 `java.time` 패키지에 위치한다.
