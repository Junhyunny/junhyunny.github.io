...
* 인증에 실패하면 예외가 발생합니다.
    * 예외가 발생하면 401 코드와 에러 메시지를 전달합니다.
...
* 각 비즈니스 케이스에 맞는 동작을 수행하는지 검증합니다.
    ...
    * 인증 실패인 경우 적절한 에러 메시지와 상태 코드를 확인합니다.
...
* 에러가 발생했을 때 에러 코드와 메시지가 클라이언트에게 전달되는지 확인하기 어렵습니다. 
...
* 각 비즈니스 케이스에 맞는 동작을 수행하는지 검증합니다.
    * 요청이 인증 처리가 필요 없는 경우 클라이언트는 정상적인 응답 메시지를 받습니다.
    * 요청이 인증 실패인 경우 클라이언트는 401 에러 코드와 메시지를 받습니다.
    * 요청이 성공인 경우 정상적인 응답 메시지를 받습니다. 
...
애플리케이션 실행 시 설정(config) 혹은 빈(bean) 주입까지 정상적으로 수행되는지 확인하려면 결합 테스트를 수행해야 합니다. 
@SpringBootTest 애너테이션을 활용한 결합 테스트는 모든 컨텍스트를 필요로 하기 때문에 실행 속도가 느립니다. 
테스트를 통해 조금이라도 빠른 피드백을 받기 위해 @WebMvcTest 애너테이션을 사용합니다.

* @SpringBootTest 애너테이션을 사용하면 애플리케이션 실행에 필요한 모든 컨텍스트를 띄웁니다.
