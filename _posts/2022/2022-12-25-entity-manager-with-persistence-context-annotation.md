...
[EntityManagerFactory 클래스][entity-manager-with-entity-manager-factory-link] 포스트에선 스레드 안전(thread safe)하지 않은 엔티티 매니저(EntityManager)를 안전하게 사용하는 방법들 중 한 가지인 `EntityManagerFactory` 클래스에 대해 정리하였다. 이번엔 `@PersistenceContext` 애너테이션을 통해 엔티티 매니저의 스레드 안전성을 도모하는 방법에 대해 정리해보았다.

## 1. @PersistenceContext 애너테이션

`@Autowired` 애너테이션을 통해 빈(bean)을 주입 받듯이 `@PersistenceContext` 애너테이션을 사용하면 `EntityManager` 빈을 주입 받을 수 있다. `EntityManager`의 실제 구현 클래스의 객체가 아닌 프록시(proxy) 객체를 주입 받는다.
...
주입 받은 엔티티 매니저를 호출하면 프록시 내부에 `invocationHandler` 객체로 등록된 `SharedEntityManagerInvocationHandler` 인스턴스를 사용한다. `SharedEntityManagerInvocationHandler` 인스턴스의 `invoke` 메서드를 살펴보면 다음과 같은 내용을 확인할 수 있다.

- `EntityManagerFactoryUtils.doGetTransactionalEntityManager` 호출 라인
  - 해당 스레드에 트랜잭션이 시작된 엔티티 매니저가 있다면 이를 반환한다.
  - 해당 스레드에 엔티티 매니저가 없다면 새로운 엔티티 매니저를 생성 후 반환한다.
- `Object result = method.invoke(target, args)` 호출 라인
  - 획득한 엔티티 매니저(target)로 프록시 외부에서 호출한 메서드를 실행한다.
...
`EntityManagerFactory` 클래스를 직접 사용하면 `@Transactional` 애너테이션을 통한 트랜잭션 처리가 불가능하다. 반면에 `@PersistenceContext` 애너테이션을 통해 주입 받은 엔티티 매니저를 사용하면 `@Transactional` 애너테이션을 통한 트랜잭션 처리가 가능하다. 간단한 예시 코드를 통해 확인해보겠다.

### 3.1. PcAnnotationService 클래스

- 데이터 생성 후 조회하는 간단한 비즈니스 로직을 트랜잭션 처리한다.
...
- 엔티티 매니저를 `@PersistenceContext` 애너테이션을 통해 주입 받는다.
- 데이터 추가, 조회 기능을 제공한다.
...
위에서 다룬 내용들을 요약하면 다음과 같다.

- `@PersistenceContext` 애너테이션을 사용하면 엔티티 매니저 프록시 객체를 주입 받아서 사용한다.
- 프록시 객체를 통해 스레드 별로 엔티티 매니저를 생성하고 사용하기 때문에 스레드 안정성을 보장한다.

일반적으로 `@Autowired` 애너테이션을 통해 빈을 주입 받으면 애플리케이션 전역에서 사용 중인 객체를 전달받는다. 때문에 엔티티 매니저를 `@Autowired` 애너테이션을 통해 주입 받는 것은 위험해보인다. 하지만, 실제로 `@Autowired` 애너테이션으로 엔티티 매니저를 주입 받으면 `@PersistenceContext` 애너테이션과 동일하게 프록시 객체를 주입받는다. 생성자 주입(constructor injection)을 통해 전달 받는 엔티티 매니저도 동일하다.
...
예전엔 반드시 `@PersistenceContext` 애너테이션을 사용해야 했지만, 특정 버전 이상부터는 `@Autowired` 애너테이션도 동일한 기능을 제공하는 것으로 보인다. 공식 문서에선 확인하지 못 했지만, 관련된 내용을 `StackOverflow`에 문의한 결과 다음과 같은 답변을 받을 수 있었다.
