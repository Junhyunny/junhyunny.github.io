...
공식 문서를 보면 캐시 액션을 사용한 예제를 볼 수 있다. 설정이 직관적이기 때문에 예시를 이해하는데 크게 어려움은 없다. 필자의 블로그에서 사용하는 지킬(jekyll)은 `}}`, `{{` 블럭을 변수 처리 때문에 정상적으로 보이지 않는다. 아래처럼 `{ { } }` 처럼 일부러 띄어 작성했다. 복사 후 사용한다면 괄호는 붙여서 사용하길 바란다.
