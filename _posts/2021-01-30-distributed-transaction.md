...
분산 트랜잭션의 정의를 우선 살펴보도록 하겠습니다. 
> 2개 그 이상의 네트워크 상의 시스템 간의 트랜잭션<br>
> 일반적으로 서버들은 트랜잭션 리소스를 제공하고 트랜잭션 관리자는 이러한 리소스에 대한 모든 작업을 포함하는 전역 트랜잭션을 만들고 관리한다.<br>
> \- Wikipedia

실제로 사용되는 방법들이 어떤 메커니즘을 통해 분산된 트랜잭션 처리를 하나의 트랜잭션처럼 수행하는지 이론적인 내용에 대해서 알아보도록 하겠습니다. 
...
1. First Phase(또는 Prepare Phase)
  - 각 서비스들이 commit을 하기 위한 준비 요청 단계입니다.
  - 트랜잭션 매니저 서비스로부터 commit 준비를 요청을 받습니다.
  - 각 서비스들은 commit 준비 성공 여부에 따라 트랜잭션 매니저 서비스로 성공 또는 실패 여부를 알립니다.
1. Second Phase(또는 Commit Phase)
  - 분산 트랜잭션에 참여한 모든 서비스로부터 준비 완료에 대한 메세지를 받을 때까지 대기합니다.
  - 모든 서비스로부터 준비 성공 응답을 받는 경우 트랜잭션 매니저는 commit 메세지를 보내 모든 작업을 commit 합니다.
  - 하나의 서비스라도 준비 성공 응답을 받지 못한 경우 트랜잭션 매니저는 rollback 메세지를 보내 모든 작업을 rollback 합니다.

##### 2단계 commit 성공 / 실패 시나리오
<p align="center"><img src="/images/distributed-transaction-1.JPG"></p>

## Saga pattern
마이크로 서비스 아키텍처 환경에서는 서로 다른 서비스가 각각의 API end point를 통해 요청을 처리하기 때문에 `2단계 commit`은 사용하기 어렵습니다. 
마이크로 서비스 아키텍처에서 `2단계 commit` 대신 Saga 패턴을 통해 분산 트랜잭션을 처리합니다.
Saga 패턴에서 트랜잭션의 관리 주체는 어플리케이션(서비스)입니다. 
서비스들끼리 이벤트를 주고 받으며 각 서비스들은 자신이 소유권을 가진 데이터베이스의 트랜잭션에만 관심이 있습니다.
`2단계 commit`처럼 데이터의 원자성이 보장되지 않기 때문에 `eventual consistency`를 보장하며 보상 트랜잭션 처리가 필요합니다.
...
이벤트 전달은 Kafka와 같은 메세지 큐를 이용하여 비동기 방식으로 전달합니다. 
...
`2단계 commit` 방법은 데이터베이스에 의해 처리되는 방식이라고 하니 구현 예제를 만드는 것이 어려울 듯 합니다. 
`Saga 패턴` 방식을 직접 구현하고 싶은데 고민해야 될 내용이 너무 많을 듯 합니다. 

기본적으로 떠오르는 것들을 정리해보았습니다. 
- 트랜잭션에 참여한 서비스 정보(ip, port, instance no)를 찾는 방법
- 분산된 서비스들 사이에서 발생한 트랜잭션을 동일한 트랜잭션으로 처리할 수 있도록 만드는 Shared Key 
